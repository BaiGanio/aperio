...
// Converts the unit test reporter output into docs/tools/dashboards/unit/unit-data.js.
...
const outputPath = resolve(ROOT, option("--output", "docs/tools/dashboards/unit/unit-data.js"));
...
async function run() {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          groups[group].push({
            ...
            path: relative(unitDir, fullPath),
          });
        }
      }
    }
  ...
  await writeFile(
    outputPath,
    `window.APERIO_UNIT = ${JSON.stringify(data)};\n`,
    "utf8"
  );
...
run().catch((err) => {
  console.error("unit dashboard generation failed:", err.message);
  process.exit(1);
});
