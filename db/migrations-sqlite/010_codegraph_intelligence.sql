-- 010_codegraph_intelligence.sql — SQLite
...
-- Mirror of db/migrations/010_codegraph_intelligence.sql — keep in lockstep.
...
ALTER TABLE cg_edges ADD COLUMN confidence TEXT NOT NULL DEFAULT 'EXTRACTED'
  CHECK (confidence IN ('EXTRACTED', 'INFERRED', 'AMBIGUOUS'));
ALTER TABLE cg_edges ADD COLUMN confidence_score REAL
  CHECK (confidence_score IS NULL OR (confidence_score >= 0 AND confidence_score <= 1));
ALTER TABLE cg_edges ADD COLUMN provenance TEXT;
ALTER TABLE cg_edges ADD COLUMN relation_context TEXT;
...
ALTER TABLE cg_repos ADD COLUMN index_schema_version INTEGER NOT NULL DEFAULT 0;
ALTER TABLE cg_repos ADD COLUMN graph_revision       INTEGER NOT NULL DEFAULT 0;
ALTER TABLE cg_repos ADD COLUMN analyzed_revision    INTEGER;
ALTER TABLE cg_repos ADD COLUMN analyzed_at          TEXT;
