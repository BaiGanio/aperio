...
// Thrown when an encryption key EXISTS in the key store but cannot be read
// (keychain locked, or an ACL that needs user interaction). Callers must treat
// this as "do not regenerate" — overwriting a key that still protects real data
// would make the encrypted database permanently undecryptable.
...
function macStoreKeyRaw(keyHex) {
  ...
  // Key is passed via stdin (-w with no value) so it never appears in `ps`.
...
function macStoreKey(keyHex) {
  macStoreKeyRaw(keyHex);
  logger.info('[encrypt] Key stored in macOS Keychain');
}

// One-time-per-process, best-effort: re-store an existing key with -A so future
// launches (and spawned child processes) can read it without a per-launch
// keychain prompt. Keys created before the -A fix (#180) carry a restrictive
// ACL; this upgrades them transparently the first time a read succeeds. Never
// fatal — if it fails, the only cost is the prompt may reappear next launch.
let _aclUpgradeAttempted = false;
function maybeUpgradeKeyAcl(keyHex) {
  if (!isMac || _aclUpgradeAttempted) return;
  _aclUpgradeAttempted = true;
  try {
    macStoreKeyRaw(keyHex);
    logger.debug('[encrypt] Keychain key access-control refreshed (-A)');
  } catch (err) {
    logger.debug(`[encrypt] Keychain ACL refresh skipped: ${err.message}`);
  }
}
...
function macGetKey() {
  try {
    return execFileSync('security', [
      ...
      // timeout: a restrictive ACL can pop a blocking SecurityAgent dialog. In a
      // spawned/background process that dialog may never be answered, which would
      // hang startup (the original "frozen terminal" symptom). Cap the wait so we
      // fail fast into the clear KeyUnreadableError guidance instead of hanging.
      encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'], timeout: 8000
    }).trim();
  } catch (err) {
    // errSecItemNotFound → `security` exits 44: the key genuinely does not
    // exist, so the caller may safely generate a fresh one. ANY other failure
    // (keychain locked, or an ACL that needs interaction — exit 45/128) means
    // the key most likely EXISTS but we can't read it; regenerating would
    // orphan an existing encrypted DB, so surface it instead of returning null.
    if (err && err.status === 44) return null;
    throw new KeyUnreadableError(
      'The database encryption key is in the macOS Keychain but could not be read ' +
      '(the keychain may be locked, or the key predates the access-control fix). ' +
      'Run `npm run db:fix-keychain` to repair access, or approve the Keychain prompt if one appears.'
    );
  }
}
...
function winStoreKey(keyHex) {
  ...
  // Key is read from stdin so it never appears on the command line.
  ...
  execFileSync('powershell', [
    ...
    `Set-Content -Path '${escapedPath}' -Value $e`
  ], { input: keyHex, stdio: ['pipe', 'ignore', 'pipe'] });
...
function winGetKey() {
  ...
  try {
    ...
    return execFileSync('powershell', [
      '-NoProfile', '-NonInteractive', '-Command',
      `$e=Get-Content -Path '${escapedPath}' -Raw;` +
...
export function encryptFile(srcPath, destPath, keyBuf) {
...
/**
 * True if the file at `path` begins with the plaintext SQLite header. Used to
 * tell an encrypted blob from a normal SQLite file without opening it.
 */
export function isPlaintextSqlite(path) {
  try {
    return readFileSync(path).toString('utf8', 0, 16) === SQLITE_MAGIC;
  } catch {
    return false;
  }
}
...
/**
 * Read the EXISTING encryption key without ever generating one. Returns a
 * 32-byte Buffer, or null if no key exists. Throws KeyUnreadableError if a key
 * exists but cannot be read. Used by the "encryption disabled but file still
 * encrypted" reconcile in sqlite.js, which must decrypt with the original key
 * but must never mint a new one (that would orphan the data).
 * @returns {Buffer|null}
 */
export function readExistingKey() {
  const keyHex = getKey();
  return keyHex && keyHex.length === 64 ? keyToBuffer(keyHex) : null;
}
...
export function getOrCreateKey() {
  ...
  if (keyHex && keyHex.length === 64) {
    ...
    maybeUpgradeKeyAcl(keyHex);   // transparently repair pre-#180 ACLs
