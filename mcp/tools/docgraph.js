...
const TOOLS = [
  ...
  {
    name: "doc_batch",
    description: "Read a doc_manifest candidate list in bounded batches. Returns one entry per document with found/read/skipped coverage and per-file reasons. Each read document carries the raw `text` (for verification) plus structured evidence extracted from it: `dates` — an array of {role, raw, value, confidence}, where role is one of invoice_date/document_date/statement_date/receipt_date/payment_date/due_date/service_period_start/service_period_end/unlabeled_date, and `value` is ISO YYYY-MM-DD or null when the raw token's format is ambiguous; `amounts` — an array of {value, currency, raw, label}, where currency is a 3-letter code or null when undetectable, and label (e.g. amount_due/subtotal/total/paid) is null when no nearby money label was found in a known language. label can also be 'likely_total' — a best-effort guess (the last currency-bearing amount with no matched label, most invoices print the amount actually owed after any breakdown/subtotal/tax lines) for documents in a language the extractor doesn't have label patterns for yet; treat it as lower-confidence than a real label, and still cross-check against `text` before reporting a figure to the user. An empty `dates`/`amounts` array means none were found — never treat a missing field as zero or as 'not present in the source', only as 'not detected by extraction'; fall back to `text` to check. Do not call once per file; pass the manifest candidates together.",
    schema: {
