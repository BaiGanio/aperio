// tests/reporters/unit-json.js
// Custom test reporter that outputs structured JSON for the unit dashboard.
// Only events whose source file is under tests/unit/ are included.
// Groups by the subdirectory immediately under tests/unit/ (mirrors
...
function isUnitFile(file) {
  return typeof file === "string" && /[/\\]tests[/\\]unit[/\\]/.test(file);
}

export function createUnitReporter() {
  const tests = [];
...
export default createUnitReporter();
