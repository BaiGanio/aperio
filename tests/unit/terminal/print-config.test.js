...
function buildExpectedOutput({ get, precedence, warnings }) {
  ...
  if (provider === "llamacpp") {
    out += formatRow("LLAMACPP_MODEL", "LLAMACPP_MODEL", get, "unsloth/gemma-4-E4B-it-qat-GGUF:Q4_K_XL");
...
describe("CLI /config output format", () => {
  ...
  test("unset vars show default fallback values", () => {
    ...
    assert.ok(plain.includes("unsloth/gemma-4-E4B-it-qat-GGUF:Q4_K_XL"), "unset LLAMACPP_MODEL shows curated fallback");
