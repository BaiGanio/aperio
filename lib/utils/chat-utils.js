...
export function redrawHeader() {
  ...
  // Live state replaces the old jargon bar: "ready" when idle, the spinner
  // label (e.g. "thinking…") while working. This is the always-visible
  // confirmation that Aperio is up and listening.
  ...
  // Navbar: the at-a-glance system strip — mode, docker, storage. Dim so it
  // reassures without competing with the friendly line above.
  const line4 = `${GRAY}${DIM}  ${_headerMode}  ·  Docker ${_headerDockerOn ? "on" : "off"}  ·  ${_headerDB}${R}`;
...
export function getHeaderInfo() {
  return { mode: _headerMode, model: _headerModel, dockerOn: _headerDockerOn, db: _headerDB };
}
