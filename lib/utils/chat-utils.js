...
/** Closest ANSI stand-in for the aurora purple the mascot wears everywhere else. */
...
export function redrawHeader() {
  ...
  // Navbar: the at-a-glance system strip — mode, docker, storage, language,
  // and context pressure. Dim so it reassures without competing with the
  // friendly line above.
  const langName = LANG_NAMES[_headerLang] || "English";
  const ctxPart = _headerCtxPct != null ? `  ·  ctx ${_headerCtxPct}%` : "";
  const line4 = `${GRAY}${DIM}  ${_headerMode}  ·  Docker ${_headerDockerOn ? "on" : "off"}  ·  ${_headerDB}  ·  ${langName}${ctxPart}${R}`;
...
/** Update the context pressure % shown in the navbar. null hides it. */
export function setHeaderCtxPct(pct) {
  _headerCtxPct = pct;
}
...
// ─── llama.cpp helpers ────────────────────────────────────────────────────────
export function parseLlamaCppPort(env = process.env) {
  return Number.parseInt(env.LLAMACPP_PORT || "8080", 10);
}

export function llamacppBase(llamacppPort) {
  return process.env.LLAMACPP_BASE_URL || `http://127.0.0.1:${llamacppPort}`;
}

export async function llamacppHealthy(llamacppPort) {
  try {
    const r = await fetch(`${llamacppBase(llamacppPort)}/health`, { signal: AbortSignal.timeout(1500) });
    return r.ok;
  } catch { return false; }
}
