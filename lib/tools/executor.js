...
// Membership means "a mangled argument here can do damage nobody gets to see
// first" — it disables JSON repair, schema hints, and in-turn result replay.
// `db_execute` is deliberately NOT a member (2026-08-14): it is two-phase
// confirm-gated, and its proposal renders connection, statement type, SQL and
// params to the user before anything runs (lib/handlers/database/
// databaseHandlers.js), so every field a repair could alter is reviewed by a
// human first. That is the property the other entries lack — see
// mcp/tools/extraction.js, which notes that extraction_template_delete is
// "protected only by DESTRUCTIVE_TOOLS' JSON-repair refusal ... not a
// two-phase interrupt like db_execute". Its membership was costing real
// tool-calling accuracy on the one tool WS2 depends on: two local models each
// burned a turn re-emitting an identical malformed db_execute (see
// id/reference/tech-debt.md, "db_execute argument validation").
...
// the corruption guard on a file or direct-write tool is never a config away.
// (A user who wants db_execute back under the guard can add it via the env var.)
...
// Ornith also leaks a SECOND, angle-bracket shape — not the bbcode one above.
// Recorded verbatim from a live run (id/reference/tech-debt.md, "Ornith tool-
// call leakage — angle-bracket shape unparsed"):
//
//   <tool_call> <function=db_schema> <parameter=connection> aperio </parameter> </function> </tool_call>
//
// `detectToolCallLeak()`'s generic `<tool_call>` pattern already flags this as
// a leak (triggering the retry-with-thinking-suppressed path), but neither
// this file's other extractors nor the retry recovered it — the model kept
// re-emitting the identical shape, and the retry gave up with "I tried to use
// one of my tools but couldn't issue the call correctly" every time. A call
// with no parameters at all (`<function=db_connections> </function>`) is real
// and observed too, so the parameter tag is optional, not required.
export function extractAngleToolCall(text) {
  if (!text) return null;
  const head = text.match(/<tool_call>\s*<function\s*=\s*([a-zA-Z_][\w-]*)\s*>/i);
  if (!head) return null;
  const rest = text.slice(head.index + head[0].length);
  const input = {};
  let lastEnd = 0;
  const argRe = /<parameter\s*=\s*([a-zA-Z_][\w-]*)\s*>([\s\S]*?)<\/parameter>/gi;
  let m;
  while ((m = argRe.exec(rest)) !== null) {
    const raw = m[2].trim();
    let val = raw;
  ...
  const trailing = rest.slice(lastEnd)
    .replace(/<\/function>\s*<\/tool_call>/i, "")
    .replace(/^[-–—\s]*(?:Response|Result|Answer|Output)\s*:\s*/i, "")
    .trim();
  return { name: head[1], input, trailing };
}
...
// (llama.cpp's own internal template quote marker — the same marker id/
// reference/tech-debt.md's "db_execute — a malformed call the model is never
// told how to fix" found leaking into args elsewhere, here leaking into the
// answer text instead). Recorded verbatim (tech-debt.md, "gemma-4-E4B tool-
// call leakage — pipe-delimited shape"):
//
//   <|tool_call>call:doc_search{query:<|"|>Northwind Labs<|"|>}<tool_call|>
//
// `detectToolCallLeak()`'s generic `<\|\s*tool_call\s*\|?>` pattern already
// flags this as a leak; this recovers it by swapping the quote marker back to
// `"` and parsing the `call:name{...}` body as a JSON-object literal (bare
// keys allowed, same as the plain `call:name{...}` fallback below).
export function extractPipeAngleToolCall(text) {
  if (!text) return null;
  const normalized = text.replace(/<\|\s*"\s*\|>/g, '"');
  const head = normalized.match(/\bcall\s*:\s*([a-zA-Z_][\w.\-]*)\s*\{/);
  if (!head) return null;
  const braceStart = head.index + head[0].length - 1;
  const end = matchBrace(normalized, braceStart);
  if (end === -1) return null;
  let parsed;
  try { parsed = JSON.parse(quoteBareKeys(stripJsonComments(normalized.slice(braceStart, end)))); } catch { return null; }
  if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) return null;
  const cleaned = Object.fromEntries(Object.entries(parsed).filter(([, v]) => v != null && v !== "" && v !== "None" && v !== "null"));
  const trailing = normalized.slice(end).replace(/<\s*\|?\s*tool_call\s*\|?\s*>/gi, "").trim();
  return { name: head[1], input: cleaned, trailing };
}

// `toolNames`, when non-empty, bounds every fallback below to a REAL registered
// tool. Without this gate the bracket-free fallbacks below match on shape alone
// (bare "identifier(...)" / "identifier{...}" at the end of the text), so a
// perfectly ordinary answer that happens to end in a parenthetical — e.g. "US
// Letter (612 × 792 points)" — is misread as a call to a tool named "Letter"
// and dispatched for real, which MCP then legitimately rejects as unknown
// (-32602). Callers that can't supply the registry (none currently) still get
// the old permissive behavior by omitting the argument.
export function extractTextToolCall(text, messages, toolNames = []) {
  const known = (name) => !toolNames.length || toolNames.includes(name);
  const bracket = extractBracketToolCall(text);
  if (bracket && known(bracket.name)) return bracket;
  const angle = extractAngleToolCall(text);
  if (angle && known(angle.name)) return angle;
  const pipeAngle = extractPipeAngleToolCall(text);
  if (pipeAngle && known(pipeAngle.name)) return pipeAngle;
...
// it back instead of re-running the tool. Stateful tools are deliberately
// excluded: a repeated run_node_script after write_file may be running a new
// script revision, so replaying an old failure would make repair impossible.
// Scoped to the CURRENT turn (after the last real user message) so a legitimate
// re-ask in a later turn still re-runs.
...
export function findPriorToolResult(messages, name, args) {
  if (DESTRUCTIVE_TOOLS.has(name)) return null;
...
// The tool-result side of a call. Two shapes reach here: the OpenAI-compatible
// `{role:"tool", content:[…]}` executeToolCalls pushes, and the Anthropic-style
// `{role:"user", content:[{type:"tool_result"}]}` executeInterceptedToolCall
// pushes. Both must be skipped when measuring back-to-back repeats.
function isToolResultMessage(m) {
  if (m?.role === "tool") return true;
  return m?.role === "user" && Array.isArray(m.content) &&
    m.content.length > 0 && m.content.every(b => b?.type === "tool_result");
}

// Signature of an assistant message's tool calls: name + order-independent args
// for every tool_use block, or null when the message issues no call. A leading
// text block (a tool preamble) does not disqualify the message — the model is
// still calling a tool, not answering.
function toolCallSignature(m) {
  if (m?.role !== "assistant" || !Array.isArray(m.content)) return null;
  const uses = m.content.filter(b => b?.type === "tool_use");
  if (!uses.length) return null;
  return uses.map(b => `${b.name}:${stableStringify(b.input ?? {})}`).join("|");
}

// How many times in a row the same call may be issued before the loop gives up
// on tools for this turn. 3 = the original call plus two repeats.
export const TOOL_REPEAT_LIMIT = 3;

// Appended to the system prompt on the one tool-free pass the breaker forces.
// The tools are already gone from the request at that point; this explains why,
// so the model writes its answer instead of narrating another call attempt.
export const TOOL_REPEAT_NUDGE =
  "You have already issued the same tool call several times in a row and received its result each time. " +
  "No tools are available for this reply. Answer the user now, using only what those results already gave you. " +
  "If they are not enough, say plainly what is missing — do not describe or attempt another tool call.";

// Per-turn tool-step cap — the unconditional termination bound.
//
// TOOL_REPEAT_LIMIT above only fires on IDENTICAL back-to-back calls, and the
// `tool-repeated-call-detection` middleware counts the same shape. A model that
// alternates between two different tools (doc_manifest → db_schema →
// doc_manifest → …) resets both counters on every call and slips past them, so
// with every provider loop written as `while (true)` the only remaining backstop
// is the turn's wall clock. This cap bounds the loop by construction: after N
// tool-calling passes in one turn the tools come off and the next pass must
// answer.
//
// 6. There is NO per-turn wall clock anywhere in the product —
// LLAMACPP_FETCH_TIMEOUT_MS bounds one request, nothing bounds the turn — so
// this cap is the only thing standing between a looping model and a spinner
// that never stops.
//
// The number is set on the developer's call, and the reasoning is the load-
// bearing part: a model that has not reached its goal in the first 3-4 passes
// is very rarely one pass away from reaching it, so a high ceiling buys a
// longer wait for the same failure rather than a better answer. At the ~42 s
// per pass recorded on the Ornith loop, 6 fires in about 4 minutes; 64 (the
// value copied from CODEX_TURN_MAX_TOOL_CALLS in the first draft of this) would
// have taken ~45 minutes, long after any real user pressed Stop.
//
// The cost is real and worth stating: the deepest legitimate chain observed —
// doc_manifest → doc_batch → db_schema → db_execute → db_query — is 5 passes,
// so a document flow runs one pass short of the cap. If those turns start
// getting cut off, raise APERIO_TURN_MAX_TOOL_STEPS; that is the knob for it.
export const DEFAULT_TURN_STEP_LIMIT = 6;

// Read live from the environment (never frozen at import) so the DB-settings
// overlay and tests can change it without a process restart — same validation as
// codex-turn-meter.js's resolveLimit: the WHOLE value must be a non-negative safe
// integer. Only an explicit, valid 0 disables the cap; anything malformed keeps
// the default. Parsing a prefix here (parseInt) would be a safety hole, not a
// convenience: "0.5" and "0oops" both parse to 0, silently switching off the only
// bound on a turn that calls different tools forever.
export function resolveTurnStepLimit(env = process.env) {
  const raw = env.APERIO_TURN_MAX_TOOL_STEPS;
  if (raw == null || String(raw).trim() === "") return DEFAULT_TURN_STEP_LIMIT;
  const n = Number(raw);
  if (!Number.isSafeInteger(n) || n < 0 || Object.is(n, -0)) return DEFAULT_TURN_STEP_LIMIT;
  return n;
}

// Appended to the system prompt on the tool-free pass the step cap forces.
// Same contract as TOOL_REPEAT_NUDGE: the tools are already gone from the
// request, this only explains why so the model answers instead of narrating
// another call attempt.
export const TURN_STEP_NUDGE =
  "You have used a large number of tool calls in this single reply and have reached the per-reply limit. " +
  "No tools are available for this reply. Answer the user now with what you already have. " +
  "If it is not enough, say plainly what is still missing and what you would do next — do not describe or attempt another tool call.";

// Runaway-loop breaker. `findPriorToolResult` stops a repeated call from being
// RE-EXECUTED, but nothing stopped the model from re-ISSUING it: every provider
// loop is `while (true)`, and before the step cap above the only backstop was
// the turn's wall clock. Observed on Ornith-1.0-9B-MTP (2026-08-18): `doc_manifest` issued
// 188 times in one turn — every repeat correctly served from the dedup cache and
// carrying the "do not call it again" reuse note — for nearly nine minutes,
// until the 600 s timeout killed the turn with no answer produced.
//
// Counts how many times the model issued its MOST RECENT tool call back-to-back
// with identical arguments, within the current turn only (a legitimate re-ask in
// a later turn starts from scratch). Tool-result messages in between are skipped;
// an assistant text answer, a different call, or a real user message resets the
// run. Returns 0 when the tail is not a tool call at all.
export function countTrailingRepeatedToolCalls(messages = []) {
  let start = 0;
  for (let i = messages.length - 1; i >= 0; i--) {
    if (isRealUserMsg(messages[i])) { start = i + 1; break; }
  }
  let sig = null, count = 0;
  for (let i = messages.length - 1; i >= start; i--) {
    const m = messages[i];
    if (isToolResultMessage(m)) continue;
    const s = toolCallSignature(m);
    if (s === null) break;
    if (sig === null) sig = s;
    else if (s !== sig) break;
    count++;
...
// and answers from it. Shared by every provider's tool-dispatch path (native
// and text-intercepted here, anthropic.js/gemini.js directly) so the wording
// can't drift into per-provider copies.
export function reuseNote(name, cached) {
...
export class ToolExecutor {
  // `toolNames`: the registered tool names for this turn, forwarded to
  // extractTextToolCall so its prose-parsing fallbacks can't manufacture a
  // call to a tool that doesn't exist (see extractTextToolCall's docs).
  // `answerOnly`: the repeat breaker fired for this turn (see
  // countTrailingRepeatedToolCalls). Every tool path is closed off — native
  // calls are discarded and text interception is skipped — so this pass can only
  // produce a final answer. That is what guarantees the loop terminates: without
  // it a model that ignores `noTools` and leaks a call in prose would spin again.
  constructor(callTool, emitter, messages, toolNames = [], answerOnly = false) { this.callTool = callTool; this.emitter = emitter; this.messages = messages; this.toolNames = toolNames; this.answerOnly = answerOnly; this.streamUsage = zeroUsage(); }
  ...
  async executeThinkingResponse(cleanText, toolCalls, streamHandler, isThinkingModel, reasoningContent = null) {
    if (this.answerOnly) toolCalls = [];
    const intercepted = !this.answerOnly && cleanText.trim() ? extractTextToolCall(cleanText, this.messages, this.toolNames) : null;
    if (intercepted) {
    ...
    if (toolCalls.length > 0) { streamHandler.flushBufferedContent(); this.emitter.send({ type: "reasoning_done" }); this.emitter.send({ type: "stream_end", text: "", usage: this.streamUsage }); await this.executeToolCalls(toolCalls, cleanText, reasoningContent); if (this.emitter._confirmPending) { delete this.emitter._confirmPending; return ""; } return null; }
    if (!cleanText.trim()) {
  ...
  async executeNonThinkingResponse(cleanText, toolCalls, streamHandler, reasoningContent = null) {
    if (this.answerOnly) toolCalls = [];
    if (toolCalls.length > 0) { streamHandler.flushBufferedContent(); this.emitter.send({ type: "stream_end", text: "", usage: this.streamUsage }); await this.executeToolCalls(toolCalls, cleanText, reasoningContent); if (this.emitter._confirmPending) { delete this.emitter._confirmPending; return ""; } return null; }
    if (!this.answerOnly && cleanText.trim()) { const intercepted = extractTextToolCall(cleanText, this.messages, this.toolNames); if (intercepted) { streamHandler.tokenBuffer = ""; const r = await this.executeInterceptedToolCall(intercepted, reasoningContent); if (this.emitter._confirmPending) { delete this.emitter._confirmPending; return ""; } return r; } }
