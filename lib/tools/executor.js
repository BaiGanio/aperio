...
export class ToolExecutor {
  ...
  parseArgs(argsStr) {
    ...
    try { return JSON.parse(argsStr); } catch (err) {
      logger.warn(`[executor] parseArgs failed — invalid JSON from model: ${err.message} | raw: ${argsStr.slice(0, 120)}`);
      // Return a sentinel so callTool can surface the error to the model rather than
      // calling the handler with undefined fields. The model will see the error and retry.
