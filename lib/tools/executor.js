...
export class ToolExecutor {
  ...
  constructor(callTool, emitter, messages, toolNames = []) { this.callTool = callTool; this.emitter = emitter; this.messages = messages; this.toolNames = toolNames; this.streamUsage = zeroUsage(); }
  ...
  async executeThinkingResponse(cleanText, toolCalls, streamHandler, isThinkingModel, reasoningContent = null) {
    const intercepted = cleanText.trim() ? extractTextToolCall(cleanText, this.messages, this.toolNames) : null;
    if (intercepted) {
  ...
  async executeNonThinkingResponse(cleanText, toolCalls, streamHandler, reasoningContent = null) {
    ...
    if (cleanText.trim()) { const intercepted = extractTextToolCall(cleanText, this.messages, this.toolNames); if (intercepted) { streamHandler.tokenBuffer = ""; const r = await this.executeInterceptedToolCall(intercepted, reasoningContent); if (this.emitter._confirmPending) { delete this.emitter._confirmPending; return ""; } return r; } }
