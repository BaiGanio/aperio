...
const DESTRUCTIVE_TOOLS = new Set([
  ...
  "wiki_write", "remember", "update_memory", "forget", "backfill_embeddings",
...
export class ToolExecutor {
  ...
  async executeToolCalls(toolCalls, cleanText = "", reasoningContent = null) {
    ...
    const results = [];
    for (const tc of toolCalls) results.push({ type: "tool_result", tool_use_id: tc.id, content: await this.callTool(tc.name, parsedByTc.get(tc.id)) });
