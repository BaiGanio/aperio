...
export function extractTextToolCall(text, messages) {
  ...
  for (let i = searchIn.indexOf("{"); i !== -1; i = searchIn.indexOf("{", i + 1)) {
    ...
    if (!/"(?:name|call|tool|function)"\s*:\s*"[^"]+"/.test(candidate)) continue;
    let parsed;
    try { parsed = JSON.parse(stripJsonComments(candidate)); } catch { continue; }
    const name = [parsed.name, parsed.call, parsed.tool, parsed.function].find(v => typeof v === "string" && v);
    if (!name) continue;
    const params = parsed.parameters ?? parsed.input ?? parsed.arguments ?? parsed.args ?? {};
