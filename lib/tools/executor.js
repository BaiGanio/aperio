  ...
  parseArgs(argsStr) {
    ...
    try { return JSON.parse(argsStr); } catch (err) {
      ...
      try {
        ...
        logger.warn(`[executor] parseArgs repaired malformed JSON from model | raw: ${argsStr.slice(0, 120)}`);
        return result;
      } catch { /* fall through to error */ }
      logger.error(`[executor] parseArgs failed — invalid JSON from model: ${err.message} | raw: ${argsStr.slice(0, 120)}`);
