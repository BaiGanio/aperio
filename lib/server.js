...
export async function createApp({
  ...
  function renderHtmlWithLocale(file, lang) {
    ...
    const inject = `<script>window.__APERIO_LANG__=${JSON.stringify(lang)};</script>`;
    return html.replace(
  ...
  app.use("/uploads", staticGuard, express.static(resolve(process.cwd(), "var/uploads")));
  app.use("/scratch", staticGuard, express.static(resolve(process.cwd(), "var/scratch")));
  ...
  if (autoListen) {
    httpServer.listen(PORT, HOST, async () => {
      ...
      if (isBootstrapped()) {
        ...
        if (!skipBrowser) openBrowser(`${scheme}://localhost:${actualPort}`);
        ...
        if (!skipBrowser) openBrowser(`${scheme}://localhost:${actualPort}/setup`);
      } else {
