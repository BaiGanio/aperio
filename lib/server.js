...
export async function createApp({
  ...
  const cspDirectives = {
    defaultSrc: ["'self'"],
    // jsDelivr survives in scriptSrc alone: the Mermaid bundle in index.html is
    // the only CDN asset left (#466). Bootstrap Icons — the stylesheet AND the
    // woff2 it pulls in — now come from /vendor/, so neither styleSrc nor
    // fontSrc has any reason to trust the CDN.
  ...
  async function bootApp() {
    ...
    // Round-table agents are created only on the first Discuss turn. Agents on
    // the same side of the local/cloud privacy boundary reuse the main MCP
    // connection instead of spawning duplicate tool-server processes.
    const roundtable = await bootRoundtable({
      root: __dirname,
      version,
      provider,
      createAgent,
      mcpConnections: [agent.getMcpConnection?.()],
    });
    ...
    // WebSocket
    const idleTimeoutMs = (Number(process.env.IDLE_TIMEOUT_SECONDS) || 180) * 1000;
    const { wss, broadcastToClients: sendToClients, liveClientCount } = createWsServer({
      httpServer, allowedHosts, makeWsHandler,
      agent, roundtable,
      store, isShuttingDown: () => isShuttingDown,
      // Reaping a dead socket costs two sweeps, so both must fit inside one
      // idle window — otherwise a socket orphaned by laptop sleep would keep
      // the watchdog vetoed forever. Clamped so a very short or very long
      // IDLE_TIMEOUT_SECONDS still yields a sane sweep rate.
      pingIntervalMs: Math.min(60_000, Math.max(5_000, Math.round(idleTimeoutMs / 3))),
    });
    broadcastToClients = sendToClients;

    // Watchdog. Built after the WS server on purpose: it needs `wss` for its
    // ordered teardown (previously omitted, which left that branch dead) and
    // liveClientCount() as the throttle-proof "a tab is still open" signal.
    ...
    const watchdog = createWatchdog({
      ...
      // #454: a turn still generating defers an idle-timeout kill — see
      // agent.getActiveTurnCount() (lib/agent/index.js).
      isBusy: () => (agent.getActiveTurnCount?.() ?? 0) > 0,
      // #454: so does an open browser socket. The HTTP heartbeat is a page
      // timer Chrome freezes in a background tab; the socket is not.
      hasLiveClients: () => liveClientCount() > 0,
    });
    ...
    const gracefulShutdown = createGracefulShutdown({
      ...
      codegraphBoot, docgraphBoot, watcherRegistry, shutdownEmbeddings, shutdownReindex,
