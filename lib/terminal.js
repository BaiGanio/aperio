...
process.on("SIGINT", () => {
  ...
  if (_sigintCount === 1) {
    ...
    if (_sessionId && _sessionMessages) {
      ...
      try { finaliseSession(_sessionId, _sessionMessages, null, false, { onShutdown: true }); } catch { /* non-fatal */ }
      _sessionId = null;
...
async function runProxy(port, initialReasoning) {
  ...
  const proxyModelVar = (process.env.AI_PROVIDER || "").toLowerCase() === "llamacpp" ? "LLAMACPP_MODEL"
    : (process.env.AI_PROVIDER || "").toLowerCase() === "deepseek" ? "DEEPSEEK_MODEL"
    : (process.env.AI_PROVIDER || "").toLowerCase() === "gemini" ? "GEMINI_MODEL"
    : (process.env.AI_PROVIDER || "").toLowerCase() === "codex" ? "CODEX_MODEL"
    : "ANTHROPIC_MODEL";
  initHeader(`proxy :${port}`, `${process.env.AI_PROVIDER} (${process.env[proxyModelVar]})`, showReasoning, lang);
