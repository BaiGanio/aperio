...
import { existsSync, writeFileSync, mkdirSync, readFileSync, statSync, realpathSync } from "fs";
...
import { getRecommendedModel, ollamaCtxStatus, recommendServeContextLength } from "./providers/index.js";
...
import {
  ...
  updateHeaderModel, updateHeaderReasoning, updateHeaderLang,
  // spinner
...
import { resolveStrings } from "./terminal/strings.js";
...
async function ensureOllama() {
  ...
  // Size the served context window to what the selected model can hold in RAM
  // (see startOllama.js / recommendServeContextLength) so the KV cache fits and
  // the app never over-keeps. Explicit OLLAMA_CONTEXT_LENGTH / OLLAMA_NUM_CTX win.
...
function printWelcome({ showExamples = true, lang = "en" } = {}) {
...
function printHelp({ proxy = false, showExamples = true, lang = "en" } = {}) {
  const S = resolveStrings(lang);
...
function printHelpFor(target, { proxy = false, lang = "en" } = {}) {
  const S = resolveStrings(lang);
...
// active lang.
function handleLangCommand(cmd, currentLang) {
...
function buildWorkspaceDirective(scratchDir) {
  return (
    `## Session workspace\n` +
...
function restartProcess({ rl, beforeSpawn } = {}) {
...
function printStatus({ reasoning, stats, examples, lang } = {}) {
...
async function printConfig({ port } = {}) {
  const SRC = { db: "from UI", env: "from .env", default: "default" };
  let get;                 // (key) => { value, label }
  let warnings = [];
  let precedence = process.env.APERIO_CONFIG_PRECEDENCE || "env";
  ...
  if (!get) {
    ...
    if ((process.env.AI_PROVIDER || "").toLowerCase() === "ollama" && ctx.mismatch) {
      warnings = [`OLLAMA_NUM_CTX (${ctx.assumed}) exceeds OLLAMA_CONTEXT_LENGTH (${ctx.real}); ` +
        `clamped to ${ctx.real}. Raise OLLAMA_CONTEXT_LENGTH or lower OLLAMA_NUM_CTX.`];
    }
  }
  ...
  out += row("AI_PROVIDER", "AI_PROVIDER", "anthropic");
  if (provider === "ollama") {
    out += row("OLLAMA_MODEL", "OLLAMA_MODEL", "llama3.1");
    out += row("OLLAMA_NUM_CTX", "OLLAMA_NUM_CTX", "32768");
    out += row("OLLAMA_CONTEXT_LENGTH", "OLLAMA_CONTEXT_LENGTH");
  }
...
async function runProxy(port, initialReasoning) {
  ...
  initHeader(`proxy :${port}`, `${process.env.AI_PROVIDER} (${process.env.OLLAMA_MODEL})`, showReasoning, lang);
...
async function runStandalone(initialReasoning) {
  ...
  const { runAgentLoop: baseRunAgentLoop, handleRememberIntent, fetchMemories, buildGreeting, OLLAMA_NO_TOOLS, provider, callTool } = agent;
    ...
    rl.once("line", async line => {
      ...
      if (isRestartCommand(cmd)) {
        if (isHardRestart(cmd)) {
          restartProcess({
            rl,
            beforeSpawn: () => { try { finaliseSession(sessionId, messages); } catch { /* */ } _sessionId = null; },
          });
          return;
        }
      ...
      const userContent = pendingAttachmentBlocks.length > 0
        ? [...pendingAttachmentBlocks, { type: "text", text: raw }]
        : raw;
      pendingAttachmentBlocks = [];
      ...
      if (OLLAMA_NO_TOOLS && isRememberIntent(raw)) {
  ...
  // Welcome banner + model-generated greeting. Used at boot and again after an
  // in-process `restart`. The greeting's emitter re-prompts when it finishes.
  async function greet() {
    printWelcome({ showExamples, lang });
    startSpinner("waking up");
    const { prompt: greetingPrompt, greetingMemBlock } = await buildGreeting(lang);
    ...
    // greetingMemBlock is the richer continuity set (capable models only); it is
    // injected into THIS turn's system prompt only and never persisted.
    const greetOpts = { noTools: true, lang };
