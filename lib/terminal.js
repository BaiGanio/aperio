...
/**
 * Checks if command asks for help (e.g. "help" or "?")
 */
export function isHelpCommand(cmd) {
  return /^(help|\?)$/i.test(cmd.trim());
}
...
function printHelp({ proxy = false } = {}) {
  ...
  section("Everyday", [
    ["attach <file>",  "add a PDF, image, or document to your next message"],
    ["summarize",      "boil this conversation down to a few bullet points"],
    ["remember that…", "tell me something to keep in mind for next time"],
  ]);
  if (proxy) {
    section("Deeper thinking", [
      ["discuss on",  "have two of me debate before answering (then: discuss off)"],
    ]);
  }
  section("Your stuff", [
    ["memories",    "see what I've remembered about you"],
    ["forget <id>", "make me forget one thing"],
    ["sessions",    "list your past conversations"],
    ["resume <id>", "pick a past conversation back up"],
    ["handoff",     "write a summary to start fresh without losing context"],
  ]);
    ...
    rl.once("line", async line => {
      ...
      if (isDiscussCommand(cmd)) {
        process.stdout.write(
          ...
          `${DIM}  Start it in another terminal with ${R}${BOLD}npm start${R}${DIM}, then run ${R}${BOLD}npm run chat${R}${DIM} again —${R}\n` +
