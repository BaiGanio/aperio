...
// Handle `lang` / `lang <code>`: bare shows the current choice; a known code
// switches and persists (so it sticks next session). Returns the active lang.
function handleLangCommand(cmd, currentLang) {
  ...
  if (!LANG_NAMES[target]) {
    process.stdout.write(`\n${RED}  ${S.lang_unknown}: ${target}${R}  ${GRAY}— try: lang de, lang fr, lang bg${R}\n\n`);
    return currentLang;
  }
