...
import { matchTemplates, classifyMatch, proposalKeywords } from "./matchHandlers.js";
 ...
 * literal distinctive words pulled from the document's own text — NOT the
...
export function inferTemplateProposal(text, { name } = {}) {
  ...
  return {
    ...
    match_keywords: proposalKeywords(text),
    fields,
    confidence: 0,
  };
}

// A keyword-confident match (same issuer) can still be the wrong FORM from
// that issuer — a credit note's refund_due where the template expects
// amount_due, say. match_keywords only ever encode issuer identity, so
// before trusting a confident match we also check that the candidate
// document's own labeled fields (extractFields, the same resolver a real
// extraction would use) actually fit the template's shape. Threshold picked
// empirically against the real household-gen corpus (2026-08-22): every
// genuine same-template document resolved 100% of fields, while the one
// clear same-issuer/different-form case found (an electricity bill vs its
// own credit note) resolved only 33%/75% depending on match direction — 0.8
// separates both cleanly. It does NOT generalize to same-shape-different-
// issuer collisions (e.g. two billers' payment-order receipts sharing the
// same amount/date fields) — those still auto-match; tracked in tech-debt.md.
export const FIELD_SHAPE_THRESHOLD = 0.8;
 ...
 *   { status: "proposed", token, proposal, ranked, shapeMismatch? } — pending user confirmation
 *
 * `shapeMismatch` is present on "no-evidence"/"proposed" when a keyword-
 * confident template was rejected for not fitting this document's own
 * fields — carried through so a caller can explain the mismatch rather than
 * present it as an unrelated cold start.
 */
export async function matchOrPropose(ctx, { text, name } = {}) {
  ...
  if (classified.status === "confident") {
    const { results } = extractFields(text, classified.top.template);
    const totalFields = classified.top.template.fields.length;
    if (totalFields === 0 || results.length / totalFields >= FIELD_SHAPE_THRESHOLD) {
      return { status: "matched", template: classified.top.template, ranked };
    }
    shapeMismatch = { template: classified.top.template, resolvedFields: results.length, totalFields };
  } else if (classified.status === "ambiguous") {
    return { status: "ambiguous", ranked };
  }

  const proposal = inferTemplateProposal(text, { name });
  if (!proposal.fields.length) return { status: "no-evidence", ranked, ...(shapeMismatch && { shapeMismatch }) };

  const { token } = await templateHandlers.proposeTemplate(ctx, proposal);
  return { status: "proposed", token, proposal, ranked, ...(shapeMismatch && { shapeMismatch }) };
}
