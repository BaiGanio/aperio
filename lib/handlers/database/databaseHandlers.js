...
import { classify, countPlaceholders, describeSqlitePlaceholders } from "../../db-connect/classify.js";
import {
...
async function validateExecutionArgs(ctx, args) {
  ...
  // neither can be schema-required. The cost is that checkArgs() derives its
  // `required` list from that schema and can therefore never report either one
  // missing, and db_execute is a DESTRUCTIVE tool, so hintFromIssues() is
  // suppressed for it anyway (lib/agent/index.js:538). This message is the only
  // correction that ever reaches the model — hence the explicit "what to pass".
