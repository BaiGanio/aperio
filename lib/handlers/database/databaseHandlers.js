...
import { EXTRACTION_CONNECTION, extractionDbPath, provisionExtractionConnection, isManagedExtractionFile, reservedExtractionNameError } from "../../db-connect/extraction.js";
...
function validateBoundParams(engine, sql, normalizedParams, backslashEscapesOverride) {
  if (engine !== "sqlite") {
    const expected = countPlaceholders(sql, engine, backslashEscapesOverride);
...
async function validateExecutionArgs(ctx, args) {
  ...
  if (isReservedExtractionName && meta && !(meta.provisioned && isManagedExtractionFile(meta, ctx.store))) {
    throw reservedExtractionNameError(meta, ctx.store);
  }
  ...
  // meta.backslashEscapes is an optional per-connection override (true/false)
  // for the engine's usual backslash-escaping default — null/undefined for
  // every existing connection, which keeps today's engine-guess behavior.
  validateBoundParams(engine, c.statements[0], normalizedParams, meta?.backslashEscapes ?? null);
