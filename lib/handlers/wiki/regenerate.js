...
// by choice, silently firing up Ollama is hostile. The refresh provider must be
// opt-in via env, just like ROUNDTABLE_AGENTS.

import Anthropic from "@anthropic-ai/sdk";
import { GoogleGenerativeAI } from "@google/generative-ai";
import logger from "../../helpers/logger.js";
import { ensureOllama } from "../../helpers/startOllama.js";
...
const SUPPORTED = new Set(["ollama", "llamacpp", "deepseek", "anthropic", "gemini"]);

// Parse "WIKI_REFRESH_PROVIDER=ollama:llama3.1" into { name, model } — or null if unset/malformed.
...
export async function regenerateArticle(ctx, slug) {
  ...
  // Step 2: if the provider is a local engine and auto-start is on, ensure it's running.
  if (cfg.name === "ollama" && process.env.WIKI_REFRESH_AUTOSTART_OLLAMA === "true") {
    try { await ensureOllama(); }
    catch (err) {
      log("error", `ensureOllama failed: ${err.message}`, { stack: err.stack });
      return { ok: false, reason: `ollama auto-start failed: ${err.message}` };
    }
  }
