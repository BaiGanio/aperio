...
import { ensureLlamaCpp, getLoadedModels } from "../../helpers/startLlamaCpp.js";
...
import { isVectorSearchable } from "../../helpers/vecMeta.js";
...
export function refreshRequestModel(model, mainProvider = resolveProvider({ name: "llamacpp" })) {
  return model === mainProvider?.model
    ? (mainProvider.requestModel || LLAMACPP_MAIN_ALIAS)
    : model;
}

export async function checkLlamaCppModelServed(model, requestModel = model) {
  const payload = await getLoadedModels();
  if (!payload) return null;
  const served = new Set();
  for (const entry of payload.data ?? []) {
    if (entry?.id) served.add(entry.id);
    for (const alias of entry?.aliases ?? []) served.add(alias);
  }
  if (served.has(requestModel)) return null;
  const loaded = [...served].join(", ") || "none";
  return `model not served: ${model}; loaded: ${loaded}. Restart Aperio to pick up the preset.`;
}

// One-shot text completion. Returns string. Throws on transport/API errors;
// callers log and surface a structured failure.
async function complete({ name, model }, { system, user, maxTokens = 2048, requestModel }) {
  if (name === "llamacpp") {
    ...
    requestModel ??= refreshRequestModel(model);
...
export function buildUserPrompt(article, memories) {
...
export function extractCitedMemoryIds(body) {
  const ids = new Set();
...
export async function regenerateArticle(ctx, slug) {
  ...
  if (cfg.name === "llamacpp") {
    const requestModel = refreshRequestModel(
      cfg.model,
      ctx?.provider?.name === "llamacpp" ? ctx.provider : undefined,
    );
    const availabilityError = await checkLlamaCppModelServed(cfg.model, requestModel);
    if (availabilityError) {
      log("error", availabilityError, { provider: cfg.name, model: cfg.model });
      return { ok: false, reason: availabilityError };
    }
  }
  ...
  let memories;
  try {
    // FTS-only while the memories store is stale/reindexing (issue #287).
    const queryEmbedding = await isVectorSearchable(ctx.store, "memories")
      ? await ctx.generateEmbedding(recallQuery)
      : null;
  ...
  try {
    body_md = await complete(cfg, {
      ...
      maxTokens: 2048,
      requestModel: cfg.name === "llamacpp"
        ? refreshRequestModel(cfg.model, ctx?.provider?.name === "llamacpp" ? ctx.provider : undefined)
        : undefined,
    });
