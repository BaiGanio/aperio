...
export async function searchArticles(store, generateEmbedding, { query, tags, status, limit = 10, mode = 'auto' }) {
  ...
  // FTS-only while the wiki store is stale/reindexing (issue #287).
  const vectorOk = await isVectorSearchable(store, 'wiki');
  if (store.wiki) {
    const queryEmbedding = (mode === 'fulltext' || !vectorOk) ? null : await generateEmbedding(query);
    return store.wiki.search({ query, queryEmbedding, tags, status, limit, mode });
  }
  return _pgSearchArticles(store.pool, generateEmbedding, { query, tags, status, limit, mode: vectorOk ? mode : 'fulltext' });
}
