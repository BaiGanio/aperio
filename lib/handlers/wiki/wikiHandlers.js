...
import { embedForStore } from '../../helpers/vecMeta.js';
// regenerate.js imports wikiWriteHandler from this file — use a lazy import to break the cycle.

// Exported so selfWikiHandlers.js can reuse them — both are table-agnostic
// (no wiki_articles/memories references), so there's nothing self-specific
// to duplicate.
export function hashSources(rows) {
  const payload = rows
...
export function modelTag() {
  return process.env.AI_PROVIDER === 'llamacpp'
    ? (process.env.LLAMACPP_MODEL  || 'llamacpp')
    : (process.env.ANTHROPIC_MODEL || 'claude');
}

// Canonical UUID shape. Used to pre-filter source ids before the Postgres
// ::uuid[] cast so a single malformed value cannot abort the whole query.
...
export async function wikiWriteHandler(ctx, { slug, title, summary, body_md, tags, source_memory_ids = [] }) {
  ...
  // Tolerant provenance: keep the source ids that resolve to a live memory and
  // drop the rest (unknown ids, expired memories, or malformed UUIDs) instead of
  // rejecting the whole write. A weak model that mis-transcribes one UUID out of
  // several would otherwise hit an all-or-nothing failure and retry the entire
  // wiki_write, which can exhaust a bounded turn budget (observed as a model-tier
  // chain-recall timeout). Real provenance is preserved — a malformed or unknown
  // id was never a valid link — and the intended article still gets written.
  let sourceRows = [];
  let validIds   = source_memory_ids;
  let droppedIds = [];
  if (source_memory_ids.length) {
    if (store.wiki) {
      // In-memory cache path: resolve source memories from the cache.
      ...
      // Postgres: pre-filter to UUID-shaped ids so a malformed value cannot abort
      // the ::uuid[] cast, then resolve the survivors via SQL.
      const wellFormed = source_memory_ids.filter(id => UUID_RE.test(id));
    ...
    const found = new Set(sourceRows.map(r => String(r.id)));
    validIds   = source_memory_ids.filter(id => found.has(String(id)));
    droppedIds = source_memory_ids.filter(id => !found.has(String(id)));
    if (droppedIds.length)
      logger.warn(`[wiki_write] ${slug}: dropped ${droppedIds.length} unrecognized source id(s): ${droppedIds.join(', ')}`);
  }
  const droppedNote = droppedIds.length
    ? ` ⚠️ ${droppedIds.length} unrecognized source id(s) omitted: ${droppedIds.join(', ')}`
    : '';
  const source_hash  = hashSources(sourceRows);
  // Gated: `wiki` is one of the five vec_meta-tracked stores, so while it is
  // stale/reindexing — or current toward another process's embedding
  // configuration — the article is persisted without a vector and the wiki
  // reindex adapter embeds it later (issue #340).
  const { embedding, deferred } = await embedForStore(store, 'wiki',
    () => generateEmbedding(`${title}. ${summary ?? ''} ${body_md}`));
  const generated_by = modelTag();

  if (store.wiki) {
    try {
      const { id, revision, inserted } = await store.wiki.upsert(
        { slug, title, summary, body_md, tags, generated_by, source_hash, source_memory_ids: validIds },
        embedding
      );
      if (deferred) logger.info(`[wiki_write] ${slug} saved without an embedding — the wiki store is being reindexed; its reindex driver will embed it.`);
      else if (!embedding) logger.warn(`[wiki_write] no embedding for ${slug}`);
      const verb = inserted ? 'Created' : `Updated (rev ${revision})`;
      return { content: [{ type: 'text', text: `✅ ${verb} wiki article "${title}" [${slug}] (id: ${id}, sources: ${validIds.length})${droppedNote}` }] };
    } catch (err) {
  ...
  try {
    ...
    if (deferred) logger.info(`[wiki_write] ${slug} saved without an embedding — the wiki store is being reindexed; its reindex driver will embed it.`);
    else if (!embedding) logger.warn(`[wiki_write] no embedding for ${slug} — semantic search will skip it until backfill`);
    const verb = inserted ? 'Created' : `Updated (rev ${revision})`;
    return { content: [{ type: 'text', text: `✅ ${verb} wiki article "${title}" [${slug}] (id: ${id}, sources: ${validIds.length})${droppedNote}` }] };
  } catch (err) {
...
// ── Propose wiki from memories ─────────────────────────────────────────────
export async function proposeWikiHandler(ctx, { slug, title, summary, body_md, tags, source_memory_ids = [] }) {
  const { store } = ctx;
  if (!slug || !/^[a-z0-9][a-z0-9-]*$/.test(slug))
    return { content: [{ type: 'text', text: '❌ Invalid slug — use lowercase letters, numbers and hyphens.' }] };
  if (!title?.trim() || !body_md?.trim())
    return { content: [{ type: 'text', text: '❌ title and body_md are required.' }] };

  const generated_by = process.env.AI_PROVIDER === 'llamacpp'
    ? (process.env.LLAMACPP_MODEL || 'llamacpp')
    : (process.env.ANTHROPIC_MODEL || 'claude');

  const draft = await store.proposeWikiDraft({
    slug, title: title.trim(), summary: summary?.trim() ?? null,
    body_md: body_md.trim(), tags: tags ?? [],
    generated_by, source_memory_ids,
  });

  return {
    content: [{
      type: 'text',
      text: `📝 Wiki draft proposed: "${draft.slug}" (rev ${draft.revision})\nThe user will review it before publishing.`
    }]
  };
}
