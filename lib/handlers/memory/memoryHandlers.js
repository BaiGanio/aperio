...
export async function rememberHandler(ctx, { type, title, content, tags, importance, tier, expires_at, lang, confidence }) {
...
export async function recallHandler(ctx, { query, type, tags, limit: _limit, maxTier, search_mode = "auto", lang, as_of, order = "importance" }) {
  ...
  const rawRows = await store.recall({ query, queryEmbedding, type, tags, limit, mode: search_mode, lang: localeToPgConfig(lang), asOf: as_of, order, maxTier: resolvedMaxTier });
  ...
  // Tier 2 (sensitive) is withheld for now; the redact-via-config path
  // (APERIO_MEMORY_TIER2_CLOUD) will be added in a follow-up phase.
  // Local (Ollama) sessions see everything within maxTier.
  const rows = ctx.providerIsLocal === false
    ? rawRows.filter(m => (m.tier ?? 1) <= 1)
    : rawRows;
