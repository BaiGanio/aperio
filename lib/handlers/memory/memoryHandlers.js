...
export async function recallHandler(ctx, { query, type, tags, limit: _limit, search_mode = "auto", lang, as_of }) {
  ...
  const rows = await store.recall({ query, queryEmbedding, type, tags, limit, mode: search_mode, lang: localeToPgConfig(lang), asOf: as_of });

  if (!rows.length)
    return { content: [{ type: "text", text: "No memories found." }] };

  const formatted = rows.map(m => {
    const simNote  = m.similarity  !== undefined ? ` [similarity: ${(m.similarity * 100).toFixed(0)}%]` : "";
