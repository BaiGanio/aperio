...
export async function rememberHandler(ctx, { type, title, content, tags, importance, expires_at, lang }) {
  ...
  const mem = await store.insert(
    { type, title, content, tags: tags ?? [], importance: importance ?? 3,
      ...
      lang: localeToPgConfig(lang) },
    embedding
  );

  if (!embedding) logger.warn(`⚠️  Embedding unavailable for memory id=${mem.id} — will retry on next backfill`);
...
export async function recallHandler(ctx, { query, type, tags, limit: _limit, search_mode = "auto", lang }) {
  ...
  const rows = await store.recall({ query, queryEmbedding, type, tags, limit, mode: search_mode, lang: localeToPgConfig(lang) });
