import { modelTag } from "../wiki/wikiHandlers.js";
...
// Exported so selfWikiHandlers.js can reuse the same refusal (identical gate,
// same local-only guarantee — see agent-self-memory.md Phase 2).
...
export async function selfRememberHandler(ctx, { title, content, tags, importance, lang, confidence }) {
  ...
  // Gated: while self_memories is stale/reindexing — or current toward another
  // process's embedding configuration — write the row bare and let that
  // store's reindex driver embed it (issue #340).
  const { embedding, deferred } = await embedForStore(store, "self_memories",
  ...
  if (deferred) logger.info(`[self_remember] id=${mem.id} saved without an embedding — self-memory is being reindexed; its reindex driver will embed this row.`);
  else if (!embedding) logger.warn(`⚠️  Embedding unavailable for self-memory id=${mem.id} — full-text search still works`);
  const embeddingNote = embedding
    ? " (with semantic embedding)"
    : deferred
      ? " (no embedding yet — self-memory embeddings are being reindexed)"
      : " (no embedding — semantic search unavailable)";
  return {
...
export async function selfRecallHandler(ctx, { query, tags, limit: _limit, search_mode = "auto", lang }) {
  ...
  // FTS-only while this store is stale/reindexing (issue #287). The mode is
  // downgraded too, not just the embedding: an explicit "semantic" mode with no
  // embedding disables full-text search as well, leaving an unfiltered listing.
  const vectorOk = vectorEnabled() && await isVectorSearchable(store, "self_memories");
  ...
  const rows = await store.recallSelf({ query, queryEmbedding, tags, limit, mode: effectiveMode, lang: localeToPgConfig(lang) });
...
export async function selfUpdateHandler(ctx, { id, title, content, tags, importance }) {
  ...
  if ((title || content) && vectorEnabled()) {
    ({ embedding, deferred } = await embedForStore(store, "self_memories",
      () => generateEmbedding(`${title ?? current.title}. ${content ?? current.content}`)));
  }

  try {
    const updated = await store.updateSelf(id, input, embedding);
    // Mirrors updateMemoryHandler's fallback (issue F-R2-06): a failed embed
    // (embedding falsy, not deferred to the reindex driver) must not just
    // leave the row's stale vector in place forever — retry it. `deferred`
    // means the store is gated/reindexing (issue #340) and already owned by
    // that driver, so queueing here would only re-race the same gate.
    if ((title || content) && !embedding && !deferred) {
      ctx.selfEmbeddingQueue?.enqueue(updated.id, `${updated.title}. ${updated.content}`);
    }
