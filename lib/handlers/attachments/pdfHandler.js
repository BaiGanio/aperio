 ...
 */
export async function handlePdf(att, name, uploadDir, { urlBase = "/scratch", _extractPdfText = extractPdfText, _fs = fs } = {}) {
  ...
  try {
    ...
    logger.info(`📑 PDF extracted: type=${result.type} pages=${result.pageCount} scanned=${result.scannedPages.length} chars=${result.text.length}`);
    ...
    if (result.type === "scanned" || result.type === "mixed") {
      ...
      logger.info(`📑 PDF saved for vision: ${name} → ${safeFilename} (${Math.round(rawBuffer.length / 1024)}KB)`);
      meta.savedPath = savedPath;
      meta.url = `${urlBase}/${safeFilename}`;
    }

    return { ...buildPdfResult(result, name, savedPath), meta };

  } catch (err) {
    logger.error(`❌ PDF extraction failed for ${name}: ${err.message}`);
    return {
      blocks: [],
