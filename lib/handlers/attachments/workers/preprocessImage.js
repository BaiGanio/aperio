...
export async function preprocessImage(input, options = {}) {
  ...
  const image = sharpFn(input, { animated: false }); // animated:false → first frame only
  const meta  = await image.metadata();

  // Log what came in — useful when debugging local model failures
  logger.debug(
    `preprocessImage: ${meta.format} ${meta.width}x${meta.height} ` +
    `channels=${meta.channels} alpha=${meta.hasAlpha} -> RGB ${size}x${size}`
  );
