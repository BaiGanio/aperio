import path from "path";
import logger from "../../helpers/logger.js";
import { join } from "path";
import { getArtifactWorkspace } from "../../helpers/artifactWorkspace.js";

import { handleImage } from "./imageHandler.js";
import { handleText  } from "./textHandler.js";
import { handlePdf   } from "./pdfHandler.js";
import { handleDocx  } from "./docxHandler.js";
import { handlePptx  } from "./pptxHandler.js";

// ─── Extension sets ───────────────────────────────────────────────────────────

const IMAGE_EXTS = new Set([".jpg", ".jpeg", ".png", ".gif", ".webp"]);

const TEXT_EXTS = new Set([
  ".txt", ".md", ".markdown",
  ".js", ".ts", ".jsx", ".tsx", ".mjs", ".cjs",
  ".py", ".go", ".rs", ".java", ".c", ".cpp", ".h",
  ".json", ".yaml", ".yml", ".toml",
  ".html", ".css", ".scss", ".sql", ".sh", ".bash",
  ".csv", ".xml", ".graphql", ".prisma",
]);

// Secret/dotfile deny-list — refuse env files and known credential files
// before extension routing (INPUT-01).
const DENIED_BASENAMES = new Set([
  ".npmrc", ".netrc", ".pgpass", ".htpasswd", ".dockercfg",
  ".git-credentials", "id_rsa", "id_dsa", "id_ecdsa", "id_ed25519",
]);
const DENIED_EXTS = new Set([".pem", ".key", ".pfx", ".p12", ".keystore"]);
function isSecretName(name) {
  const base = name.toLowerCase();
  if (base.startsWith(".env")) return true;
  if (DENIED_BASENAMES.has(base)) return true;
  return DENIED_EXTS.has(path.extname(base));
}

const SUPPORTED_LABEL =
  "images (jpg/png/gif/webp), text files (txt/md/js/ts/py/json/…), PDFs, Word docs (.docx), PowerPoint (.pptx)";

// ─── Sanitize a user-supplied filename ────────────────────────────────────────

/**
 * Strips path traversal, control characters, unicode bidi overrides,
 * and truncates to 200 chars. Applied once at the entry point so all
 * downstream handlers receive a safe name.
 */
function sanitizeFilename(raw) {
  let safe = path.basename(raw || "unnamed");
  safe = safe.replace(/[\x00-\x1f\x7f-\x9f\u200e\u200f\u202a-\u202e]/g, "");
  if (safe.length > 200) safe = safe.slice(0, 197) + "...";
  return safe || "unnamed";
}

// ─── Token estimate ───────────────────────────────────────────────────────────

/**
 * Rough token estimate for a handler's text blocks — char/4, matching the
 * heuristic used everywhere else in the app. This is what a file actually
 * costs the model (its extracted text, fences and all).
 */
function estimateBlockTokens(blocks) {
  const text = (blocks ?? [])
    .filter(b => b.type === "text")
    .map(b => b.text || "")
    .join("");
  return Math.max(0, Math.ceil(text.trim().length / 4));
}

// ─── Public API ───────────────────────────────────────────────────────────────

/**
 * Routes each attachment to the correct handler and merges results.
 * Returns content blocks + hint for the agent message, and attachment metadata
 * for session persistence (thumbnails for images, name/type for everything else).
 *
 * @param {object[]} attachments  - Raw attachments array from the WS message
 * @param {string}   __dirname    - Server root (used to resolve the uploads dir for scanned PDFs)
 * @returns {{ contentBlocks: object[], hint: string, attachmentMeta: object[] }}
 */
export async function processAttachments(attachments, __dirname, {
  _handleImage = handleImage,
  _handleText  = handleText,
  _handlePdf   = handlePdf,
  _handleDocx  = handleDocx,
  _handlePptx  = handlePptx,
  storageDir   = null,
  urlBase      = null,
} = {}) {
  if (!attachments?.length) return { contentBlocks: [], hint: "", attachmentMeta: [] };

  const workspace = storageDir && urlBase ? null : await getArtifactWorkspace();
  const attachmentDir = storageDir ?? join(workspace.dir, "attachments");
  const attachmentUrlBase = urlBase ?? `${workspace.urlBase}/attachments`;

  const contentBlocks = [];
  let hint = "";
  const attachmentMeta = [];

  for (const att of attachments) {
    const name = sanitizeFilename(att.name);
    const ext  = path.extname(name).toLowerCase();

    let result;

    if (isSecretName(name)) {
      logger.warn(`⚠️  Refused secret/credential attachment: ${name}`);
      result = {
        blocks: [],
        hint: `\n[System: Attachment refused — secret/credential files are not accepted: ${name}.]`,
        meta: { name, type: att.type || "application/octet-stream" },
      };
    }
    else if (IMAGE_EXTS.has(ext)) result = await _handleImage(att, name, {
      uploadDir: attachmentDir,
      urlBase: attachmentUrlBase,
    });
    else if (TEXT_EXTS.has(ext)) result = await _handleText(att, name, ext);
    else if (ext === ".pdf")     result = await _handlePdf(att, name, attachmentDir, { urlBase: attachmentUrlBase });
    else if (ext === ".docx")    result = await _handleDocx(att, name);
    else if (ext === ".pptx")    result = await _handlePptx(att, name);
    else {
      logger.warn(`⚠️  Unsupported attachment: ${name}`);
      result = {
        blocks: [],
        hint: `\n[System: Attachment received but not supported: ${name} (${ext}). Supported: ${SUPPORTED_LABEL}.]`,
        meta: { name, type: att.type || "application/octet-stream" },
      };
    }

    contentBlocks.push(...result.blocks);
    hint += result.hint;
    if (result.meta) {
      // Persist a token estimate for non-image files so restored sessions can
      // show their real cost (the client has no file data to recompute from).
      // Images are excluded — their cost is a provider-specific constant the
      // client derives from the active model.
      if (!IMAGE_EXTS.has(ext)) result.meta.tokens = estimateBlockTokens(result.blocks);
      attachmentMeta.push(result.meta);
    }
  }

  return { contentBlocks, hint, attachmentMeta };
}
