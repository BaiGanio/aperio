  ...
  try {
    ...
    if (slideEntries.length === 0) {
      return {
        ...
        meta: { name, type: att.type || "application/vnd.openxmlformats-officedocument.presentationml.presentation" },
      };
    }
    ...
    logger.info(`📊 PPTX extracted: ${name} (${sizeKb}KB, ${slideEntries.length} slides)`);
    ...
    logger.error(`❌ PPTX extraction failed for ${name}: ${err.message}`);
    return {
      blocks: [],
