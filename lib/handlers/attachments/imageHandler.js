...
export async function handleImage(att, name, {
  ...
  try {
    ...
    logger.info(`🔧 Preprocessed image: ${safeName} → ${relativePath}`);

    return {
      blocks: [
        { type: "text", text: `[Image: ${safeName}]` },
        { type: "image", source: { type: "base64", media_type: "image/png", data: normalisedBase64 } },
      ],
      hint: `\n[System: Image "${safeName}" is attached inline above. Full-size copy saved to ${fullPath}.]`,
      meta: { name: safeName, type: "image/png", thumbnail, savedPath: fullPath, url: `/uploads/${filename}` },
    };
  } catch (err) {
    logger.error(`❌ preprocessImage failed for ${safeName}: ${err.message}`);
    return {
      blocks: [],
