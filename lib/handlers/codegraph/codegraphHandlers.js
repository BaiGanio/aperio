...
import { neighbors as graphNeighbors, shortestPath } from '../../codegraph/graph.js';
import { loadCachedGraph } from '../../codegraph/graphCache.js';
...
async function _search(ctx, args) {
  ...
  const result = await backend.mod.search(ctx.store, args, {
    ...
    vectorEnabled:     await vectorGate(ctx.store, "codegraph", ctx.vectorEnabled),
  });
...
async function _neighbors(ctx, { qualified, repo, direction = 'both', kinds, depth = 1, limit = 50 }) {
  ...
  const { graph } = await loadCachedGraph(backend.mod, ctx.store, repoId);
...
async function _insights(ctx, { repo, view = 'summary', limit = 20 }) {
  ...
  switch (view) {
    ...
    case 'cycles':      return asText({ ...base, cycles: importCycles(await loadCachedGraph(mod, ctx.store, repoId), { limit: cap }) });
    default:            return asText({ ...base, summary: await mod.analysisSummary(ctx.store, repoId) });
  }
}

async function _graph(ctx, { repo, limit = 300, focus, community }) {
  const backend = backendOf(ctx);
  if (!backend) return NOT_AVAILABLE;
  if (!repo) { const e = new Error('repo is required'); e.userFacing = true; throw e; }
  const { mod } = backend;
  ...
  const payload = await buildGraphPayload(mod, ctx.store, repoId, { limit, focus: focusList, community });
