...
// SQLite backend; LanceDB returns a clean "not available" message.
...
async function _context(ctx, { qualified, padding = 2 }) {
  const backend = backendOf(ctx);
  if (!backend) return NOT_AVAILABLE;
  const sym = await backend.mod.context(ctx.store, { qualified });
  if (!sym) {
