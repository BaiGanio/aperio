 ...
 *        pushes, e.g. a WebSocket broadcast. Called after a background (interval/
 *        watcher) run with { jobId, verdict, mode, durationMs, trigger, error }.
 *        Not called for manual run-now (the caller already gets the result). Optional.
...
export function createAgentScheduler({ callTool, createAgent, root, version, jobs = [], watcherEvents, recordRun, notify } = {}) {
  ...
  // Push a "job finished" notification to the UI (Phase 5). Best-effort and only
  // for background triggers — manual run-now already returns the result to the
  // caller, so a banner there would be redundant.
  function safeNotify(payload) {
    if (typeof notify !== "function") return;
    if (payload.trigger === "manual") return;
    try {
      notify(payload);
    } catch (err) {
  ...
  async function runJob(job, triggerCtx = {}) {
    ...
    try {
      ...
      safeNotify({ jobId: job.id, verdict: "ok", mode, durationMs: Date.now() - startedAt, trigger: triggerLabel });
