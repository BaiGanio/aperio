...
import { buildBackgroundJobSpec } from "../agent/job-spec.js";
...
export function createAgentScheduler({ callTool, createAgent, root, version, jobs = [], watcherEvents, recordRun, notify } = {}) {
  ...
  async function runFreeform(job, entries, triggerCtx = {}) {
    const spec = job.spec ?? buildBackgroundJobSpec(job);
    ...
    const { emitter, toolsUsed, offloadStats } = makeSinkEmitter();
    ...
    return {
      ...
      answer,
      model,
      artifactCount: offloadStats.count,
      artifactBytes: offloadStats.bytes,
    };
  }
  ...
  async function safeRecordRun({
    job, mode, startedAt, verdict, trigger, tools, error, answer, model,
  ...
  async function runJob(job, triggerCtx = {}) {
    ...
    try {
      ...
      await safeRecordRun({
        job, mode, startedAt, verdict: "ok", trigger: triggerLabel,
        tools: out.tools, answer: out.answer, model: out.model,
        artifactCount: out.artifactCount, artifactBytes: out.artifactBytes,
      });
