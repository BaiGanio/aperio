 ...
 */
export function createAgentScheduler({ callTool, createAgent, root, version, jobs = [], watcherEvents, recordRun } = {}) {
  ...
  // Persist a one-line run summary to the DB (Phase 4). Best-effort, like the
  // on-disk run record: a recording failure must not fail the job itself.
  async function safeRecordRun({ job, mode, startedAt, verdict, trigger, tools, error, answer }) {
  ...
  async function runJob(job, triggerCtx = {}) {
    ...
    try {
      ...
      const record = writeAgentRunRecord({
        job, entries, tools: out.tools, startedAt, verdict: "ok", trigger: triggerLabel,
      });
      await safeRecordRun({ job, mode, startedAt, verdict: "ok", trigger: triggerLabel, tools: out.tools, answer: out.answer });
      ...
      writeAgentRunRecord({ job, entries, startedAt, verdict: "error", error: err.message, trigger: triggerLabel });
  ...
  function wireWatcherJobs(events) {
    const watcherJobs = jobs.filter(j => j.enabled && j.trigger?.kind === "watcher");
  ...
  const watcherCount  = jobs.filter(j => j.enabled && j.trigger?.kind === "watcher").length;
