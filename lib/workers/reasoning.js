...
export const REASONING_ADAPTERS = [
  ...
  // ── Qwen3 ────────────────────────────────────────────────────────────────────
  // Two delivery modes depending on the Ollama build:
  //   • Newer builds split thinking into a dedicated `delta.reasoning` field.
  //   • Others leave it inline in `delta.content` as <think>…</think>.
  // We handle both: the reasoning field is emitted directly, and content is run
  // through a tag splitter so an inline <think> block is surfaced as hidden
  // reasoning_* events instead of leaking into the answer.
  {
    match: "qwen3",
    thinks: true,
    noTools: false,
  ...
  // Ollama implementation uses the same dedicated field as Qwen3.
  {
    ...
    thinks: true,
    noTools: false,
    // Gemma writes a *plan* to call a tool inside its thinking channel and then
    // stops (finish_reason "stop") without ever emitting the tool_call when the
    // input is large — leaving an empty answer. Suppressing reasoning on
    // tool-eligible turns makes it commit to the call directly. The Ollama
    // OpenAI-compat endpoint only honors reasoning_effort:"none" for this
    // (it ignores `think:false`). See runOllamaLoop.
