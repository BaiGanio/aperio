...
// thinking in one of several ways depending on the serving backend:
//   • A dedicated native field — `delta.reasoning` (Ollama) or `delta.reasoning_content`
//     (llama.cpp's llama-server) — split the two backends emit for the same thing.
...
function makeInlineThinkAdapter(match) {
  return {
    ...
    processDelta(delta, state, emit) {
      // Native reasoning field — Ollama uses delta.reasoning, llama.cpp uses delta.reasoning_content.
      ...
      if (thinking) {
        ...
        emit({ type: "reasoning_token", text: thinking });
      }
...
export const REASONING_ADAPTERS = [
  ...
  // ── DeepSeek V4 Flash ────────────────────────────────────────────────────
  // Flash models always return reasoning_content in responses and the API
  // requires it echoed back. Use thinks=true with thinking_mode so responses
  // are formatted correctly. Content-only adapter — no inline thinking parsing.
  {
  ...
  // ── DeepSeek V4 Pro (thinking mode, with tools) ────────────────────────────
  // Pro models support the thinking_mode API parameter and expect reasoning_content
  // to be echoed back in history (the API enforces this).
  {
  ...
  // ── DeepSeek-R1 ──────────────────────────────────────────────────────────────
  // Same native reasoning field as Qwen3 (delta.reasoning / delta.reasoning_content
  // depending on backend); also cannot use structured tool calls.
  {
    match: "deepseek-r1",
    thinks: true,
    noTools: true,
  ...
  // Ollama implementation uses the same dedicated field as Qwen3.
  {
    ...
    processDelta(delta, state, emit) {
      // Thinking field — Ollama uses delta.reasoning, llama.cpp uses delta.reasoning_content
      const thinking = delta.reasoning || delta.reasoning_content;
      if (thinking) {
        if (!state.sentReasoningStart) {
          state.sentReasoningStart = true;
