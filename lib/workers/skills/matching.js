...
// Product/actor boilerplate appears throughout bundled skill descriptions and
// in ordinary prompts about Aperio itself. It identifies the host, not the
// user's intent, so it must not complete an otherwise weak match. Keep these
// words available to curated keyword phrases, where authors chose them as part
// of a specific signal such as "fresh agent". User and agent-authored skills
// retain their vocabulary because those descriptions may define that domain.
const SKILL_DESCRIPTION_STOPWORDS = new Set([
  "aperio", "agent", "agents", "every",
]);
...
// stray short words.
export function skillTokens(text, minLen, extraStopwords = null) {
  return new Set(
    (text ?? "").toLowerCase().split(/\W+/).filter(w =>
      w.length >= minLen && !SKILL_STOPWORDS.has(w) && !extraStopwords?.has(w)
    )
  );
}
...
// phrases. Legacy fields without commas remain lists of independent one-word
// entries.
...
export function scoreSkill(skill, msgWords, msgTokens) {
  ...
  const descriptionStopwords = skill.source === "bundled" ? SKILL_DESCRIPTION_STOPWORDS : null;
  const descHits = [...skillTokens(skill.description, 4, descriptionStopwords)]
    .filter(t => msgWords.has(t));
