 ...
 */
export function writeRoundtableRecord({ sessionId, userText, turns, agents, verdict, agreedText, discussionContext }) {
  ...
  const discussion = [
    ...
    ``,
    ...(discussionContext
      ? [`### Shared context (summary of prior conversation)`, ``, String(discussionContext).trim(), ``]
      : []),
    ...sections,
...
/**
 * Write a combined manifesto file from both agents' final personal statements.
 * Best-effort: failure is logged but never breaks the round-table.
 *
 * @param {object} opts
 * @param {string} [opts.sessionId]
 * @param {string} opts.userText
 * @param {string} opts.primaryManifesto
 * @param {string} opts.verifierManifesto
 * @param {{primary:object,verifier:object}} opts.agents
 * @param {"agreed"|"no_agreement"|"interrupted"} opts.verdict
 * @returns {string|null} the written path, or null on failure.
 */
export function writeRoundtableManifesto({ sessionId, userText, primaryManifesto, verifierManifesto, agents, verdict }) {
  // Disk safety under tests comes from the fs stubs in the test suite, not an
  // early return: callers consume the returned `path`, so it must be computed.
  const filePath = sessionId
    ? join(ROUNDTABLES_DIR, `aperio-manifesto-${sessionId}.md`)
    : (() => {
        const iso  = new Date().toISOString().replace(/[:.]/g, "-");
        const slug = String(userText).toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 40) || "discussion";
        return join(ROUNDTABLES_DIR, `aperio-manifesto-${iso}-${slug}.md`);
      })();
  ...
  const content = [
    ...
    `---`,
    ``,
    `## ${agentLabel(agents.primary, "primary")}`,
    ``,
    String(primaryManifesto).trim(),
    ``,
    `---`,
    ``,
    `## ${agentLabel(agents.verifier, "verifier")}`,
    ``,
    String(verifierManifesto).trim(),
    ``,
  ].join("\n");
  ...
  try {
    ...
    logger.error(`[roundtable] failed to write manifesto: ${err.message}`);
    return null;
  }
}
 ...
 *
 *  - The OpenAI-compatible error envelope returned verbatim by the llamacpp loop
...
export function buildManifestoPrompt(userText, character) {
  const charContext = character ? ` as a ${character}` : "";
  return `PHASE: MANIFESTO\nThe round-table discussion has concluded${charContext}. The original question was:\n\n> ${quoteBlock(userText)}\n\nWrite your personal manifesto — a final statement of your convictions on this matter. This is not a summary of the discussion or the consensus. It is your individual voice, your principles, your final word.\n\nKeep it to 5–6 sentences maximum. Plain prose — no headings, no bullet lists, and no preamble about tensions or philosophy. State your conviction directly and stop.`;
}
...
export async function runRoundTable({
  ...
  async function turn(agent, msgs, emitter, phase, content) {
    ...
    const reply = await agent.runAgentLoop(
      msgs,
      emitter,
      {
        lang,
        aperioSessionId: sessionId,
        providerSessionKey: `codex:roundtable-${agentId}`,
      },
      getAbort,
      setAbort,
    );
  ...
  // Optional staged summary (from the Discuss confirm flow) — prepended to the
  // first ANSWER and REVIEW prompts only, so both agents open with the same
  // shared framing. Later rounds reuse each agent's own buffer.
  const contextBlock = discussionContext && String(discussionContext).trim()
    ? `Shared context (summary of the prior conversation):\n${String(discussionContext).trim()}\n\n---\n\n`
    : "";
  ...
  try {
    ...
    const answerContent = withUserAttachments(contextBlock + buildAnswerPrompt(userText), userContent);
    ...
    const reviewContent = withUserAttachments(contextBlock + buildReviewPrompt(userText, a1), userContent);
  ...
  if (agreed) {
    ...
    const manifestoResult = await _generateManifestos({ primary, verifier, aMessages, bMessages, aEmitter, bEmitter, baseEmitter, userText, sessionId, agents: { primary, verifier }, verdict: "agreed", getAbort });
    return { agreed: true, text: agreedText, rounds: agentTurns + (manifestoResult?.turns ?? 0), ...(manifestoResult ? { manifesto: manifestoResult } : {}) };
  }
  ...
  const manifestoResult = await _generateManifestos({ primary, verifier, aMessages, bMessages, aEmitter, bEmitter, baseEmitter, userText, sessionId, agents: { primary, verifier }, verdict: "no_agreement", getAbort });
  return { agreed: false, positions, rounds: agentTurns + (manifestoResult?.turns ?? 0), ...(manifestoResult ? { manifesto: manifestoResult } : {}) };
}

/**
 * Generate manifestos from both agents — best-effort, never breaks the round-table.
 * Called after the deliberation concludes (agreed / no_agreement).
 * Returns { primaryManifesto, verifierManifesto, path } or null on failure.
 *
 * @param {object} opts
 * @returns {Promise<{primaryManifesto:string, verifierManifesto:string, path:string}|null>}
 */
async function _generateManifestos({ primary, verifier, aMessages, bMessages, aEmitter, bEmitter, baseEmitter, userText, sessionId, agents, verdict, getAbort }) {
  try {
    let turns = 0;
    const _turn = async (agent, msgs, emitter, agentId) => {
      baseEmitter.send({ type: "roundtable_phase", phase: "manifesto", agent_id: agentId });
      const content = buildManifestoPrompt(userText, agent.character);
      msgs.push({ role: "user", content });
      const reply = await agent.runAgentLoop(
        msgs,
        emitter,
      ...
      turns += 1;
      return foldReplyToPlainText(reply);
    };
    const primaryManifesto  = await _turn(primary,  aMessages, aEmitter, "primary");
    const verifierManifesto = await _turn(verifier, bMessages, bEmitter, "verifier");
    const path = writeRoundtableManifesto({ sessionId, userText, primaryManifesto, verifierManifesto, agents, verdict });
    return { primaryManifesto, verifierManifesto, path, turns };
  } catch (err) {
    logger.error(`[roundtable] manifesto generation failed: ${err.message}`);
    return null;
  }
}
