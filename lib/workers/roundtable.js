 ...
 * document and write it under <project>/var/roundtables/ — mirrors the handoff
 * writer in wsHandler.js (same var/<thing> convention, same iso+slug naming).
 *
 * The intermediate A↔B turns are otherwise lost: the orchestrator only pushes
 * the final consensus (or no-consensus summary) onto the shared transcript.
 * This captures the full discussion for the project's back story.
 *
 ...
 */
export function writeRoundtableRecord({ userText, turns, agents, verdict, agreedText }) {
  const iso  = new Date().toISOString().replace(/[:.]/g, "-");
  const slug = String(userText).toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 40) || "discussion";
  const path = join(ROUNDTABLES_DIR, `aperio-roundtable-${iso}-${slug}.md`);

  const idLabel = (agent, fallbackId) => {
    const character = agent?.character ? ` — ${agent.character}` : "";
    const who = agent ? `${agent.provider.name} (${agent.provider.model})` : fallbackId;
    return `${fallbackId === "primary" ? "Agent A" : "Agent B"}${character} · ${who}`;
  };
  ...
  const sections = turns.flatMap((t, i) => {
    ...
    return [`## ${i + 1}. ${t.agentId === "primary" ? "Agent A" : "Agent B"}${charTag} — ${phaseLabel}`, ``, String(t.text).trim(), ``];
  });
  ...
  const header = [
    ...
    `- **Agent A:** ${idLabel(agents.primary, "primary")}`,
    `- **Agent B:** ${idLabel(agents.verifier, "verifier")}`,
    ``,
    `## Question`,
    ``,
  ...
  const footer = verdict === "agreed" && agreedText
    ? [``, `## Final consensus`, ``, String(agreedText).trim()]
    : [];

  const doc = [...header, ``, ...sections, ...footer].join("\n") + "\n";
