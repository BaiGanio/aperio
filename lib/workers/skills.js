...
function containsTokenSequence(messageTokens, phraseTokens) {
 ...
 */
function scoreSkill(skill, msgWords, msgTokens) {
...
function hasPositiveSkillName(msg, name) {
  const tokenMatches = [...msg.matchAll(/[a-z0-9]+/g)];
  const messageTokens = tokenMatches.map(match => match[0]);
  const nameTokens = name.toLowerCase().replace(/-/g, " ").split(/\s+/).filter(Boolean);
  ...
  for (let i = 0; i < messageTokens.length; i++) {
    ...
    const clauseStart = Math.max(
      msg.lastIndexOf(".", tokenMatches[i].index),
      msg.lastIndexOf(",", tokenMatches[i].index),
      msg.lastIndexOf(";", tokenMatches[i].index),
      msg.lastIndexOf(":", tokenMatches[i].index),
      msg.lastIndexOf("!", tokenMatches[i].index),
    ...
    const priorClauseWords = msg
      .slice(clauseStart + 1, tokenMatches[i].index)
      .match(/[a-z0-9]+/g) ?? [];
