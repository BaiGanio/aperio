...
/**
 * Load all skills from the bundled skills/ dir, then merge a user overlay on top.
 *
 ...
 */
export function loadSkillIndex(skillsDir, overlayDir) {
  ...
  const ingest = (dir, source) => {
    for (const filePath of findSkillFiles(dir)) {
      try {
        ...
        if (!skill) continue;
        if (source === "user" && byName.has(skill.name)) skill.overridden = true;
