 ...
 * @param {object}  [deps]    - optional dependency overrides (testing).
 * @param {Function} [deps.complete] - override for `complete` from completion.js
 * @param {object}  [deps.logger]   - override for the default logger
 */
export function inferMemories(callTool, deps = {}) {
  const callComplete = deps.complete ?? complete;
  ...
  async function runInference() {
    try {
      ...
      const response = await callComplete([{ role: 'user', content: PROMPT(memories) }]);
      ...
      if (stored > 0) log.info(`🧠 Inferred ${stored} new pattern(s)`);
    } catch (err) {
      // Inference failures are non-fatal — log at debug level only
      log.debug(`[infer] skipped: ${err.message}`);
    }
  }
