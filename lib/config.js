...
export const GROUPS = [
  ...
  { id: "features", title: "2 · OPTIONAL FEATURES",
    blurb: "Extra capabilities, all off or at their default until you turn them on. These are normally toggled in the app's Settings panel (saved to the database); they're listed here for headless / file-only setups. With APERIO_CONFIG_PRECEDENCE=db (the default) a value saved in the UI overrides what you set here." },
...
export const CONFIG = [
  ...
  { key: "APERIO_CONFIG_PRECEDENCE", section: "essentials", type: "select", tier: 0,
    show: "commented", default: "db", example: "db", options: ["db", "env"],
    help: "Who wins when a setting is in BOTH the Settings UI (DB) and your .env:\n• db (default) → the UI is authoritative; a saved value overrides .env.\n• env → a value present in .env wins over the DB, so you can override from the file without touching the UI. Settings that are ONLY in the DB (no .env entry) still apply — env mode overrides, it does not ignore the DB.\nEnv-only (edit in .env), since it decides whether .env can win." },
