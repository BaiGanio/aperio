...
// taskkill's exit code, not merely "the spawn succeeded". /T walks the tree
// and can come back nonzero — access denied on a descendant, or a helper that
// refuses the soft (no /F) close request — and treating a successful SPAWN as
// a successful KILL means the caller is told the tree is gone while it is
// still running.
//
// 128 is "no such process", and it is NOT success. That reasoning holds on
// POSIX, where the target is a process GROUP and ESRCH therefore means the
// group is empty — every descendant included, because the PGID outlives the
// leader (see signalChild). It does not transfer to Windows, which has no
// groups: 128 speaks only for the single parent PID. `taskkill /T` discovers
// descendants by walking down from that PID, so once git has exited there is
// nothing left to enumerate and the walk reports 128 having examined no
// descendant at all. A detached helper that outlived git is exactly the case
// this teardown exists for, so 128 must read as "could not confirm", not
// "confirmed clean".
const TASKKILL_NO_SUCH_PROCESS = 128;

// Windows has no process groups and no signals, so child.kill() reaches ONLY
// the git process: a git-spawned ssh or remote helper survives it, keeps
// running after spawnGit has already reported a timeout and destroyed its
// pipes, and can still complete a push we believed we had cancelled. taskkill
// /T is the platform's process-tree teardown (/F to force).
//
// Resolves true ONLY on exit code 0 — the one outcome that means taskkill
// found the parent, walked its tree and terminated it. Everything else is
// unconfirmed: taskkill missing from PATH on a stripped image, an EPERM/EACCES
// spawn error, a nonzero walk, or the 128 above. Where the failure might mean
// taskkill never acted, child.kill() is still attempted as a last resort so
// git itself does not survive; that fallback is pointless for 128, where the
// parent is already gone and only untouchable descendants can remain.
//
// This is a genuine platform limitation, not a coding gap: closing it properly
// needs a Job Object created at spawn time (KILL_ON_JOB_CLOSE), which Node
// does not expose without a native addon. Until then Windows reports honestly
// rather than guaranteeing.
//
// Exported only so this decision table can be exercised directly, on any
// platform, by driving taskkill's exit code through a spawn mock — the logic
// is what three review rounds kept finding bugs in, and asserting on the
// source text instead of running it is what let them survive. What still
// cannot be tested off win32 is whether real taskkill emits these codes.
// Not part of the git_* tool surface.
export function killWindowsTree(child, force) {
  ...
  return new Promise((resolve) => {
    ...
    killer.on("error", () => resolve(fallback()));
    killer.on("close", (code) => {
      if (code === 0) { resolve(true); return; }
      // The parent is already gone; child.kill() has nothing to add, and the
      // descendants it could not reach are precisely what stays unaccounted for.
      if (code === TASKKILL_NO_SUCH_PROCESS) { resolve(false); return; }
      resolve(fallback());
    });
  });
}
...
// lib/helpers/startLlamaCpp.js. On Windows the equivalent is taskkill /T
// above. Falls back to the single child on any failure, and treats an
// already-exited target as success — it is exactly the outcome we wanted.
//
// Always a promise: on POSIX process.kill() has already delivered the signal
// to the whole group by the time it returns, so the answer is immediate, but
// on Windows it is only known once taskkill exits. Callers await either way.
// Exported only so this decision table can be exercised directly, by driving
// process.kill/child.kill through mocks — the same rationale as
// killWindowsTree above. Not part of the git_* tool surface.
export function signalChild(child, sig) {
  if (SUPPORTS_PROCESS_GROUPS && child.pid) {
    try { process.kill(-child.pid, sig); return Promise.resolve(true); }
    catch (err) {
      // ESRCH means the group is already gone — the outcome we wanted, so
      // the single-process fallback below is irrelevant and success stands.
      // Any other error (EPERM, or the child was never a group leader) means
      // the group was NOT reached, so only the single git process can be
      // signalled here. Even if THAT signal is delivered, it says nothing
      // about any descendant git itself forked — the exact thing the group
      // kill exists to reach — so this is unconfirmed regardless of what
      // child.kill() returns; only "true" would misreport a possibly-still-
      // running tree as a confirmed teardown.
      if (err?.code !== "ESRCH") {
        try { child.kill(sig); } catch { /* best effort */ }
        return Promise.resolve(false);
      }
    }
  } else if (!SUPPORTS_PROCESS_GROUPS) {
    return killWindowsTree(child, sig === "SIGKILL");
  }
  try { child.kill(sig); } catch { /* best effort */ }
  return Promise.resolve(true);
}

// A teardown that cannot be confirmed must not hold the promise open forever —
// that is the "the advertised timeout is not hard" failure the escalation
// chain exists to prevent. So every wait on a kill is bounded, and a wait that
// runs out reports "unconfirmed" rather than blocking.
function withKillDeadline(promise, ms) {
  let timer;
  return Promise.race([
    promise.then((ok) => { clearTimeout(timer); return ok; }),
...
// In a partial clone (extensions.partialClone / remote.<name>.promisor), git
// silently FETCHES a missing object from the promisor remote in the middle of
// an ostensibly local command — `show`, `log`, `diff`, `cat-file`, all of
// them. Verified empirically (git 2.50.1): with the blob deleted from an
// allowed repo configured as a partial clone, a plain read-only
// `git show HEAD:f.txt` under SAFE_GIT_CONFIG executed a PATH binary named
// git-remote-ssh three times, because the repo also set remote.origin.vcs=ssh
// — a path assertAllowedRemote() never sees, since `show` is not one of the
// two REMOTE_TOUCHING_COMMANDS it gates. It is also unexpected network egress
// on a call the caller believes is purely local.
//
// GIT_NO_LAZY_FETCH=1 turns that off: the same command then fails with "bad
// object" and the helper is never executed — verified. It is set on every
// preflight unconditionally (a preflight only reads metadata and must never
// dial out to do it) and, in spawnGit, on every command except fetch/push,
// where talking to the remote is the whole point and both the helper and
// transport checks have already run.
//
// A git too old to know the variable ignores it silently; see
// LAZY_FETCH_SUPPRESSION_MIN_GIT for the floor and what happens below it.
// assertRemoteConfigIsSafe() also runs on EVERY invocation now, not just
// fetch/push — the remote.<name>.vcs half of it needs no git support at all.
//
// The cost is real and accepted: on a genuine partial clone, a read command
// that needs an absent blob now reports "bad object" instead of fetching it.
// A git_fetch first is the way to materialize it.
// When lazy fetching IS allowed the variable is removed rather than set to
// "0": a caller's own GIT_NO_LAZY_FETCH=1 in the environment is their choice
// to keep, and this function has no business quietly overriding it in the
// permissive direction. Only the restrictive direction is forced.
function gitEnv({ allowLazyFetch = false } = {}) {
  const env = { ...process.env };
  if (!allowLazyFetch) env.GIT_NO_LAZY_FETCH = "1";
  return env;
}

function runPreflightGit(args, options) {
  return execFileAsync("git", [...SAFE_GIT_CONFIG, ...args], { env: gitEnv(), ...options });
}
...
// `git rev-parse` prints the path followed by exactly one "\n" and adds no
// other whitespace, so stripping that single terminator — and nothing else —
// recovers the path verbatim. trim() must not be used here: on POSIX a path
// may legitimately contain newlines (and trailing spaces), and trim() would
// silently corrupt it into a DIFFERENT path that then gets boundary-checked
// in place of the real one. See resolveGitPaths() for the escape that caused.
function stripOneTrailingNewline(stdout) {
  return stdout.endsWith("\n") ? stdout.slice(0, -1) : stdout;
}

async function revParseOne(cwd, flag) {
  const { stdout } = await runPreflightGit(["rev-parse", flag], {
    cwd, timeout: TIMEOUT_MS, shell: false,
  });
  return stripOneTrailingNewline(stdout);
}

export async function resolveRepoRoot(cwd) {
  try {
    return await revParseOne(cwd, "--show-toplevel");
  } catch {
...
// before comparison.
//
// One flag per invocation, NOT all three in one call. Asking for all three at
// once returns three newline-separated records, and on POSIX a path may
// contain newlines — so splitting on "\n" mis-assigns the fields entirely.
// Verified empirically (git 2.50.1): from a linked worktree named "wt\nX\nY"
// under an allowed directory, the three-flag call parsed as
// repoRoot=".../allowed/wt", gitDir="X", gitCommonDir="Y", while the REAL
// git-dir and common-dir (".../main/.git", outside the allowlist) were never
// seen by assertPathBoundary at all — and assertNoEscapingMetadataSymlinks
// then walked the nonexistent "X"/"Y", hit ENOENT, and returned clean, so the
// whole metadata scan was skipped too. rev-parse has no NUL-delimited output
// mode for these flags, so separate invocations (each stripping exactly one
// terminating newline) are the only unambiguous parse. Three preflight
// spawns instead of one, per git_* call — the cost of a correct boundary.
async function resolveGitPaths(cwd) {
  try {
    const [repoRoot, gitDir, gitCommonDirRaw] = await Promise.all([
      revParseOne(cwd, "--show-toplevel"),
      revParseOne(cwd, "--absolute-git-dir"),
      revParseOne(cwd, "--git-common-dir"),
    ]);
...
// `git config --get-regexp` without -z is AMBIGUOUS, and dangerously so: it
// separates the key from the value with a single space, while a config
// SUBSECTION may itself contain spaces. Verified empirically (git 2.50.1) that
// `[remote "foo bar"] promisor = true` prints as
//     remote.foo bar.promisor true
// so splitting at the first space yields the key "remote.foo", which matches
// none of the patterns these scans test — the entry is silently skipped. That
// was a live bypass: a repo whose promisor remote is named with a space was
// verified to lazy-fetch during `git show`, and with a matching
// `remote.foo bar.vcs = ssh` it executed a planted git-remote-ssh from PATH.
//
// `-z` removes the ambiguity: records are NUL-terminated, and within a record
// the FIRST newline separates key from value. Verified empirically, including
// the three cases that carry meaning:
//     remote.a.promisor\0            -> no newline at all: git's implicit TRUE
//     remote.b c.promisor\n\0        -> present but empty: FALSE
//     remote.d.promisor\nline1\nline2\0 -> a value may itself contain newlines
...
async function discoverConfiguredDrivers(repoRoot) {
  ...
  try {
    ({ stdout } = await runPreflightGit(
      ["config", "-z", "--get-regexp", "^(filter\\..*\\.(clean|smudge|process|required)|merge\\..*\\.driver|diff\\..*\\.(textconv|command))$"],
      { cwd: repoRoot, timeout: TIMEOUT_MS, shell: false }
    ));
  } catch (err) {
  ...
  // -z for the same reason as the remote scan (see parseConfigRecords): a
  // driver name may contain a space, and a space-split key would skip it, so
  // its command would never be blanked. Not currently reachable — verified
  // empirically that `.gitattributes` cannot SELECT such a driver, because
  // attribute values are themselves space-delimited (`path filter=my filter`
  // parses as filter=my), so a space-named driver can never be invoked. Parsed
  // correctly anyway rather than resting the guarantee on that second fact.
  for (const { key } of parseConfigRecords(stdout)) {
...
//
// Called on EVERY invocation, not only the fetch/push pair — a partial
// clone's lazy fetch turns `show`/`log`/`diff` into remote operations that
// honour remote.<name>.vcs too (see gitEnv()).

// GIT_NO_LAZY_FETCH is only honoured from git 2.39 onward. Established by
// reading the release tarballs rather than trusting a changelog: the string
// appears NOWHERE in the 2.30.2, 2.34.8, 2.35.8, 2.36.6, 2.37.7 or 2.38.5
// trees, and appears in 2.39.5 at promisor-remote.c:24 as
// `git_env_bool("GIT_NO_LAZY_FETCH", 0)`. Confirmed behaviourally against a
// locally built 2.39.5: the fake-ssh probe fetches without the variable and is
// suppressed with it.
//
// Every platform this project targets clears that bar — Debian 12 ships 2.39
// and Ubuntu 24.04 ships 2.43 (vms/Vagrantfile), CI runs ubuntu-24.04/macos/
// windows-11 — but nothing STOPS an install on an older host (Ubuntu 22.04 is
// 2.34, Debian 11 is 2.30), and there the suppression is silently a no-op.
//
// There is no config-based substitute to fall back on: verified on BOTH 2.39.5
// and 2.50.1 that `-c remote.<name>.promisor=false` and
// `-c extensions.partialClone=` each still fetch, because the promisor remote
// is registered from the repository format read during setup, before any -c
// override applies. So on an old git the only honest options are "refuse" or
// "leak" — and only for a repo that is actually a partial clone, which is the
// sole configuration where a lazy fetch can happen at all. An ordinary repo
// (including the shallow `git clone --depth 1` the Debian VM itself does) is
// never affected.
const LAZY_FETCH_SUPPRESSION_MIN_GIT = [2, 39];

// Deliberately NOT cached across calls. It is consulted only when a repo turns
// out to be a partial clone — rare enough that the extra spawn is noise — and
// caching it would freeze a stale answer across a git upgrade, the same
// resolve-fresh-every-call stance resolveRepoRoot() takes for repo paths.
async function gitVersionAtLeast([wantMajor, wantMinor]) {
  let stdout;
  try {
    ({ stdout } = await runPreflightGit(["--version"], { timeout: TIMEOUT_MS, shell: false }));
  } catch (err) {
    throw new GitPolicyError(`could not determine the installed Git version, refusing to proceed: ${err.message}`);
  }
  const parsed = stdout.match(/^git version (\d+)\.(\d+)/);
  if (!parsed) {
    throw new GitPolicyError(`could not parse the installed Git version from "${stdout.trim()}", refusing to proceed`);
  }
...
// git's own boolean spelling, which is words OR an integer — not a fixed list
// of literals. git_config_bool() tries the words first and otherwise parses
// the value as an integer, where any zero is false. So "0" is not the only
// false spelling: verified against real git with `git config --bool` that
// "00", "+0", "-0" and even "0k" (the k/m/g size suffixes apply here too) all
// report FALSE, while "1", "true", "yes" and "on" report true. Matching only
// the literal "0" classified an ordinary repo as a partial clone and rejected
// every local command on an old git.
//
// A key printed with NO value at all is git's implicit true
// (`[remote "a"]\n\tpromisor`), which is why the caller passes null for that
// case rather than an empty string — an explicitly empty value is FALSE, and
// parseConfigRecords keeps the two distinguishable.
//
// Anything git itself would reject as a boolean is treated as true: for the
// promisor question that is the fail-closed direction (assume partial clone,
// refuse on an old git) rather than waving the repo through.
const GIT_BOOL_TRUE_WORDS = new Set(["true", "yes", "on"]);
const GIT_BOOL_FALSE_WORDS = new Set(["false", "no", "off", ""]);
const GIT_BOOL_INTEGER = /^[+-]?\d+[kmg]?$/;
function configValueIsTrue(value) {
  if (value === null) return true;
  const normalized = value.trim().toLowerCase();
  if (GIT_BOOL_TRUE_WORDS.has(normalized)) return true;
  if (GIT_BOOL_FALSE_WORDS.has(normalized)) return false;
  // parseInt stops at the suffix, which is what we want: the suffix only
  // scales the magnitude, and no multiple of zero is nonzero.
  if (GIT_BOOL_INTEGER.test(normalized)) return Number.parseInt(normalized, 10) !== 0;
  return true;
}

// One config scan covering both remote-side risks, rather than a second spawn
// per call for the promisor half: the foreign-VCS helper override, and whether
// this repo is a partial clone at all.
async function assertRemoteConfigIsSafe(repoRoot, { allowLazyFetch }) {
  let stdout;
  try {
    ({ stdout } = await runPreflightGit(
      ["config", "-z", "--get-regexp", "^(remote\\..*\\.(vcs|promisor)|extensions\\.partialclone)$"],
      { cwd: repoRoot, timeout: TIMEOUT_MS, shell: false }
    ));
  } catch (err) {
    if (err.code === 1) return; // nothing configured — the normal case
  ...
  for (const { key, value } of parseConfigRecords(stdout)) {
    if (key.endsWith(".vcs")) {
      vcsKeys.push(key);
    } else if (key === "extensions.partialclone") {
      if (value !== null && value.trim()) isPartialClone = true;
    } else if (key.endsWith(".promisor") && configValueIsTrue(value)) {
      isPartialClone = true;
    }
  }

  if (vcsKeys.length) {
    throw new GitPolicyError(`remote helper override not allowed: this repo configures ${vcsKeys.join(", ")}, which would run an external git-remote-* helper instead of git's own https/ssh transport`);
  }

  // fetch/push are allowed to reach the remote, so an old git costs nothing
  // there; it is only the commands that must stay local that need the
  // suppression to actually work.
  if (!allowLazyFetch && isPartialClone && !(await gitVersionAtLeast(LAZY_FETCH_SUPPRESSION_MIN_GIT))) {
    const [minMajor, minMinor] = LAZY_FETCH_SUPPRESSION_MIN_GIT;
    throw new GitPolicyError(
      `this repository is a partial clone, and the installed Git is older than ${minMajor}.${minMinor}, which ignores GIT_NO_LAZY_FETCH — a read-only command could silently fetch a missing object from the promisor remote. Upgrade Git to ${minMajor}.${minMinor} or newer, or use a full clone.`
    );
  }
}
...
export function spawnGit(argv, { cwd, timeout, allowLazyFetch = false }) {
  return new Promise((resolve) => {
    ...
    try {
      child = spawn("git", argv, {
        cwd,
        shell: false,
        // See gitEnv(): a partial clone turns a read-only command into a
        // remote fetch, which is how a repo-chosen git-remote-* helper got to
        // run under `git show`.
        env: gitEnv({ allowLazyFetch }),
    ...
    // Monotone: a tree kill that genuinely succeeded is never downgraded by a
    // later attempt that could not confirm. On Windows the second attempt
    // routinely CANNOT confirm — once git has exited, `taskkill /T` has no
    // parent left to walk and returns "no such process" (see
    // TASKKILL_NO_SUCH_PROCESS) — so without this, a real teardown followed by
    // a redundant probe would be reported as unconfirmed.
    let treeKillConfirmed = false;
    const recordKill = (ok) => {
      treeKillConfirmed = treeKillConfirmed || ok;
      return treeKillConfirmed;
    };

    // The forced tree kill, started at most once and remembered, so that both
    // the escalation chain and an early "close" wait on the SAME teardown
    // instead of racing to settle around it.
    let forcedKill = null;
    const startForcedKill = () => {
      if (!forcedKill) {
        forcedKill = withKillDeadline(signalChild(child, "SIGKILL"), KILL_GRACE_MS).then(recordKill);
      }
      return forcedKill;
    };

    // Settling a TIMED-OUT call must not happen until the tree is actually
    // torn down. "close" fires when git has exited AND its pipes are closed —
    // neither of which says anything about a descendant that ignored SIGTERM
    // and never held those pipes. Reproduced on POSIX before this fix: a fake
    // git whose grandchild traps TERM and redirects its stdio away from the
    // pipes died on the first rung, "close" arrived at ~1007ms, finish()
    // cleared the SIGKILL timer, and the grandchild was still alive
    // afterwards. So the forced kill is issued and awaited here rather than
    // being cancelled by the early close. Bounded by withKillDeadline(), and
    // when the bound runs out the caller is TOLD the teardown is unconfirmed
    // instead of being handed a clean-looking cancellation.
    const settleTimedOut = async (note) => {
      if (settled) return;
      const confirmed = await startForcedKill();
      finish(1, confirmed
        ? note
        : `${note}\n[aperio] could not confirm every process git spawned was terminated`);
    };

    timers.push(setTimeout(() => {
      timedOut = true;
      // Not awaited: the soft rung is only an invitation to exit, and the
      // grace timer below is what decides how long it is given. It can never
      // reject — signalChild resolves a boolean on every path.
      //
      // Deliberately NOT fed to recordKill(). On Windows a soft `taskkill /T`
      // can exit 0 having merely posted WM_CLOSE to the tree, which a console
      // process is free to ignore; counting that as a confirmed teardown is
      // the same over-claim as counting a bare spawn. Only the forced kill
      // confirms.
      void signalChild(child, "SIGTERM");
      ...
      timers.push(setTimeout(() => {
        void settleTimedOut(`[aperio] git command timed out after ${timeout}ms and did not exit after SIGTERM`);
      }, KILL_GRACE_MS));
    }, timeout));
    ...
    child.on("close", (code) => {
      if (timedOut) {
        void settleTimedOut(`[aperio] git command timed out after ${timeout}ms`);
        return;
      }
...
export async function runGit({ cwd, argv, mutating }) {
  ...
  // Unconditional, ahead of the fetch/push-only URL check below: a partial
  // clone's lazy fetch makes ANY command a potential remote operation, so the
  // remote-helper override has to be denied for all of them, and a git too old
  // to suppress that fetch has to be refused outright. See gitEnv().
  const allowLazyFetch = REMOTE_TOUCHING_COMMANDS.has(argv[0]);
  await assertRemoteConfigIsSafe(repoRoot, { allowLazyFetch });
  ...
  const result = await spawnGit(
    ...
    // Lazy fetching stays available only where dialling out is the command's
    // own purpose and both remote checks above have already cleared it.
    { cwd: repoRoot, timeout: TIMEOUT_MS, allowLazyFetch }
  );
