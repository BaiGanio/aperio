...
import { resolveAssignmentDate } from "./facts/contract.js";
...
// service_period_start/end are deliberately absent: resolveAssignmentDate()
// (facts/contract.js) never uses them either, since every bill in this
// corpus covers the PREVIOUS month's consumption — keying the period on the
// service period would slide every charge one month backwards (#314, tech-debt.md).
const TERMINAL_DATE_ROLES = new Set(["due_date", "invoice_date", "document_date"]);
...
export function composeMemoryFromDoc(dates, amounts, docInfo = {}) {
  ...
  // is resolved by resolveAssignmentDate() (facts/contract.js) — the single
  // source of truth for "which month does a charge belong to", already
  // ranking invoice/document date over the due date (which can fall a month
  // later and would misclassify a May bill as June, #314) and already
  // excluding the service period. Ambiguous dates (value: null, or multiple
  // distinct values for the winning role) are handled the same way — the
  // resolver reports no usable date and this returns null.
  ...
  const resolved = resolveAssignmentDate(terminalDates);
  if (!resolved.date) return null;
  const periodDate = terminalDates.find(d => d.role === resolved.source && d.value === resolved.date);
  ...
  const periodContext = periodDate.role === "invoice_date" || periodDate.role === "document_date"
