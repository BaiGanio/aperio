...
function dateHint(row) {
  ...
  // Interpolating a Date directly calls its default toString() ("Tue Jun 03
  // 2026..."), which the numeric YYYY-MM/MM-YYYY regex below can never match
  // — silently losing every Postgres row's date signal for period filtering.
  const mtime = row.mtime instanceof Date ? row.mtime.toISOString() : (row.mtime ?? "");
  const source = `${mtime} ${row.rel_path ?? ""} ${row.title ?? ""}`;
  return source.match(/20\d{2}[-_]\d{1,2}(?:[-_]\d{1,2})?|\b(?:0?[1-9]|1[0-2])[-_]20\d{2}\b/)?.[0] ?? null;
}

// Canonicalizes a dateHint (which may be "YYYY-MM[-DD]" or "MM-YYYY", with
// either "-" or "_" separators) down to "YYYY-MM" so it can be compared
// against a requested period regardless of which shape produced it.
...
export function buildCandidateManifest(rows = [], { query = "", limit = RETRIEVAL_LIMITS.maxCandidates, now = new Date() } = {}) {
  ...
  const utilityQuery = terms.some(term => ["utilities", "utility"].includes(term));
  let relevant = utilityQuery ? ordered.filter(candidate => candidate.score >= 5) : ordered;
  ...
  // against unrelated months once a corpus exceeds maxCandidates.
  if (requestedPeriod) {
    const periodMatches = relevant.filter(candidate => canonicalPeriod(candidate.date_hint) === requestedPeriod);
  ...
  while (index < candidates.length) {
    ...
    for (const subBatch of splitIntoByteBoundedSubBatches(admitted, maxBatchBytes)) {
      ...
      for (const candidate of subBatch) {
        ...
        bytes += resultBytes;
        read++;
        documents.push({ ...candidate, ...result, status: "read", bytes: resultBytes });
      }
    }
  }
  return {
