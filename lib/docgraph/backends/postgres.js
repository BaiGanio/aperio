...
const snippetOf = (t) => (t.length <= SNIPPET_CHARS ? t : t.slice(0, SNIPPET_CHARS).trimEnd() + '…');
...
async function reindexDocument(client, docId, parsed) {
  await client.query(`UPDATE docgraph_documents SET title=$1, summary=$2 WHERE id=$3`,
    [parsed.title ?? null, parsed.summary ?? null, docId]);
  ...
  for (const s of parsed.sections) {
    ...
    const { rows } = await client.query(
      ...
      [docId, parent, s.ord, s.level, s.heading ?? null, s.text ?? '']
    );
...
export async function indexRepoFiles(store, rootPath, fileIterator, { generateEmbedding } = {}) {
  ...
  try {
    ...
    return { ...counts, docCount: dc[0].n, chunkCount: cc[0].n };
  } catch (err) {
...
export async function search(store, { query, folder, mime, limit = 20 }, { generateEmbedding, vectorEnabled } = {}) {
  ...
  if (!queryVec) {
    const params = [query];
    const conds = [`to_tsvector('simple', c.text) @@ plainto_tsquery('simple', $1)`];
  ...
  const { rows } = await pool.query(`
    ...
    fts_ranked AS (
      SELECT c.id, ROW_NUMBER() OVER (
               ORDER BY ts_rank(to_tsvector('simple', c.text), plainto_tsquery('simple', $2)) DESC) AS rank
        FROM docgraph_chunks c JOIN docgraph_documents d ON d.id = c.document_id
       WHERE to_tsvector('simple', c.text) @@ plainto_tsquery('simple', $2) ${extras}
       LIMIT 60
    ),
    fused AS (
    ...
    SELECT c.id AS chunk_id, c.section_id, c.text AS chunk_text,
           d.rel_path, d.title, d.mime, s.heading, s.level, r.root_path, fu.rrf AS score
      FROM fused fu
  ...
  `, params);
  return { matches: rows.map((r) => toHit(r, r.score)), mode: 'hybrid' };
}
