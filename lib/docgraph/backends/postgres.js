...
export async function listChunksWithoutEmbeddings(store) {
  const { rows } = await store.pool.query(
    `SELECT id, text FROM docgraph_chunks WHERE embedding IS NULL`
  );
...
export async function batch(store, { candidates = [], signal, batch_size, max_file_bytes, max_batch_bytes, max_total_bytes } = {}) {
  ...
  return retrieveInBatches(candidates, {
    ...
    readBatch: async (batchCandidates) => {
      ...
      `, [ids]);
      return rows.map(row => ({
        id: row.id, text: row.text ?? '', bytes: Buffer.byteLength(row.text ?? '', 'utf8'),
