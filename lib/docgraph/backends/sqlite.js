...
//   • Embeddings are written inline during indexing (mirrors codegraph's
//     embedInline) — the generic embedding-queue targets a single fixed table
//     and can't address vec_docgraph_chunks.
...
export async function indexRepoFiles(store, rootPath, fileIterator, { generateEmbedding } = {}) {
  ...
  return { ...counts, docCount, chunkCount };
}

// Incremental single-document index (watcher add/change). Reads the file,
// short-circuits on unchanged sha256, otherwise re-extracts + embeds inline.
export async function indexOneFile(store, rootPath, relPath, { mime, extract, generateEmbedding } = {}) {
  const db = store.db;
...
// a column name. Tokenize on non-word chars and rejoin as plain terms.
...
export async function search(store, { query, folder, mime, limit = 20 }, { generateEmbedding, vectorEnabled } = {}) {
  ...
  const toHit = (r, score) => ({
    ...
    chunk_id: r.chunk_id,
    snippet: snippetOf(r.chunk_text),
  });
  ...
  // ── Hybrid path (RRF) — vec0 MATCH must be a top-level WHERE clause, so run
  // vector + FTS as separate statements and fuse in JS (same as codegraph).
  ...
  if (repoId) { vecConds.push(`d.repo_id = @repo`); ftsConds.push(`d.repo_id = @repo`); params.repo = repoId; }
  if (mime)   { vecConds.push(`d.mime = @mime`);    ftsConds.push(`d.mime = @mime`);    params.mime = mime; }
  ...
  const detail = detailById(ranked.map(([id]) => id));
  const matches = ranked
    .map(([id, score]) => { const r = detail.get(id); return r ? toHit(r, score) : null; })
    .filter(Boolean);
