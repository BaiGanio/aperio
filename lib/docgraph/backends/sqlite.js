...
// Internal — called only from inside a db.transaction() (see each call site
// below), with the CONFIRMED-removed relPaths from a delete just above it,
// not a merely-planned set. Purges unconditionally — no existence re-check
// needed — because SQLite grants only one writer at a time for the whole
// file (cross-process, not just this Node process), so nothing else can
// touch this repo's documents or memories for the rest of the transaction
// (#360 review round 4, P1 & P2 — mirrors purgeBridgeMemoriesForRemovedPaths
// in backends/postgres.js). Wiki staleness is marked BEFORE the delete —
// wiki_article_sources rows cascade-delete the instant their memory row
// goes, so looking them up after would always find nothing.
function purgeBridgeMemoriesForRemovedPathsSync(db, rootPath, relPaths) {
  ...
  db.prepare(`
    DELETE FROM memories
     WHERE source = 'docgraph'
       AND EXISTS (SELECT 1 FROM json_each(tags) je WHERE je.value IN (${placeholders}))
  `).run(...tags);
}

// Deletes the document AND purges its bridge memory in ONE db.transaction()
// (#360 review round 4, P1 & P2): a failure in either half rolls back both,
// so a document can never end up deleted while its memory silently
// survives with no way to retry — the next removeFile/sweepMissing/reindex
// simply gets a clean shot at the whole thing again.
export async function removeOneFile(store, rootPath, relPath) {
  ...
  const tx = db.transaction(() => {
    ...
    if (info.changes > 0) purgeBridgeMemoriesForRemovedPathsSync(db, rootPath, [relPath]);
    return { removed: info.changes > 0 };
  });
  const result = tx();
  // The purge above writes straight to the `memories` table, bypassing
  // store.delete() — the only path that normally keeps store.cache in sync
  // (#360 review). Refresh once the transaction has committed so a purged
  // bridge memory doesn't linger in the cache until an unrelated write.
  if (result.removed) await store.refreshCache();
  return result;
}
...
export async function documentExists(store, rootPath, relPath) {
  ...
  return !!db.prepare(`SELECT 1 FROM docgraph_documents WHERE repo_id = ? AND rel_path = ?`).get(repo.id, relPath);
}
...
// vec_docgraph_chunks entirely) would otherwise leave its chunks permanently
// unembedded. Called once at watcher startup alongside bulkPending, scoped
// to rootPath's own repo — otherwise every watched root re-enqueues every
// other root's pending chunks on its own startup.
...
export async function listRepoRoots(store) {
  return store.db.prepare(`SELECT root_path FROM docgraph_repos ORDER BY root_path`)
...
export async function listChunksWithoutEmbeddings(store, rootPath, { limit = null, offset = 0 } = {}) {
  ...
  const rows = db.prepare(`
    ...
    LEFT JOIN vec_docgraph_chunks v ON v.rowid = c.id
    WHERE v.rowid IS NULL AND r.root_path = ?
    ORDER BY c.id
    LIMIT ? OFFSET ?
  `).all(rootPath, limit ?? -1, offset);
...
export async function sweepMissingFiles(store, rootPath, statFn) {
  const db = store.db;
  const repo = db.prepare(`SELECT id FROM docgraph_repos WHERE root_path = ?`).get(rootPath);
  if (!repo) return { removed: 0, relPaths: [] };
  ...
  // The stat loop above is async (one fs call per document) and can't run
  // inside a sync db.transaction(), so `gone` is only a plan, not a fact
  // yet — see removeOneFile's comment for why the delete+purge below is
  // folded into one transaction (#360 review round 4, P1 & P2). Reporting
  // relPaths from RETURNING rather than `gone` matters here specifically: a
  // path can have been reindexed back between the stat check and this
  ...
  const tx = db.transaction(() => {
    ...
    const removed = db.prepare(`
      DELETE FROM docgraph_documents WHERE repo_id = ? AND rel_path IN (${placeholders}) RETURNING rel_path
    `).all(repo.id, ...gone).map(r => r.rel_path);
    purgeBridgeMemoriesForRemovedPathsSync(db, rootPath, removed);
    return removed;
  });
  const removed = tx();
  // Same cache-sync gap as removeOneFile above — the purge writes straight
  // to `memories`, so refresh store.cache once the delete has committed.
  if (removed.length) await store.refreshCache();
...
export async function deleteDocumentsPage(store, rootPath, limit) {
  ...
  const tx = db.transaction(() => {
    const repo = db.prepare(`SELECT id FROM docgraph_repos WHERE root_path = ?`).get(rootPath);
    if (!repo) return [];
    const relPaths = db.prepare(`
      DELETE FROM docgraph_documents
       WHERE id IN (SELECT id FROM docgraph_documents WHERE repo_id = ? ORDER BY id LIMIT ?)
      RETURNING rel_path
    `).all(repo.id, limit).map(r => r.rel_path);
    // Purging the page's bridge memories in the SAME transaction as the
    // delete (#360 review round 4, P1 & P2) means a purge failure rolls
    // back the delete too — a page can never leave a document gone with its
    // memory stranded and no way to retry.
    purgeBridgeMemoriesForRemovedPathsSync(db, rootPath, relPaths);
    return relPaths;
  });
  return tx();
}
...
export async function finalizeRepoDelete(store, rootPath) {
  ...
  const tx = db.transaction(() => {
    const repo = db.prepare(`SELECT id FROM docgraph_repos WHERE root_path = ?`).get(rootPath);
    if (!repo) return { relPaths: [], deleted: false };
    const relPaths = db.prepare(`DELETE FROM docgraph_documents WHERE repo_id = ? RETURNING rel_path`)
      .all(repo.id).map(r => r.rel_path);
    purgeBridgeMemoriesForRemovedPathsSync(db, rootPath, relPaths);
    const info = db.prepare(`DELETE FROM docgraph_repos WHERE id = ?`).run(repo.id);
    return { relPaths, deleted: info.changes > 0 };
  });
  const result = tx();
  // Same cache-sync gap as removeOneFile — this is also the operation that
  // always closes deleteRepo()'s paged loop, so one refresh here covers
  // every page's purge from that loop too, not just this final page.
  if (result.relPaths.length) await store.refreshCache();
  return result;
}
...
// arguments (mcp/tools/docgraph.js's `sha256: z.string().nullable().optional()`
// on each candidate) — normally an unmodified copy of an earlier doc_manifest
// row, but nothing enforces that. retrieveInBatches's session dedup keys
// SOLELY on `candidate.sha256`, so a stale or mismatched hash (copied from a
// different document, or hand-edited) would return document A's cached
// text/facts under document B's id/rel_path, silently corrupting aggregation.
// Overwrite every candidate's sha256 with the value actually stored for its
// id before dedup ever sees it — never trust the request payload for
// identity. An id no longer indexed gets sha256: null, which
// retrieveInBatches already treats as "never dedup" (the safe fallback).
//
// The SAME refresh covers the full identity (rel_path/root_path/mime/title),
// not just the hash: the dedup cache is keyed by sha256, which two DIFFERENT
// documents can share (content twins) and which is unchanged by a rename —
// so a dedup hit must attribute identity from the CURRENT row for the
// requested id, never from the first document that read this content. The
// dedup-hit branch in retrieveInBatches returns the candidate's fields for
// identity; refreshing them here guarantees they are the current DB truth,
// not stale manifest copies (llamacpp-multiturn-latency.md Step 3 review,
// round 8, P1).
function withTrustedSha256Sqlite(db, candidates) {
...
export async function batch(store, { candidates = [], signal, sessionId, deferCommit, batch_size, max_file_bytes, max_batch_bytes, max_total_bytes } = {}) {
  const db = store.db;
  const trustedCandidates = withTrustedSha256Sqlite(db, candidates);
  return retrieveInBatches(trustedCandidates, {
    ...
    readBatch: async (batchCandidates) => {
      ...
      // d.sha256 is fetched in the SAME query as the text: the session dedup
      // cache is keyed by the hash read from this very row (see
      // retrieveInBatches's pendingFactWrites), so a concurrent re-index can
      // never cache new text under an old hash (round 9, P2).
      const rows = db.prepare(`
