...
// SKIP_DIRS must only match segments *inside* the watched root — checking the
// full absolute path lets an unrelated ancestor segment (e.g. macOS temp dirs
// resolving under /private/var/folders/...) falsely match "var" and silently
// skip the whole root (found live 2026-08-21, id/reference/tech-debt.md).
export function hasSkippedSegment(rootPath, absPath) {
  const relPath = path.relative(rootPath, absPath);
  for (const part of relPath.split(path.sep)) {
    if (SKIP_DIRS.has(part)) return true;
  }
  return false;
}

export function isIndexable(rootPath, absPath) {
  const ext = path.extname(absPath).slice(1).toLowerCase();
  if (!INDEXABLE_EXTS.has(ext)) return false;
  return !hasSkippedSegment(rootPath, absPath);
}
...
export async function startWatcher(store, rootPath, events, {
  ...
  const watcherOptions = {
    ignored: (p) => {
      ...
      return hasSkippedSegment(rootPath, p);
    },
    ignoreInitial: true,
  ...
  watcher
    .on('add',    (abs) => { if (isIndexable(rootPath, abs)) schedule(path.relative(rootPath, abs), 'index'); })
    .on('change', (abs) => { if (isIndexable(rootPath, abs)) schedule(path.relative(rootPath, abs), 'index'); })
    .on('unlink', (abs) => { if (isIndexable(rootPath, abs)) schedule(path.relative(rootPath, abs), 'remove'); })
