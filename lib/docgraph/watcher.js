...
import { indexRepo, indexFile, removeFile, sweepMissing, isDocgraphAvailable, listPendingEmbeddings, SKIP_DIRS, INDEXABLE_EXTS } from './indexer.js';
...
export async function startWatcher(store, rootPath, events) {
  ...
  try {
    ...
    // change (which wipes vec_docgraph_chunks outright) would otherwise
    // leave them permanently unembedded. Same query shape as
    // initEmbeddings() for memories: fast no-op in the steady state, catches
    // everything a plain reindex would miss when it isn't. enqueueMany
    // dedupes by id, so overlap with bulkPending above is harmless.
    const stillPending = await listPendingEmbeddings(store, rootPath);
