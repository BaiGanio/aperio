 ...
 *   guess (see LIKELY_TOTAL fallback below), never a confident match.
 */
export function extractAmountCandidates(text) {
  ...
  // reasonable best guess. Only engage when this document matched NO real
  // label anywhere — a document we already partially understand (e.g. its
  // subtotal matched but a VAT line didn't) must not get a second, competing
  // "total" guess planted next to the real amount_due/grand_total/etc.
  const anyRealLabel = positions.some(p => p.entry.label !== null);
  if (!anyRealLabel) {
    const unlabeledWithCurrency = positions.filter(p => p.entry.currency !== null);
