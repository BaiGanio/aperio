...
const LABEL_LOOKBACK = 40; // chars scanned before an amount for its label
...
export function extractAmountCandidates(text) {
  ...
  for (const m of text.matchAll(AMOUNT_RE)) {
    ...
    out.push({ value, currency, raw: m[0].trim(), label });
  }
  ...
  for (const { label, re } of AMOUNT_LABELS) {
    for (const lm of text.matchAll(new RegExp(re.source, "gi"))) {
      ...
      out.push({ value: parseAmount(nm[0]), currency: null, raw: nm[0].trim(), label });
    }
  }
