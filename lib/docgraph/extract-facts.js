...
// Bulgarian labels are evidenced against the BG household fixture corpus
// (issue #250 doc-intelligence epic), same scope disclaimer as
// AMOUNT_LABELS below — not a general translation table for every locale
// Aperio's UI supports. Unlike amounts, dates had no non-English coverage at
// all until this pass: every date on a Bulgarian invoice fell through to
// "unlabeled_date", leaving a bill's May consumption period
// ("Период на отчитане") and June issue/due dates equally unlabeled — a
// model asked "last month" then has nothing to prefer the issue/due date
// over the service period it's billing for, and can easily pick the wrong
// month (observed: WS0-R red run, doc-intelligence epic #313 follow-up).
// "Дата:" is anchored to require the colon immediately after the word
// (only whitespace between) so it never also matches inside "Дата на
// издаване:", which the more specific invoice_date pattern above it claims.
const LABELS = [
  ...
  { role: "invoice_date", re: /дата\s*на\s*издаване/gi },
  { role: "due_date", re: /краен\s*срок(?:\s*за\s*плащане)?/gi },
  { role: "service_period", re: /период\s*на\s*отчитане/gi, period: true },
  // The bilingual form "Дата (Date):" puts a parenthesized English gloss between
  // the Bulgarian word and the colon — use [^:\n]* to accept any characters
  // (including parenthesized text, spaces, punctuation) up to the colon, so this
  // single pattern covers both "Дата:" and "Дата (Date):" without needing a
  // separate pattern per language (#313).
  { role: "document_date", re: /(?:^|\n)[ \t]*дата[^:\n]*:/gi },
];
...
// Keep the grouped-number form first, but prevent it from accepting the
// first three digits of an ungrouped four-or-more-digit amount. Without the
// boundary, `1234,56` is tokenized as `123` followed by `4,56`.
const NUMBER_TOKEN_SRC = "\\d{1,3}(?:[.,\\s]\\d{3})*(?:[.,]\\d{1,2})?(?!\\d)|\\d+(?:[.,]\\d{1,2})?(?!\\d)";
...
const AMOUNT_LABELS = [
  ...
  { label: "amount_due", re: /(?:^|\n)[ \t]*за\s*плащане/i },
  // "Сума за плащане" / "Обща сума за плащане" — the other everyday Bulgarian
  // phrasing for the same field. Deliberately NOT line-anchored, unlike the
  // bare "за плащане" above: the two traps that anchoring exists to avoid,
  // "Краен срок за плащане" (a date) and "Основание за плащане" (free text),
  // never contain "сума", so requiring the noun is a stronger guard than
  // requiring line-start — and it also catches the "Обща сума…" form, which
  // an anchor would miss.
  { label: "amount_due", re: /сума\s*за\s*плащане/i },
  // Credit notes state a refund, not a charge: "СУМА ЗА ВЪЗСТАНОВЯВАНЕ (с ДДС)".
  // Given its own label rather than amount_due because it is terminal for the
  // document but is not money owed — the value reduces the category it
  // belongs to. Printed signs vary (some printers omit the minus); the
  // extractor normalizes an unsigned refund_due to a credit in
  // extractAmountCandidates, so the value is always negative when it reaches
  // downstream aggregation.
  { label: "refund_due", re: /сума\s*за\s*възстановяване/i },
  // "ОБЩА СУМА" is what a Bulgarian till receipt prints for its final total.
  // It is a grand_total rather than an amount_due because it states the face
  // value of the document; where a form says "Обща сума за плащане", the
  // more specific "сума за плащане" pattern above ends later in the label
  // window and wins, which is the intended precedence.
  { label: "grand_total", re: /обща\s*сума/i },
  { label: "subtotal", re: /стойност\s*без\s*ддс/i },
  { label: "subtotal", re: /междинна\s*сума/i },
  ...
  // Mapped to "amount_due" (not "amount") because in every bilingual form this
  // pattern targets, the labeled figure IS the amount actually owed — and the
  // more specific label helps the model categorize and the memory bridge
  // auto-promote the fact for trend-question recall (#314).
  { label: "amount_due", re: /\(amount\)/i },
];
...
const PAYMENT_DESC_WINDOW = 200; // chars scanned after the amount for payment-details context
...
/**
 * @param {string} text
 * @returns {Array<{value, currency, raw, label, description?}>} `currency` is
 *   a 3-letter ISO code or null when a money-shaped number carried no
 *   detectable currency marker; `value` is null (never 0) when the number
 *   itself couldn't be parsed. `label` may be "likely_total" — a
 *   language-agnostic guess from either the tax-percentage-adjacency signal
 *   or the whole-document fallback below, never a confident match.
 *   `description` is an optional free-text string extracted from a nearby
 *   payment-details line (bilingual split-field forms only).
 */
export function extractAmountCandidates(text) {
  ...
  for (const m of text.matchAll(AMOUNT_RE)) {
    ...
    const parsed = parseAmount(numberRaw);
    // Credit notes/refunds print negative amounts with a bare minus directly
    // against the digit (Bulgarian "-34,20 лв") — NUMBER_TOKEN_SRC has no
    // sign of its own, so the match starts one char late and silently drops
    // it, turning a refund into a charge for any downstream aggregation.
    // Only the number-before-currency form (m[3]) can have this: the
    // currency-before-number form's "\s?" allows just one optional
    // whitespace char between currency and number, never a "-". A hyphen
    // preceded by another digit is a range ("10-20 EUR") or hyphenated
    // identifier ("Order 123-45.20 EUR") splitting on the number token, not
    // a sign — a genuine minus is never itself glued to a preceding digit.
    const beforeHyphen = m.index >= 2 ? text[m.index - 2] : "";
    const isNegative = m[3] !== undefined && text[m.index - 1] === "-" && !/\d/.test(beforeHyphen);
    const value = parsed === null ? null : (isNegative ? -parsed : parsed);
    const start = isNegative ? m.index - 1 : m.index;
    const end = m.index + m[0].length;
    const raw = text.slice(start, end).trim();
    const label = labelFor(text, m.index);
    const key = `${value}|${currency}|${label}|${start}`;
    if (seen.has(key)) continue;
    seen.add(key);
    consumed.push([start, end]);
    positions.push({ entry: { value, currency, raw, label }, index: start });
  }
  ...
  for (const { label, re } of AMOUNT_LABELS) {
    for (const lm of text.matchAll(new RegExp(re.source, "gi"))) {
      const windowStart = lm.index + lm[0].length;
      // The value belongs to the same printed field, so scan to the end of
      // its line. A fixed character window can cut a longer padded amount in
      // half (e.g. `1234,56`), silently changing its magnitude or dropping it.
      const lineBreak = text.indexOf("\n", windowStart);
      const windowEnd = lineBreak === -1 ? text.length : lineBreak;
  ...
  // Credit notes state money coming back even when the printer omits the
  // minus sign ("СУМА ЗА ВЪЗСТАНОВЯВАНЕ: 34,20 лв"). The label — not the
  // printed sign — is the document's claim, so an unsigned refund is
  // normalized to a credit here: downstream aggregation sums signed amounts,
  // and a positive refund would inflate spending instead of reducing it.
  // An already-negative printed value is left alone (negating it again would
  // flip it back to a charge). Applied to every candidate the label matched,
  // including the currency-less bare-number path above.
  for (const p of positions) {
    if (p.entry.label === "refund_due" && p.entry.value !== null && p.entry.value > 0) {
      p.entry.value = -p.entry.value;
    }
  }
  ...
  if (currencyHits.length) {
    for (const p of positions) {
      ...
      if (hit) {
        p.entry.currency = hit.currency;
        // Bilingual split-field: extract payment-description context from a
        // nearby "Основание (Payment details):" line so the model sees what
        // the amount pays for (e.g. "Интернет 05/2026") alongside the figure.
        // The currency-link window is intentionally wider here because
        // label can be 55+ chars past the amount. Use a dedicated wider scan.
        const afterAmount = text.slice(numberEnd, numberEnd + PAYMENT_DESC_WINDOW);
        const pdMatch = afterAmount.match(/(?:основание\s*)?\(payment\s*details\)\s*:\s*(.+)/i);
        if (pdMatch) {
          const desc = pdMatch[1].trim();
          if (desc) p.entry.description = desc;
        }
      }
    }
  }
...
// Role vocabularies only — not the matching regexes, which stay private so
// nothing outside this module can re-implement or drift from the matching
// logic above. Document Intelligence WS3 (issue #250) validates a learned
// template's amount_label/date_role fields against these at template-create
// time, so a template can only ever reference a role this module actually
// produces.
export const AMOUNT_LABEL_ROLES = Object.freeze(
  [...new Set(AMOUNT_LABELS.map((l) => l.label)), "likely_total"].sort()
);
export const DATE_LABEL_ROLES = Object.freeze(
  [...new Set(LABELS.flatMap((l) => (l.period ? [`${l.role}_start`, `${l.role}_end`] : [l.role]))), "unlabeled_date"].sort()
);
