// lib/docgraph/indexer.js
// Dispatcher for the document graph. Mirrors lib/codegraph/indexer.js: walking
// the filesystem is shared here, DB writes are backend-specific. Only the
// SQLite backend exists in this milestone (Postgres parity is Phase 6).
//
// docgraph indexes human content (Markdown/text in Phase 1–2), explicitly NOT
// code — code files belong to codegraph. The skip list is therefore narrower
// than codegraph's; the extension filter is the real gate.

import { readdir, stat } from 'fs/promises';
import path from 'path';
import { extract as extractMd } from './extract-md.js';
import { extract as extractHtml } from './extract-html.js';
import { extract as extractPdf } from './extract-pdf.js';
import { extract as extractDocx } from './extract-docx.js';
import { extract as extractXlsx } from './extract-xlsx.js';
import { extract as extractPptx } from './extract-pptx.js';
import { extract as extractEml } from './extract-eml.js';
import { generateEmbedding as defaultEmbed } from '../helpers/embeddings.js';
import { gatedEmbedder } from '../helpers/vecMeta.js';
import { isReadPathAllowed, getAllowlist, loadAllowlist } from '../routes/paths.js';
import * as sqliteBackend from './backends/sqlite.js';
import * as pgBackend from './backends/postgres.js';
import logger, { logError } from '../helpers/logger.js';

export const SKIP_DIRS = new Set(['.git', 'node_modules', 'trash', 'var', 'coverage']);

const DOCX_MIME = 'application/vnd.openxmlformats-officedocument.wordprocessingml.document';
const XLSX_MIME = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';
const PPTX_MIME = 'application/vnd.openxmlformats-officedocument.presentationml.presentation';

// extension → { mime, extract }. The single source of truth for what docgraph
// indexes.
const EXTRACTORS = {
  md:       { mime: 'text/markdown', extract: extractMd },
  mdx:      { mime: 'text/markdown', extract: extractMd },
  markdown: { mime: 'text/markdown', extract: extractMd },
  rst:      { mime: 'text/x-rst',    extract: extractMd },
  txt:      { mime: 'text/plain',    extract: extractMd },
  text:     { mime: 'text/plain',    extract: extractMd },
  html:     { mime: 'text/html',     extract: extractHtml },
  htm:      { mime: 'text/html',     extract: extractHtml },
  pdf:      { mime: 'application/pdf', extract: extractPdf },
  docx:     { mime: DOCX_MIME,         extract: extractDocx },
  xlsx:     { mime: XLSX_MIME,         extract: extractXlsx },
  pptx:     { mime: PPTX_MIME,         extract: extractPptx },
  eml:      { mime: 'message/rfc822',  extract: extractEml },
};

// Extensions docgraph can index — used by the watcher to filter fs events.
export const INDEXABLE_EXTS = new Set(Object.keys(EXTRACTORS));

function pickExtractor(file) {
  const bare = path.extname(file).slice(1).toLowerCase();
  return EXTRACTORS[bare] ?? null;
}

export function pickBackend(store) {
  if (store?.pool) return { mod: pgBackend,     kind: 'postgres' };
  if (store?.db)   return { mod: sqliteBackend, kind: 'sqlite' };
  return null;
}

export function isDocgraphAvailable(store) {
  return pickBackend(store) !== null;
}

export async function* walk(dir, root = dir) {
  let entries;
  try {
    entries = await readdir(dir, { withFileTypes: true });
  } catch (err) {
    // The root is the user's requested corpus. Treating an unreadable root as
    // an empty iterator produces a convincing but false "0/0 docs" success.
    // Nested unreadable directories remain skippable so one protected child
    // does not prevent the rest of an otherwise readable corpus from indexing.
    if (dir === root) {
      err.message = `Cannot read document root ${root}: ${err.message}`;
      throw err;
    }
    logError(`[docgraph] walk skip ${dir}`, err);
    return;
  }
  for (const entry of entries) {
    if (entry.name.startsWith('.') && entry.name !== '.') continue;
    const full = path.join(dir, entry.name);
    if (entry.isDirectory()) {
      if (SKIP_DIRS.has(entry.name)) continue;
      yield* walk(full, root);
    } else if (entry.isFile()) {
      const picked = pickExtractor(full);
      if (!picked) continue;
      yield { abs: full, rel: path.relative(root, full), mime: picked.mime, extract: picked.extract };
    }
  }
}

// ── Public API (dispatcher) ──────────────────────────────────────────────────

// deferEmbedding=true skips inline embedding and returns the chunks as `pending`
// for the caller (the watcher) to drain through the async chunk-embedding queue.
// The CLI / one-shot callers leave it false so embeddings are written before exit.
export async function indexRepo(store, rootPath, { generateEmbedding = defaultEmbed, deferEmbedding = false, onProgress } = {}) {
  if (!isReadPathAllowed(rootPath)) {
    throw new Error(
      `Refusing to index ${rootPath} — not within an allowed folder ` +
      `(currently: ${getAllowlist().join(', ')}). Add the folder in Settings and retry.`
    );
  }
  const backend = pickBackend(store);
  if (!backend) throw new Error('docgraph requires the Postgres or SQLite backend.');

  // Inline embedding is an ordinary write and gates on docgraph's vec_meta
  // status (issue #340) — see the twin comment in codegraph/indexer.js for why
  // the embedder is wrapped per row rather than checked once up front.
  const counts = await backend.mod.indexRepoFiles(store, rootPath, walk(rootPath), {
    generateEmbedding: gatedEmbedder(store, 'docgraph', generateEmbedding),
    deferEmbedding, onProgress,
  });
  logger.info(
    `[docgraph/${backend.kind}] indexed ${rootPath}: ${counts.changed}/${counts.docs} docs, ` +
    `${counts.sections} sections, ${counts.chunks} chunks` + (counts.skipped ? ` · ${counts.skipped} skipped` : '')
  );
  return counts;
}

// Matches store.deleteByTagsAndSource's own default batch size — each page
// deleted here becomes exactly one (or a few, if larger) batched purge.
const RECONCILE_PAGE_SIZE = 500;

export async function deleteRepo(store, rootPath) {
  const backend = pickBackend(store);
  if (!backend) return { deleted: false };
  // Paged. deleteDocumentsPage and finalizeRepoDelete each purge their own
  // page's bridge memories in the SAME transaction as the document delete
  // (#360 review round 4, P1 & P2) — a genuine backend failure, in either
  // half, is deliberately NOT caught here: it propagates to the caller as a
  // real error, and because the delete and the purge share one transaction,
  // nothing partial is left behind for it to describe — either a page's
  // documents AND their memories are gone together, or neither is. This
  // also bounds peak memory to one page, not the whole corpus.
  for (;;) {
    const page = await backend.mod.deleteDocumentsPage(store, rootPath, RECONCILE_PAGE_SIZE);
    if (!page.length) break;
    if (page.length < RECONCILE_PAGE_SIZE) break;
  }
  // Finalize atomically rather than a plain repo-row delete (#360 review
  // round 2, P1): a concurrent index operation could add a new document in
  // the gap between the loop above ending and a separate repo-row delete,
  // which that delete's cascade would then remove without ever retiring its
  // bridge memory. finalizeRepoDelete folds one last (unbounded, but by now
  // small) documents-delete, its memory purge, AND the repo-row delete into
  // ONE transaction, so nothing can land in any gap unseen.
  const { deleted } = await backend.mod.finalizeRepoDelete(store, rootPath);
  return { deleted };
}

// Incremental, single-document operations used by the watcher. Embedding is
// NOT done inline here — indexOneFile returns the `pending` chunks and the
// watcher feeds them to the async chunk-embedding queue. Pass `embedInlineFn`
// in opts only when a caller needs embeddings written synchronously.
export async function indexFile(store, rootPath, relPath, opts = {}) {
  const backend = pickBackend(store);
  if (!backend) return { skipped: true, reason: 'no backend' };
  const picked = pickExtractor(relPath);
  if (!picked) return { skipped: true, reason: 'unsupported extension' };
  // A caller that opts into synchronous embedding gets it gated (#340) — see
  // the twin in codegraph/indexer.js.
  return backend.mod.indexOneFile(store, rootPath, relPath, {
    mime: picked.mime, extract: picked.extract, ...opts,
    ...(opts.embedInlineFn && { embedInlineFn: gatedEmbedder(store, 'docgraph', opts.embedInlineFn) }),
  });
}

// Used by the memory-bridge promotion path (docgraphHandlers.js) to
// revalidate a document is still indexed immediately before writing a
// bridge memory for it — see documentExists() in the backends (#360).
export async function documentExists(store, rootPath, relPath) {
  const backend = pickBackend(store);
  if (!backend) return false;
  return backend.mod.documentExists(store, rootPath, relPath);
}

export async function removeFile(store, rootPath, relPath) {
  const backend = pickBackend(store);
  if (!backend) return { removed: false };
  // Covers both a plain delete and the "unlink" half of a rename — chokidar
  // reports a rename as unlink(old) + add(new), so this fires for the old
  // path and indexFile()'s own promotion pass covers the new one, leaving
  // exactly one current bridge memory with no stale predecessor (#360).
  // removeOneFile purges that memory in the SAME transaction as the
  // document delete (#360 review round 4, P1 & P2), so a purge failure
  // rolls back the whole thing instead of leaving the document gone with
  // its memory silently stranded — a genuine failure here propagates as a
  // real error rather than being swallowed.
  return backend.mod.removeOneFile(store, rootPath, relPath);
}

export async function sweepMissing(store, rootPath) {
  const backend = pickBackend(store);
  if (!backend) return { removed: 0 };
  // sweepMissingFiles purges each removed path's bridge memory in the SAME
  // transaction as its document delete (#360 review round 4, P1 & P2) — see
  // removeFile's comment.
  const { removed } = await backend.mod.sweepMissingFiles(store, rootPath, stat);
  return { removed };
}

// Write a single chunk's embedding. Used by the async chunk-embedding queue
// to backfill vectors for chunks indexed without inline embedding.
export async function setChunkEmbedding(store, chunkId, embedding) {
  const backend = pickBackend(store);
  if (!backend) return;
  return backend.mod.setChunkEmbedding(store, chunkId, embedding);
}

// All of rootPath's own chunks still missing a vector, regardless of whether
// this process's watcher has ever touched them. Used at watcher startup to
// backfill anything a provider/model change wiped that a plain re-index of
// unchanged files would never revisit. Scoped to rootPath so multiple
// watched roots don't each re-enqueue every other root's pending chunks.
// `page` ({ limit, offset }) is how the reindex driver walks a large corpus in
// bounded pages rather than materializing every pending chunk's text at once.
export async function listPendingEmbeddings(store, rootPath, page) {
  const backend = pickBackend(store);
  if (!backend) return [];
  return backend.mod.listChunksWithoutEmbeddings(store, rootPath, page);
}

// Every indexed root, so the reindex driver can page through them one at a
// time (issue #287, WS1).
export async function listRepoRoots(store) {
  const backend = pickBackend(store);
  if (!backend) return [];
  return backend.mod.listRepoRoots(store);
}

// CLI entry: `node lib/docgraph/indexer.js <folder-path>`
import { fileURLToPath } from 'url';
const __filename = fileURLToPath(import.meta.url);
if (process.argv[1] === __filename) {
  const { config } = await import('dotenv');
  config();
  const { getStore } = await import('../../db/index.js');
  const store = await getStore();
  await loadAllowlist(store);
  if (!isDocgraphAvailable(store)) {
    console.error(
      `docgraph requires Postgres or SQLite, but the current store has neither handle.\n` +
      `Set DB_BACKEND=sqlite (zero-config) or DB_BACKEND=postgres and re-run.`
    );
    process.exit(2);
  }
  const root = path.resolve(process.argv[2] ?? '.');
  let exitCode = 0;
  try {
    const counts = await indexRepo(store, root);
    console.log(JSON.stringify(counts, null, 2));
  } catch (err) {
    logError(`[docgraph] CLI indexRepo failed`, err, { root });
    console.error(`\nFAILED: ${err.message}\nSee var/logs/error-*.log for the full stack trace.`);
    exitCode = 1;
  } finally {
    await store.close?.().catch(() => {});
    process.exit(exitCode);
  }
}
