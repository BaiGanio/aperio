...
// All chunks still missing a vector, regardless of whether this process's
// watcher has ever touched them. Used at watcher startup to backfill
// anything a provider/model change wiped that a plain re-index of unchanged
// files would never revisit.
export async function listPendingEmbeddings(store) {
  const backend = pickBackend(store);
  if (!backend) return [];
  return backend.mod.listChunksWithoutEmbeddings(store);
}
