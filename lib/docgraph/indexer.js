...
export const SKIP_DIRS = new Set(['.git', 'node_modules', 'trash', 'var']);
...
// Incremental, single-document operations used by the watcher.
export async function indexFile(store, rootPath, relPath, { generateEmbedding = defaultEmbed } = {}) {
  const backend = pickBackend(store);
  ...
  return backend.mod.indexOneFile(store, rootPath, relPath, { mime: picked.mime, extract: picked.extract, generateEmbedding });
}
