...
// Pure helpers for the memory-compaction EPIC's WS0 baseline (#286, closed —
// see CHANGELOG.md). Kept as standalone recall/token measurement tooling:
// `npm run memory:baseline` measures self-preload/recall-payload token cost
// and hit-rate@k, independent of the (rejected) compaction rewriter this
// EPIC originally built it to gate. Extracted out of
// scripts/memory-compact-baseline.js so they're importable and unit-testable
// without triggering the script's DB seeding/CLI guard — mirrors the
// scripts/model-tier-bench.js + lib/helpers/modelTierBench.js split already
// used elsewhere in this repo (thin CLI entry point, testable logic in lib/).
...
// Token-counts the real recall() formatter's output for every corpus query
// (mode="auto", limit=10) — the actual payload a provider would receive.
export async function measureRecallPayloadTokens(store, queries, generateEmbedding) {
  const tokenCounts = [];
...
// Hit-rate@k for a single search_mode ("semantic" or "fulltext"), scored
// independently per the issue's acceptance criteria — never blended.
export async function measureHitRate(store, queries, mode, generateEmbedding, k = 3) {
  let hits = 0;
  for (const q of queries) {
