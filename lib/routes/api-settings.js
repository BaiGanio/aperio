...
const KNOWN_SETTING_KEYS = new Set([
  ...
  "aperio-font-scale",
  "aperio-tts",
  "aperio-voice-continuous",
  "aperio-reasoning",
  "aperio-busy-words",
  "aperio-ambient",
  ...CONFIG.map((e) => configSettingKey(e.key)),
]);
// Unmanaged .env vars adopted by the config panel (Phase 2b) live under
// config.<KEY> with no registry entry, so they can't be enumerated in the
// allowlist. Accept the namespace for env-var-shaped names only; secret-looking
// ones stay masked on GET via inferType below.
const UNMANAGED_CONFIG_KEY = /^config\.[A-Z][A-Z0-9_]*$/;
const isKnownSettingKey = (k) => KNOWN_SETTING_KEYS.has(k) || UNMANAGED_CONFIG_KEY.test(k);
...
export function mountSettingsRoutes(router, { store }) {
  ...
  router.put("/settings/:key", express.json({ limit: "64kb" }), async (req, res) => {
    try {
      if (!isKnownSettingKey(req.params.key)) {
