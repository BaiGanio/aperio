 ...
 * @param {object} opts.watchdog - Idle watchdog from createWatchdog()
 ...
 */
export function apiRouter({ agent, store, watchdog, scheduler, getAgent, getWatchdog, getScheduler, version, watcherEvents, watcherRegistry, generateEmbedding, folderIndexer }) {
  const router = Router();
  ...
  const meta = mountMetaRoutes(router, { getAgent: resolveAgent, store, getWatchdog: resolveWatchdog, version: ver });
  mountAgentRoutes(router,     { store, getAgent: resolveAgent, getScheduler: resolveScheduler });
  mountMemoryRoutes(router,    { store });
  mountWikiRoutes(router,      { store });
  mountCodegraphRoutes(router, { store, watcherEvents, watcherRegistry, folderIndexer });
  mountDocgraphRoutes(router,  { store, watcherEvents, watcherRegistry, folderIndexer });
  ...
  mountInterruptRoutes(router,    { store });
  mountDatasetRoutes(router,       { generateEmbedding });

  // Background work owned by the mounted routes (currently the metrics
  // sampler). Callers that create a router must release it — see
  // createGracefulShutdown({ apiRoutes }) — otherwise a re-mounted router
  // leaves the previous sampler running and holding its store.
  router.dispose = () => { meta?.stop?.(); };
