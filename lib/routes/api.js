...
export function apiRouter({ agent, store, watchdog, scheduler }) {
  ...
  // ── Background agents (see background-agents.md) ──────────────────────────
  // Phase 4: job defs live in the DB (agent_jobs) and every run is recorded
  // (agent_runs). Definition CRUD is always available so jobs can be configured
  // before auto-run is enabled; only running a job (and interval auto-run) is
  // gated by APERIO_AGENT_JOBS=on.
  const isAgentJobShaped = (j) =>
    (Array.isArray(j?.steps) && j.steps.length) ||
    (typeof j?.prompt === "string" && j.prompt.trim());

  // List all jobs, each with its most recent run (for the status panel).
  router.get("/agents", async (_req, res) => {
    try {
      const jobs = await store.listAgentJobs();
      const withRuns = await Promise.all(jobs.map(async (job) => {
        const [lastRun] = await store.listAgentRuns(job.id, 1);
        return { ...job, lastRun: lastRun ?? null };
      }));
      res.json({ enabled: process.env.APERIO_AGENT_JOBS === "on", jobs: withRuns });
    } catch (err) {
  ...
  router.get("/agents/:id/runs", async (req, res) => {
    try {
      ...
      res.json({ runs: await store.listAgentRuns(req.params.id, limit) });
    } catch (err) {
      logger.error("GET /api/agents/:id/runs error:", err);
      res.status(500).json({ error: err.message });
    }
  });

  router.post("/agents", express.json({ limit: "16kb" }), async (req, res) => {
    const job = req.body ?? {};
    if (!job.id || typeof job.id !== "string") {
      return res.status(400).json({ error: "job id is required" });
    }
    if (!isAgentJobShaped(job)) {
      return res.status(400).json({ error: "job needs a non-empty steps[] or prompt" });
    }
    try {
      if (await store.getAgentJob(job.id)) {
        return res.status(409).json({ error: `job "${job.id}" already exists` });
      }
      res.status(201).json(await store.upsertAgentJob(job));
    } catch (err) {
  ...
  router.put("/agents/:id", express.json({ limit: "16kb" }), async (req, res) => {
    ...
    try {
      ...
      res.json(await store.upsertAgentJob(job));
    } catch (err) {
      logger.error("PUT /api/agents/:id error:", err);
      res.status(500).json({ error: err.message });
    }
  });
  ...
  router.delete("/agents/:id", async (req, res) => {
    try {
      const ok = await store.deleteAgentJob(req.params.id);
      if (!ok) return res.status(404).json({ error: `no job with id "${req.params.id}"` });
      res.json({ ok: true });
    } catch (err) {
  ...
  // Run-now: trigger a defined job immediately. Gated by APERIO_AGENT_JOBS=on —
  // the same master switch as interval auto-run.
