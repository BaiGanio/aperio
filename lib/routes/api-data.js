        ...
        setImmediate(async () => {
          try {
            ...
            if (pending.length > 0) {
              ...
              const { embedForStore } = await import("../helpers/vecMeta.js");
              let embedded = 0;
              for (const row of pending) {
                // Gated per row (issue #340): while the memories store is
                // stale/reindexing, these rows belong to the reindex driver —
                // it scans for exactly this "no vector yet" state, so leaving
                // them bare loses nothing and writing ours would mix spaces.
                const { embedding, deferred } = await embedForStore(store, "memories",
                  () => generateEmbedding(`${row.title}. ${row.content}`));
                if (deferred) {
                  logger.info(`[data-import] backfill deferred to the reindex driver after ${embedded}/${pending.length}`);
                  break;
                }
                if (embedding) { await store.setEmbedding(row.id, embedding); embedded++; }
              }
              logger.info(`[data-import] backfill complete: ${embedded} embeddings`);
            }
          } catch (err) {
