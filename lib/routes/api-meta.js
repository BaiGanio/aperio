...
export function mountMetaRoutes(router, opts = {}) {
  ...
  // ── Identity (whoami.md) ────────────────────────────────────────────────────
  // Backs the startup banner's "more..." modal: view + edit the raw identity
  // file. Path is fixed server-side, never derived from request input, so this
  // doesn't need to route through paths.js. Reads prefer the writable overlay
  // var/id/whoami.md and fall back to the bundled id/whoami.md; saves always
  // write the overlay, so this works on a read-only root filesystem.
  router.get("/identity/whoami", (_, res) => {
    res.json({ content: agent.getWhoamiContent() });
  });

  const identityBody = express.json({ limit: "256kb" });
  router.put("/identity/whoami", identityBody, (req, res) => {
    try {
      const content = String(req.body?.content ?? "");
      agent.saveWhoamiContent(content);
      res.json({ ok: true });
    } catch (err) {
      res.status(400).json({ error: err.message });
    }
  ...
  // Read-only tool catalog for the same modal's "Tool schemas" section.
  router.get("/tools", (_, res) => {
    res.json({ tools: (agent.mcpTools || []).map(t => ({ name: t.name, description: t.description || "" })) });
  });
