...
export function mountMetaRoutes(router, opts = {}) {
  ...
  let _coreDetail = { perf: null, eff: null };
  if (os.platform() === "darwin") {
    execAsync("sysctl -n hw.perflevel0.logicalcpu hw.perflevel1.logicalcpu")
      .then(({ stdout }) => {
        const [perf, eff] = stdout.trim().split(/\s+/).map(Number);
        if (perf > 0 && eff > 0) _coreDetail = { perf, eff };
      })
      .catch(() => {});
  }
  ...
  setInterval(async () => {
    ...
    _cachedMetrics = {
      ...
      perfCores:      _coreDetail.perf,
      effCores:       _coreDetail.eff,
      ...
      systemUsedMem:  await systemUsedMemMB(),
  ...
  // Quit — the "Quit Aperio" button. Shuts the server (and Ollama, if safe)
  // down immediately instead of waiting for the idle timeout after the tab closes.
  router.post("/quit", (_req, res) => {
    res.json({ ok: true });                 // answer before we tear down
