...
import { decideGithubInterrupt } from "../../mcp/tools/github.js";
import { decideTemplateProposal } from "../handlers/extraction/templateHandlers.js";
...
export async function decideAndMaybeExecute({ store, id, body }) {
  ...
  if (decision === "approve" || decision === "edit") {
    ...
    let handled;
    if (current.tool_name === "db_execute") {
      handled = await decideDatabaseInterrupt({ store }, id, input);
    } else if (current.tool_name === "create_github_issue" || current.tool_name === "update_github_issue") {
      const result = await decideGithubInterrupt({ store }, id);
      handled = { row: await store.getAgentInterrupt(id), result };
    } else if (current.tool_name === "extraction_template_propose") {
      // decideTemplateProposal (lib/handlers/extraction/templateHandlers.js)
      // returns the raw saved row for reuse by the MCP tool's own confirm
      // branch — wrap it as an MCP-content result here for this generic
      // confirm-button consumer, same as every other branch above.
      const { row, template, error } = await decideTemplateProposal({ store }, id, input);
      handled = {
        row,
        result: { content: [{ type: "text", text: error ? `❌ ${error}` : `✅ Saved template "${template?.name}" (id: ${template?.id}).` }] },
      };
    } else {
      handled = await decideFileInterrupt({ store }, id, input);
    }
    return {
      status: 200,
      body: {
