 ...
 *
 * There is ONE allowed-folders list. It is app-wide, DB-persisted
 * (settings['allowed-paths']), and serves as both the read ceiling and the
 * write ceiling: the model may read AND write anywhere under it. Aperio does
 * not implement a read-only tier — isReadPathAllowed() and isWritePathAllowed()
 * consult the same list, and getActivePaths() returns the same array for both.
 *
 * APERIO_ALLOWED_PATHS only SEEDS this list on first run (when the setting is
 * absent); afterward the DB is authoritative and the list is edited via the UI
 * (set_paths → setAllowlist). APERIO_ALLOWED_PATHS_TO_READ and
 * APERIO_ALLOWED_PATHS_TO_WRITE are deprecated aliases: because there is only
 * one list, they are merged into the same seed rather than honoured as two
 * separate tiers. A hard FLOOR (project cwd + scratch root) is always merged in
 * so the workspace can never be excluded.
 *
...
// Env-configured seed, used only to populate the DB allowlist on first run.
// APERIO_ALLOWED_PATHS is the current name; the two _TO_READ / _TO_WRITE names
// are deprecated aliases still read here so existing .env files keep working.
// All three are merged into one seed — there is no separate read tier to
// honour — then deduped, with children of an already-listed parent removed.
const SEED_ENV = [
  process.env.APERIO_ALLOWED_PATHS,
  process.env.APERIO_ALLOWED_PATHS_TO_READ,
  process.env.APERIO_ALLOWED_PATHS_TO_WRITE,
].filter(v => v && v.trim());

export const DEFAULT_PATHS = SEED_ENV.length
  ? removeRedundantChildren([...new Set(SEED_ENV.flatMap(normalizePaths))])
  : normalizePaths(null);
...
/** The one app-wide allowed-folders list (read and write alike), including the floor. */
