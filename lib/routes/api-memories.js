...
export function mountMemoryRoutes(router, { store }) {
      ...
      setImmediate(async () => {
        try {
          ...
          for (const row of pending) {
            // Gated per row (issue #340) — see the identical loop in
            // api-data.js: a reindex owns these rows while it is running.
            const { embedding, deferred } = await embedForStore(store, "memories",
              () => generateEmbedding(`${row.title}. ${row.content}`));
            if (deferred) {
              logger.info(`[import] backfill deferred to the reindex driver after ${embedded}/${pending.length}`);
              break;
            }
            if (embedding) { await store.setEmbedding(row.id, embedding); embedded++; }
          }
          logger.info(`[import] backfill complete: ${embedded} embeddings generated`);
        } catch (err) {
  ...
  // ── Self-memories (oversight: read + delete only) ───────────────────────────
  // The agent's own walled-off store. Deliberately NOT in the generic DB browser
  // whitelist (that's the user's data surface) — it gets this dedicated audit
  // route instead, so a user can see and purge what an agent has kept. Read +
  // delete only: the UI never writes to the self store (only the agent does).
  router.get("/self-memories", async (_req, res) => {
    try {
      res.json({ raw: await store.listSelf(200) });
    } catch (err) {
      logger.error("GET /api/self-memories error:", err);
      if (!res.headersSent) res.status(500).json({ error: err.message });
    }
  });

  router.delete("/self-memories/:id", async (req, res) => {
    try {
      const title = await store.deleteSelf(req.params.id);
      if (!title) return res.status(404).json({ error: "Self-memory not found" });
      res.json({ ok: true, title });
    } catch (err) {
      logger.error("DELETE /api/self-memories/:id error:", err);
      if (!res.headersSent) res.status(500).json({ error: err.message });
    }
  });
  ...
  // ── Memory inbox (pending review queue) ──────────────────────────────────────
  router.get("/memories/pending", async (_req, res) => {
    try {
      const pending = await store.listPending();
      res.json({ pending });
    } catch (err) {
      logger.error("GET /api/memories/pending error:", err);
      res.status(500).json({ error: err.message });
    }
  });

  router.get("/memories/pending/count", async (_req, res) => {
    try {
      const count = await store.countPending();
      res.json({ count });
    } catch (err) {
      logger.error("GET /api/memories/pending/count error:", err);
      res.status(500).json({ error: err.message });
    }
  });

  router.post("/memories/pending/:id/approve", async (req, res) => {
    try {
      const result = await store.approvePending(req.params.id);
      res.json({ ok: true, memory: result });
    } catch (err) {
      logger.error("POST /api/memories/pending/:id/approve error:", err);
      res.status(404).json({ error: err.message });
    }
  });

  router.post("/memories/pending/:id/reject", async (req, res) => {
    try {
      const result = await store.rejectPending(req.params.id);
      res.json({ ok: true, ...result });
    } catch (err) {
      logger.error("POST /api/memories/pending/:id/reject error:", err);
      res.status(404).json({ error: err.message });
    }
  });
}
