...
import { persistEnvVar } from "../helpers/envFile.js";
...
export function mountAgentRoutes(router, { store, scheduler }) {
  ...
  // stop interval/watcher auto-run, and persist to .env so it survives a restart.
  router.put("/agents/enabled", async (req, res) => {
    ...
    if (process.env.NODE_ENV !== "test") {
      try {
        persistEnvVar("APERIO_AGENT_JOBS", on ? "on" : "off");  // survive restart
      } catch (err) {
