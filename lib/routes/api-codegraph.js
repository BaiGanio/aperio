...
export function mountCodegraphRoutes(router, { store, watcherEvents, watcherRegistry } = {}) {
  ...
  router.post("/codegraph/index", indexLimiter, express.json(), async (req, res) => {
    ...
    try {
      ...
      const { startWatcher } = await import("../codegraph/watcher.js");
      const { addRoot, markAllDone } = await import("../codegraph/status.js");
      addRoot(abs);
      ...
      (async () => {
        try {
          ...
          await registry.register('codegraph', abs, handle);
        } catch (err) {
          logError(`[codegraph] user-triggered watch failed`, err, { abs });
