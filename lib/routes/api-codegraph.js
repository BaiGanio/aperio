...
import { getAllowlist, setAllowlist } from "./paths.js";
import {
  ...
  neighborsHandler  as cgNeighbors,
  pathHandler       as cgPath,
  insightsHandler   as cgInsights,
  graphHandler      as cgGraph,
  deleteRepoHandler as cgDeleteRepo,
} from "../handlers/codegraph/codegraphHandlers.js";

// A query param that may repeat (?kinds=calls&kinds=imports) or arrive as a
// single value — normalize to an array, or undefined when absent.
function asArray(v) {
...
export function mountCodegraphRoutes(router, { store, watcherEvents, watcherRegistry, folderIndexer } = {}) {
  ...
  function cgRoute(fn) {
    return async (req, res) => {
      ...
      try { res.json({ enabled: true, ...await fn(req) }); }
        ...
        logError(`${req.method} ${req.path} failed`, err, { query: req.query });
  ...
  router.get("/codegraph/neighbors", cgRoute(async (r) => unwrap(await cgNeighbors(cgCtx, {
    qualified: r.query.qualified,
    repo:      r.query.repo,
    direction: r.query.direction,
    kinds:     asArray(r.query.kinds),
    depth:     r.query.depth ? parseInt(r.query.depth, 10) : undefined,
    limit:     r.query.limit ? parseInt(r.query.limit, 10) : undefined,
  }))));
  router.get("/codegraph/path", cgRoute(async (r) => unwrap(await cgPath(cgCtx, {
    from:      r.query.from,
    to:        r.query.to,
    repo:      r.query.repo,
    directed:  r.query.directed === "true" || r.query.directed === "1",
    kinds:     asArray(r.query.kinds),
    max_depth: r.query.max_depth ? parseInt(r.query.max_depth, 10) : undefined,
  }))));
  router.get("/codegraph/insights", cgRoute(async (r) => unwrap(await cgInsights(cgCtx, {
    repo:  r.query.repo,
    view:  r.query.view,
    limit: r.query.limit ? parseInt(r.query.limit, 10) : undefined,
  }))));
  router.get("/codegraph/graph", cgRoute(async (r) => unwrap(await cgGraph(cgCtx, {
    repo:      r.query.repo,
    limit:     r.query.limit ? parseInt(r.query.limit, 10) : undefined,
    focus:     asArray(r.query.focus),
    community: r.query.community || undefined,
  }))));
}
