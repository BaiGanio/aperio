...
const STREAM_IDLE_TIMEOUT_MS = parseInt(process.env.LLAMACPP_STREAM_IDLE_TIMEOUT_MS || "120000", 10);
const STREAM_IDLE_TIMEOUT = Symbol("stream-idle-timeout");

async function readWithIdleTimeout(reader, idleMs) {
  let timer;
  const idle = new Promise(resolve => { timer = setTimeout(() => resolve(STREAM_IDLE_TIMEOUT), idleMs); });
  try {
...
export class LlamaCppStreamHandler {
  ...
  async process() {
    ...
    while (true) {
      const result = await readWithIdleTimeout(reader, STREAM_IDLE_TIMEOUT_MS);
      if (result === STREAM_IDLE_TIMEOUT) {
      ...
      if (done) break;
      if (this.processChunk(decoder.decode(value, { stream: true }))) break;
    }
