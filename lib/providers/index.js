...
// two happen to coincide today because both providers build their prompt
// from a single text-only string (subprocess CLI / SDK system-prompt), but
// billing model and multimodal capability are unrelated properties. Keep
// them separately named so a future provider that is subscription-billed
// but does support images (or vice versa) doesn't silently inherit the
// wrong behavior from this set.
const IMAGE_DROPPING_PROVIDERS = new Set(["claude-code", "codex"]);
