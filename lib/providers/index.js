...
export function resolveProvider(overrides = {}) {
  ...
  // DeepSeek is a provider-level blind: no DeepSeek model accepts native
  // image_url content today (model-capabilities.js's PROVIDER_VISION map),
  // so every DeepSeek turn with an image goes through the local VLM bridge
  // (ensureVisionEngine start it on demand — see deepseek.js).
  if (PROVIDER === "deepseek") { const model = overrides.model || process.env.DEEPSEEK_MODEL; return { name: "deepseek", model, baseURL: "https://api.deepseek.com/v1", apiKey: process.env.DEEPSEEK_API_KEY, vision: false, contextWindow: 128000 }; }
