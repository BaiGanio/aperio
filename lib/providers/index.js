...
export function getRecommendedModel(profile = resolvePerfProfile(), hardware = {}) {
  const gb = hardware.totalRamGB ?? os.totalmem() / 1024 ** 3;
  if (profile === "fast-low-vram") {
    if (gb >= 20) return "qwen3:30b-a3b";
    if (gb >= 8)  return "gemma4:e4b";
    return "qwen2.5:3b";
  }
  if (profile === "quality") {
    if (gb >= 32) return "qwen3:30b-a3b";
    if (gb >= 16) return "gemma4:12b";
    if (gb >= 6)  return "gemma4:e4b";
    return "qwen2.5:3b";
  }
...
//     mapping). Ollama pulls the same underlying GGUFs, so kv facts are unchanged.
...
export const MODEL_FACTS = {
  ...
  "qwen3.5:9b":    { sizeGB: 6.6, maxContext: 262144, kvBytesPerToken: 524288,
    hf: "unsloth/Qwen3.5-9B-GGUF", architecture: "dense" },
  "Qwen3.5:9b":    { sizeGB: 6.6, maxContext: 262144, kvBytesPerToken: 524288,
    hf: "unsloth/Qwen3.5-9B-GGUF", architecture: "dense" },
  "qwen3.5:4b":    { sizeGB: 3.4, maxContext: 262144, kvBytesPerToken: 524288,
    hf: "unsloth/Qwen3.5-4B-GGUF", architecture: "dense" },
  "Qwen3.5:4b":    { sizeGB: 3.4, maxContext: 262144, kvBytesPerToken: 524288,
    hf: "unsloth/Qwen3.5-4B-GGUF", architecture: "dense" },
  // Vision bridge model — not RAM-tiered by getRecommendedModel() (a fixed
  // curated default, same role OLLAMA_VLM_MODEL/LLAMACPP_VLM_MODEL play), but
  // it needs the same facts shape so the wizard/sizer can treat it uniformly.
...
// not the Ollama-tag keys MODEL_FACTS is keyed by). Returns null for a custom
// model the user pointed at directly — callers fall back to generic facts.
export function factsForHf(hfRepo) {
  return Object.values(MODEL_FACTS).find(f => f.hf === hfRepo) ?? null;
}
...
export function recommendServeContextLength(env = process.env) {
  ...
  const facts = MODEL_FACTS[model] ?? { sizeGB: 8, maxContext: 131072 };
...
// The app talks to Ollama/llama.cpp over their OpenAI-compatible /v1 endpoint,
// which has no way to set the server's context window per request — so the
// "assumed" var (OLLAMA_NUM_CTX / LLAMACPP_CTX) is only the app's internal
// assumption for trim/cap math, NOT a window pushed onto the server. If it
// exceeds the server's real serving window, capToolResults over-keeps and the
// prompt is silently truncated server-side, leaving the model to answer blind.
// Clamp to the server's own real-window var (OLLAMA_CONTEXT_LENGTH /
// LLAMACPP_SERVE_CTX) whenever it's visible to the app, so we never assume more
// context than the server actually provides. Setting that var on the server
// (the real fix) then aligns both sides automatically.
//
// genericCtxStatus/genericContextWindow hold the shared rule; ollamaCtxStatus/
// ollamaContextWindow and llamacppCtxStatus/llamacppContextWindow are thin,
// named wrappers so callers keep reading a provider-specific function. `real`
// of 0 means the server window is unknown → no clamp.
...
const OLLAMA_CTX_KEYS = { assumedKey: "OLLAMA_NUM_CTX", realKey: "OLLAMA_CONTEXT_LENGTH" };
...
export function ollamaCtxStatus(env = process.env) {
  return genericCtxStatus(OLLAMA_CTX_KEYS, env);
}

export function ollamaContextWindow(env = process.env) {
  return genericContextWindow(OLLAMA_CTX_KEYS, env);
}
...
export function resolveProvider(overrides = {}) {
  ...
  if (PROVIDER === "llamacpp") {
    ...
    return { name: "llamacpp", model: llamacppModel, baseURL: `${LLAMACPP_BASE_URL}/v1`, llamacppBaseURL: LLAMACPP_BASE_URL, ollamaBaseURL: null, contextWindow: llamacppContextWindow() };
  }
...
/**
 * True only for the fully-local providers (Ollama, llama.cpp). Cloud providers
 ...
 * ad-hoc `provider === "ollama"` checks; flipping a provider's membership here
