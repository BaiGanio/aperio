...
// startLlamaCpp.js, which delegates to this. Guards against an empty tier value
// with the curated small model so this never returns "".
export function defaultLocalModel(profile = resolvePerfProfile(), hardware = {}, env = process.env) {
  return getRecommendedModel(profile, hardware, env) || MODEL_TIER_DEFAULTS.LLAMACPP_MODEL_TIER_8;
}
...
//     conservative default badly under-estimates some dense-cache models; exact
//     values for shipped defaults are retained here, while arbitrary cached
//     models are derived from their GGUF metadata.
...
// Parse APERIO_MODEL_FACTS_OVERRIDES env var — a JSON object keyed by HF repo
// path (with optional :quant suffix), whose values override the curated dict
// and generic fallback for uncached or custom models. Parsed once and cached.
let _parsedOverrides = null;
function parseModelFactsOverrides(env) {
  if (_parsedOverrides) return _parsedOverrides;
  const raw = env.APERIO_MODEL_FACTS_OVERRIDES;
  if (!raw) { _parsedOverrides = {}; return _parsedOverrides; }
  try {
    _parsedOverrides = JSON.parse(raw);
    if (typeof _parsedOverrides !== "object" || Array.isArray(_parsedOverrides)) throw new Error();
  } catch {
...
// pre-download catalog, env-driven overrides, then conservative generic sizing.
// The cache is keyed by both model and cache root because tests and
// multi-instance deployments can point at different Hugging Face stores.
export function resolveModelFacts(model, env = process.env) {
  ...
  const envOverride = repo ? overrides[repo] ?? overrides[model] : null;
  const facts = inspected ?? envOverride ?? factsForHf(model) ?? MODEL_FACTS[model] ?? GENERIC_MODEL_FACTS;
