...
// a non-technical user never has to guess. Tags below are MODEL_FACTS keys
// mapped to real llama.cpp GGUF downloads — a wrong tag means a failed
// download, which is exactly the frustration we avoid.
...
// kvBytesPerToken matches a modern 9B-class dense model (Qwen3.5/Ornith) rather
// than recommendContextLength's optimistic 144 KB default: an unknown model is
...
// "23% of capacity" for a window the sizer itself considered 82% full.
...
// The served window (LLAMACPP_SERVE_CTX — what actually occupies the KV cache
...
// number. Callers gate on isLocalProvider(provider.name) and pass the live
...
export function machineCapacityPct(model, env = process.env) {
  const facts = factsForHf(model) ?? MODEL_FACTS[model] ?? GENERIC_MODEL_FACTS;
  const fit = ramFitTokens({ weightsGB: facts.sizeGB, bytesPerToken: facts.kvBytesPerToken });
...
export function resolveProvider(overrides = {}) {
  ...
  if (PROVIDER === "llamacpp") {
    ...
    return { name: "llamacpp", model: llamacppModel, baseURL: `${LLAMACPP_BASE_URL}/v1`, llamacppBaseURL: LLAMACPP_BASE_URL, contextWindow: llamacppContextWindow() };
  }
