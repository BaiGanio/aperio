 ...
 *     → { type: "number" }
 */
// Only Gemini's protobuf Schema is this strict (OpenAI-shaped consumers —
// DeepSeek, llama.cpp — accept standard JSON Schema and lose real constraint
// info, e.g. `minimum`, if it's stripped for them too), so callers opt in
// with `forGemini` rather than this applying globally.
function normalizeSchema(schema, forGemini = false) {
  ...
  if (schema.properties && typeof schema.properties === "object") {
    ...
    for (const [k, v] of Object.entries(schema.properties)) {
      result.properties[k] = normalizeSchema(v, forGemini);
    }
  }

  if (schema.items && typeof schema.items === "object") {
    result = { ...result, items: normalizeSchema(schema.items, forGemini) };
  }

  // anyOf entries that weren't handled above still need to be walked
  if (Array.isArray(schema.anyOf)) {
    result = { ...result, anyOf: schema.anyOf.map(e => normalizeSchema(e, forGemini)) };
  }

  if (Array.isArray(schema.oneOf)) {
    result = { ...result, oneOf: schema.oneOf.map(e => normalizeSchema(e, forGemini)) };
  }

  // Gemini's protobuf Schema (function-calling surface) only understands a
  // fixed key set — https://ai.google.dev/gemini-api/docs/function-calling
  // lists type/format/description/nullable/enum/properties/required/items;
  // anyOf/oneOf are kept because Aperio already relies on them for union
  // schemas. Any other JSON Schema keyword a tool's raw schema happens to
  // carry (additionalProperties, exclusiveMinimum, minLength, pattern,
  // default, ...) produces a hard 400 ("Unknown name ... Cannot find
  // field"). Allowlisting once here (Gemini callers only) avoids chasing
  // each new keyword as the next tool schema trips over it.
  if (forGemini) {
    result = Object.fromEntries(
      Object.entries(result).filter(([k]) => GEMINI_SCHEMA_KEYS.has(k)),
    );
  }
...
const GEMINI_SCHEMA_KEYS = new Set([
  ...
  "properties", "required", "items", "anyOf", "oneOf",
]);

export function zodToJsonSchema(zs, { forGemini = false } = {}) {
  if (!zs?._def) {
    const { $schema, ...rest } = zs ?? { type: "object", properties: {}, required: [] };
    return normalizeSchema(rest, forGemini);
  }
  ...
  return normalizeSchema({ type: "object", properties, required }, forGemini);
}
