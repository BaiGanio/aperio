...
import { machineCapacityPct, isLocalProvider, isSubscriptionProvider } from "../../providers/index.js";
...
/**
 * The four rates the browser needs to price a turn, or `null` when the model is
 * not on the price sheet. Written once and used by BOTH announces — the connect
 * one and the mid-session provider switch — because a switch that announced a
 * different shape from the connect would leave the UI pricing the new provider's
 * tokens with whatever it happened to keep from the old one.
 *
 * `cacheRead`/`cacheWrite` are per-model and nullable: a provider that charges no
 * separate cache rate publishes none. On a long Anthropic conversation nearly all
 * input is a cache read billed at a tenth of the input rate, so sending only
 * `in`/`out` overstated the running estimate several times over.
 */
function costRatesFor(model) {
  const p = getPricing(model);
  return p ? { in: p.in, out: p.out, cacheRead: p.cacheRead, cacheWrite: p.cacheWrite } : null;
}
 ...
 *
 * Usage in server.js:
 *   wss.on("connection", makeWsHandler({ agent, roundtable, store, varRoot }));
 *
 * @param {object} opts
 * @param {object}      opts.agent              - Main chat agent (single-turn chat).
 * @param {object|null} [opts.roundtable]        - Lazy round-table agent manager.
 ...
 */
export function makeWsHandler({ agent, roundtable = null, store, varRoot = process.cwd(), isShuttingDown = () => false }) {
  ...
  return function onConnection(ws) {
    ...
    send("provider", {
      ...
      costRates:     costRatesFor(provider().model),
      ...
      roundtableAvailable: roundtable?.roundtableAvailable === true,
      roundtableReason: roundtable?.roundtableAvailable ? null : roundtable?.roundtableUnavailableReason,
      agents: roundtable?.roundtableAvailable
        ? roundtable.agentDescriptors
      ...
      await runWithPaths(getAllowlist(), getAllowlist(), scratchDir, async () => {
        ...
        try {
          ...
          switch (data.type) {
            ...
            case "handoff":
              await handleHandoff(data.focus, { messages, sessionId, currentLang, runAgentLoop, emitter, sessionLogger, send, firstMessageSynthetic });
              return;
    ...
    async function handleChat(data, turn = null) {
      ...
      // ── Round-table mode ───────────────────────────────────────────────────
      // Opt-in per turn via `roundtable: true`. The configured pair is created
      // lazily on the first Discuss request and reused for later requests.
      //
      ...
      if (data.roundtable === true && roundtable?.roundtableAvailable) {
        try {
          const { primaryRoundtable, verifier } = await roundtable.getAgents();
