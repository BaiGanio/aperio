...
export function makeWsHandler({ agent, primaryRoundtable = null, verifier = null, roundtableAvailable = false, roundtableUnavailableReason = null, store, varRoot = process.cwd(), isShuttingDown = () => false }) {
  ...
  return function onConnection(ws) {
    ...
    // Separate from abortController: the prompt-cache warm-up (WS2 of
    // trash/plans/prompt-cache-hygiene) is a background, non-user-visible
    // request. It must be abortable the instant a real chat message arrives
    ...
    async function init() {
      ...
      // cost (prompt-cache hygiene, trash/plans/prompt-cache-hygiene WS2). It is
      // deliberately NOT seeded into `messages` — leaving history empty means the
      // first real user turn is the first message the model sees. (memCtx is
      // still persisted on the agent by buildGreeting, so the preview is carried.)
    ...
    // A confirmation button (GitHub issue mutation, file write/delete, database
    // write, or folder authorization) was clicked. The action is fully resolved
