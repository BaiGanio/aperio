...
import { machineCapacityPct } from "../../providers/index.js";
 ...
 */
export function makeWsHandler({ agent, primaryRoundtable = null, verifier = null, roundtableAvailable = false, roundtableUnavailableReason = null, store, __dirname }) {
  ...
  return function onConnection(ws) {
    ...
    send("provider", {
      ...
      contextCapacityPct: provider().name === "ollama" ? machineCapacityPct(provider().model) : null,
    ...
    async function init() {
      ...
      // greeting turn on the ollama path (harmless/ignored elsewhere). Anthropic
