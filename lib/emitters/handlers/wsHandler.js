...
export function makeWsHandler({ agent, primaryRoundtable = null, verifier = null, roundtableAvailable = false, store, __dirname }) {
  ...
  return function onConnection(ws) {
    ...
    async function init() {
      ...
      // showing one opaque number. The real total still comes from the provider's
      // reported input_tokens on the greeting turn.
      try { send("startup_breakdown", getStartupBreakdown()); } catch { /* non-fatal */ }
