 ...
 *
 * @param {object} opts
 * @param {object}      opts.agent              - Primary agent (the answerer in round-table).
 * @param {object|null} [opts.verifier]         - Verifier agent if round-table is enabled, else null.
 ...
 */
export function makeWsHandler({ agent, verifier = null, roundtableAvailable = false, store, __dirname }) {
  ...
  return function onConnection(ws) {
    ...
    send("provider", {
      ...
      agents: roundtableAvailable
        ? [
            { id: "primary",  persona: agent.persona ?? "primary",  name: provider.name,         model: provider.model },
    ...
    ws.on("close", () => {
      ...
      try { finaliseSession(sessionId, messages, msgAttachments); } catch (err) {
    ...
    async function handleChat(data) {
      ...
      if (data.roundtable === true && roundtableAvailable && verifier) {
        try {
          await runRoundTable({
            primary:  agent,
            verifier,
