...
import { runWithPaths, clampToDefaults, ALLOWED_READ_PATHS, ALLOWED_WRITE_PATHS } from "../../routes/paths.js";
...
export function makeWsHandler({ agent, store, __dirname }) {
  ...
  return function onConnection(ws) {
    ...
    let connReadPaths  = [...ALLOWED_READ_PATHS];
    let connWritePaths = [...ALLOWED_WRITE_PATHS];
    ...
    send("provider", {
      ...
      skills:        agent.alwaysOnSkillNames,
    ...
    async function init() {
      ...
      messages.push({ role: "user", content: await buildGreeting(currentLang) });

      const thinksBefore = agent.OLLAMA_THINKS;
      await runAgentLoop(
        messages, emitter,
        provider.name !== "anthropic" ? { noTools: true, lang: currentLang } : { lang: currentLang },
        () => abortController,
        (c) => { abortController = c; }
      );
    ...
    ws.on("close", () => {
      for (const msg of messages) {
        const meta = msgAttachments.get(msg);
        if (meta) {
          for (const m of meta) {
      ...
      try { finaliseSession(sessionId, messages, msgAttachments); } catch { /* non-fatal */ }
    });
    ...
    async function handleChat(data) {
      ...
      if (attachmentMeta.length > 0) msgAttachments.set(userMsg, attachmentMeta);
