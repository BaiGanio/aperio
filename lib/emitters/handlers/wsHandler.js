...
export function makeWsHandler({ agent, primaryRoundtable = null, verifier = null, roundtableAvailable = false, store, __dirname }) {
  ...
  return function onConnection(ws) {
    ...
    const workspaceDirective =
      ...
      `Write ALL generated artifacts here — generator scripts (e.g. pptx/xlsx builder .js), ` +
      `intermediate files, and final output files. Create the directory if it does not exist. ` +
      `Do NOT write into \`skills/*/scratch/\`. Files here are retained with the session and ` +
      `cleaned up automatically when it expires, so the user can download results meanwhile.`;
    ...
    ws.on("message", async (raw) => {
      await runWithPaths(connReadPaths, connWritePaths, scratchDir, async () => {
        try {
