...
export function makeWsHandler({ agent, primaryRoundtable = null, verifier = null, roundtableAvailable = false, store, __dirname }) {
  ...
  return function onConnection(ws) {
    ...
    async function init() {
      ...
      // showing one opaque number. Identity + always-on skills come from the
      // agent; the memory-preload piece is added here since the count/size is
      // only known after buildGreeting. The real total still comes from the
      // provider's reported input_tokens on the greeting turn.
      try {
        const breakdown = getStartupBreakdown();
        breakdown.memories = preloadedMemCount;
        breakdown.memoryTokens = Math.ceil((memCtx || "").trim().length / 4);
        send("startup_breakdown", breakdown);
      } catch { /* non-fatal */ }
      ...
      messages.push({ role: "user", content: greetingPrompt, [SYNTHETIC_USER]: true });

      const thinksBefore = agent.OLLAMA_THINKS;
      const greetOpts = provider().name !== "anthropic"
        ? { noTools: true, lang: currentLang }
        : { lang: currentLang };
      // memCtx is no longer injected here — buildGreeting persists it on the agent
      // (sessionMemCtx) so every turn's system prompt carries it, not just this one.
      await runAgentLoop(
        messages, emitter,
        greetOpts,
        () => abortController,
        ...
        try {
          ...
          switch (data.type) {
            ...
            case "switch_model": {
              ...
              // structured content blocks; Ollama/Gemini/DeepSeek expect plain
    ...
    async function handleChat(data) {
      // Parse /skill <name1,name2,...> prefix from the user message.
      // Strip it so the LLM never sees the command; the forced skills are
      // injected via setPendingForcedSkills → ensureTurn below.
      // We only extract the raw names here — validation against skillIndex
      // happens in ensureTurn (which has access to the full index).
      const forcedSkillNames = [];
      let cleanedUserText = data.text || "";
      const SLASH_SKILL_RE = /^\/skill\s+/i;
      while (cleanedUserText.match(SLASH_SKILL_RE)) {
        const afterSlash = cleanedUserText.replace(SLASH_SKILL_RE, "");
        const nameGroupMatch = afterSlash.match(/^([a-zA-Z][a-zA-Z0-9-]*(?:\s*,\s*[a-zA-Z][a-zA-Z0-9-]*)*)/);
        if (!nameGroupMatch) { cleanedUserText = afterSlash.trimStart(); break; }
        const nameGroup = nameGroupMatch[1];
        cleanedUserText = afterSlash.slice(nameGroup.length).trimStart();
        const names = nameGroup.split(/\s*,\s*/).filter(Boolean);
        for (const n of names) { if (!forcedSkillNames.includes(n)) forcedSkillNames.push(n); }
      }
      if (forcedSkillNames.length) {
        logger.info(`[ws] /skill parsed — forced names: [${forcedSkillNames.join(",")}]`);
      ...
      send("tool_count", { count: OLLAMA_NO_TOOLS ? 0 : getToolCount(fullUserText, messages) });
      await sendMemories();
    }
    ...
    // A confirm-before-write button (GitHub issue create/update) was clicked. The
    // action is already fully resolved and stashed server-side under the token, so
    // we execute it straight through the MCP tool — no model round-trip — and show
    // the result. Pushed into messages[] so it persists in the session transcript.
    const CONFIRMABLE_TOOLS = new Set(["create_github_issue", "update_github_issue", "delete_file"]);
    async function handleConfirmAction(data) {
      const { token, tool } = data;
      if (!CONFIRMABLE_TOOLS.has(tool) || typeof token !== "string" || !/^(?:iss|del)_[a-z0-9]+$/.test(token)) {
