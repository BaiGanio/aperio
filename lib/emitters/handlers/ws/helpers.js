...
export function normalizeMessages(msgs) {
  for (let i = msgs.length - 1; i >= 0; i--) {
    ...
    // Check if the message contains image blocks — if so, preserve the whole
    // array as-is so the next provider receives image context.
    const hasImage = m.content.some(b => b.type === "image");
    if (hasImage) {
      // Strip tool blocks but preserve text + image blocks.
      m.content = m.content.filter(b => b.type === "text" || b.type === "image");
      // If nothing remains after filtering, drop the message.
