...
export async function handleBranchConversation({
  ...
  // Finalise current session so it's saved with its current title. `messages`
  // here is still whatever this connection had BEFORE this branch reseeds it
  // below — firstMessageSynthetic describes that (now-outgoing) array, not
  // the branch note about to replace it.
  finaliseSession(sessionId, messages, msgAttachments, sessionHadAttachments, { firstMessageSynthetic });
  ...
  // `messages` was just reseeded above with the branch-context note at [0] —
  // tell the caller so the NEXT finalise of this (child) session drops it.
  return { sessionId: childId, titleSet: true, providerSessionSourceId: null, firstMessageSynthetic: true };
}
...
export async function handleResumeSession(id, {
  ...
  firstMessageSynthetic, setFirstMessageSynthetic,
}) {
  ...
  const priorFirstMessageSynthetic = firstMessageSynthetic;

  // Reset in-memory state for the fresh resume
  messages.length = 0;
  // Keep the connection-local finalisation state in lockstep with the
  // reseeded array before the first await. The socket may close while cache
  // invalidation or the bootstrap model call is in flight.
  setFirstMessageSynthetic?.(true);
  ...
  try {
    ...
    setFirstMessageSynthetic?.(priorFirstMessageSynthetic);
    throw err;
  } finally {
    setAbort(null);
  }
  ...
  if (sessionId !== id) {
    // priorMessages is a snapshot of `messages` as it stood BEFORE this
    // resume reseeded it below — firstMessageSynthetic (the param) describes
    // that snapshot, not the resume-context note this resume just injected.
    finaliseSession(sessionId, priorMessages, msgAttachments, sessionHadAttachments, { firstMessageSynthetic });
  }
  ...
  // `messages` was just reseeded above with the resume-context note at [0] —
  // tell the caller so the NEXT finalise of this session drops it.
  return { sessionId: id, titleSet: true, providerSessionSourceId: id, firstMessageSynthetic: true };
}
