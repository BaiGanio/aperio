...
export async function handleSummarize({ auto = false } = {}, {
  ...
  provider, resetProviderSession, callTool, clearDocSessionCache, emitter, makeSinkEmitter, send, sessionLogger,
  store, firstMessageSynthetic = false,
} = {}) {
  ...
  try { appendSummary(sessionId, { content: summary, messages, firstMessageSynthetic }); } catch { /* non-fatal */ }
  ...
  // The doc_batch session dedup cache lives inside the MCP child process
  // (lib/docgraph/retrieval.js's sessionReadFacts, reached via mcp/index.js's
  // custom "aperio/clearDocSessionCache" method — never a plain MCP tool, see
  // its registration comment for why) — it is now stale: messages[] just
  // lost every tool-result turn, and ragStore.index() above never embedded
  // them either (it skips tool-only turns). A later doc_batch for a sha256
  // read before this point must do a real read again — otherwise the model
  // would silently get an "already read" pointer for a document it can no
  // longer actually see. Keyed by `docSessionId` — this CONNECTION's fixed
  // dedup namespace (wsHandler.js), not `sessionId` (the persisted
  // conversation id, which another socket resuming the same session could
  // share and whose own cache this call must never touch, round 6 P1).
  // Optional chained: clearDocSessionCache is only undefined for a
  // stub/injected agent with no real MCP child to reach.
