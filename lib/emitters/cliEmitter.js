...
import { setHeaderCtxPct } from "../utils/chat-utils.js";
...
// Capped grammar set — keeps the loaded language footprint small. Fence
// languages outside this set (and their hljs aliases js/py/sh/html) fall back
// to plain rendering. `xml` covers HTML.
const emphasize = createEmphasize({ javascript, python, bash, json, xml, css, sql });
...
export function makeCliEmitter(onTurnDone, hooks = {}, options = {}) {
  ...
  const { showReasoning = false, showStats = false } = options;
  ...
  // Render a buffered code block: syntax-highlight when the language is in the
  // capped set, else fall back to flat cyan. Line count is preserved so the
  // gutter wraps each line cleanly.
  function flushCode(out, codeLines, lang) {
    const key = lang.toLowerCase();
    let rendered;
    if (key && emphasize.registered(key)) {
      rendered = emphasize.highlight(key, codeLines.join("\n")).value.split("\n");
    } else {
      rendered = codeLines.map(l => `${CYAN}${l}`);
    }
    for (const l of rendered) out.push(`${GRAY}│${R} ${l}${R}`);
  }
  ...
  return {
    send(msg) {
      switch (msg.type) {
        ...
        case "stream_end": {
          ...
          if (hadAnswer) {
            ...
            const label = msg.agent_id === "verifier"
              ? `${MAGENTA}${BOLD}β:${R}`
              : isRt ? `${CYAN}${BOLD}α:${R}` : `${CYAN}${BOLD}Aperio${R}`;
            const rendered = renderMarkdown(answerBuffer.trimEnd());
            process.stdout.write(`\n${label}\n${rendered}\n`);
            if (showStats && msg.usage) {
        ...
        // ── llamacpp.md Phase 5: evidence-gated slow-turn hint ───────────────
        case "slow_local_turn_detected":
          process.stdout.write(
            `\n${YELLOW}🐢 ${msg.model} is generating slowly (~${msg.genTps} tok/s). ${msg.hint || "Try the fast-low-vram profile."}${R}\n`
          );
          break;
        ...
        // ── loop guard: repeated tool call broken ──────────────────────────
        case "tool_repeat_break":
          process.stdout.write(
            `\n${YELLOW}⟲ ${msg.model} repeated the same tool call ${msg.repeats}× in a row — answering without tools this turn${R}\n`
          );
          break;

        // ── loop guard: per-turn tool-step cap reached ─────────────────────
        case "tool_step_limit":
          process.stdout.write(
            `\n${YELLOW}⟲ ${msg.model} used ${msg.steps} tool-calling passes in one reply (limit ${msg.limit}) — answering without tools now${R}\n`
          );
          break;
        ...
        // ── workflow detection: multi-tool turn completed ──────────────────
        case "workflow_suggestion": {
          const chain = (msg.names || []).join(" → ");
          process.stdout.write(
            `\n${GREEN}  ⚡ workflow detected: ${chain}${R}\n` +
            `${GRAY}  💡 Save as workflow? Use \`remember\` with type=workflow${R}\n`
          );
          break;
        }
