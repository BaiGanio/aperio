...
const NARROWABLE_RECALL_TOOLS = new Set(["recall", "self_recall"]);
...
function previewText(text, tokens, limit, toolName, metadata) {
  ...
  const retrievalGuidance = NARROWABLE_RECALL_TOOLS.has(toolName)
    ? "Prefer a narrower recall query, stronger filters, or a lower limit before expanding context. " +
      "Use artifact retrieval only if the complete omitted result is necessary."
    : "Use the artifact retrieval tool when available; otherwise issue a narrower follow-up query for missing details.";
  return (
    `${head}\n\n` +
    ...
    `The complete redacted result is preserved. ${retrievalGuidance}\n\n` +
    `${tail}`
  );
}
