...
// for llamacpp/deepseek. Both must be recognized so the trimmer drops a result
...
// dump is ~37k tokens, larger than a 32k local-model window in its entirety). Left
...
export function trimByTokens(msgs, hwm, contextWindow) {
  ...
  // Pin the freshest tool_result (and its paired tool_use) so trimming never
  // evicts what the model just fetched. On a tiny window this is the difference
  // between the model answering from the result and re-issuing the identical
  // call in a loop. Pinned by object identity so it survives the splices below.
  let pinnedResult = null, pinnedUse = null;
  for (let j = rest.length - 1; j >= 0; j--) {
    if (isToolResultMsg(rest[j])) {
      pinnedResult = rest[j];
      if (j > 0 && isToolOnlyAssistant(rest[j - 1])) pinnedUse = rest[j - 1];
      break;
    }
  }
  const isPinned = m => m === pinnedResult || m === pinnedUse;
  ...
  while (i + 1 < rest.length - minTail && freed < targetFreeTokens) {
    if (isToolOnlyAssistant(rest[i]) && isToolResultMsg(rest[i + 1]) && !isPinned(rest[i]) && !isPinned(rest[i + 1])) {
  ...
  while (rest.length > minTail && freed < targetFreeTokens) {
    // Never evict the pinned freshest pair, even under emergency pressure.
    if (isPinned(rest[0]) || isPinned(rest[1])) break;
