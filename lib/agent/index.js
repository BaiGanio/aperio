...
const TOOL_PROFILES = {
  ...
  file:   new Set(["read_file", "write_file", "append_file", "edit_file", "scan_project", "generate_xlsx"]),
...
function classifyProfiles(text) {
  ...
  if (/\b(file|read|write|edit|code|folder|project|scan|open|save|create|delete|directory|xlsx|spreadsheet|excel|csv|budget|table|template)\b/.test(t)) active.add("file");
...
export async function createAgent({ root, version, clientName = "Aperio-agent", providerConfig = null, persona = null } = {}) {
  ...
  async function callTool(name, input) {
    ...
    if (args.__parse_error__) {
      logger.warn(`[callTool] ${name} — skipped due to JSON parse error in args`);
