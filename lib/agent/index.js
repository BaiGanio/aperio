...
export async function createAgent({ root, version, clientName = "Aperio-agent", providerConfig = null, persona = null, character = null } = {}) {
  ...
  function logTurnOnce(t) {
    ...
    if (!t.emitted && currentEmitter && t.skills?.length) {
      currentEmitter.send({
        type: "skills_matched",
        turn: t.turnNum,
        skills: t.skills.map(s => ({ name: s.name, description: s.description || "", always: s.load === "always" })),
      });
    }
    t.emitted = true;
  }
  ...
  const agentObj = {
    ...
    // Per-component estimate of the fixed startup prompt cost, so the UI banner
    // can explain *what* the startup tokens are instead of showing one opaque
    // number. char/4 matches the client-side estimator; the banner still shows
    // the provider's real total. Memories are no longer preloaded (0).
    getStartupBreakdown() {
      ...
      return {
        ...
        memories: 0,
        greetingTools: greetingToolCount,
      };
    },
    ...
    async buildGreeting(lang = "en") {
      // Memories are NOT preloaded into the greeting prompt anymore. Previously
      // recall({limit:20}) injected ~2K tokens of memory text on every session
      // start. With semantic search active, the model recalls on demand instead
      // (the `recall` tool is available from turn 1 — see FIRST_TURN_TOOLS), so
      // we no longer pay that fixed startup cost. memCtx/preloadedMemCount are
      // kept in the return shape for callers but are always empty/0 now.
      const memCtx = "";
      const preloadedMemCount = 0;
