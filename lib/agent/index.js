...
const TOOL_PROFILES = {
  ...
  "file-generate": new Set(["write_file", "generate_xlsx", "run_node_script"]),
...
function classifyProfiles(text) {
  ...
  if (/\b(xlsx|spreadsheet|excel|csv|budget|template|pptx|slide|slides|presentation|deck|powerpoint|slideshow|generate|chart)\b/.test(t)) {
  ...
  if (/\b(write|edit|modify|append|save|create|delete|new file|rename)\b/.test(t)) {
...
export async function createAgent({ root, version, clientName = "Aperio-agent", providerConfig = null, persona = null, character = null } = {}) {
  ...
  logger.info(`[agent] model="${provider.model}" adapter="${reasoningAdapter.match}" thinks=${state.thinks} noTools=${state.noTools}${personaTag}${characterTag}`);
  ...
  function ensureTurn(messages, userText) {
    ...
    const text = recentUserText(messages, userText);
  ...
  function logTurnOnce(t) {
    ...
    logger.info(`[tools] turn=${t.turnNum} profiles=[${[...t.profiles].join(",")}] loaded=${t.names.size}/${mcpTools.length}`);
    t.logged = true;
  }
            ...
            try {
              ...
              if (info?.path) {
                ...
                if (info.path.toLowerCase().endsWith(".pptx") && (info.action === "pack" || info.action === "verify")) {
                  // Artifacts in the per-session scratch workspace are served by
                  // the /scratch static mount; map the absolute path to that URL
                  // so the download link works in the browser.
                  const scratchRel = info.path.split("/var/scratch/")[1];
                  emitter.send({
                    type: "generated_file",
                    filename: basename(info.path),
                    url: scratchRel ? `/scratch/${scratchRel}` : info.path,
