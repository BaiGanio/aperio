...
const TOOL_PROFILES = {
  ...
  file:   new Set(["read_file", "write_file", "append_file", "edit_file", "scan_project", "generate_xlsx", "run_node_script", "syntax_check"]),
...
function classifyProfiles(text) {
  ...
  if (/\b(file|read|write|edit|code|folder|project|scan|open|save|create|delete|directory|xlsx|spreadsheet|excel|csv|budget|table|template|pptx|slide|slides|presentation|deck|powerpoint|slideshow)\b/.test(t)) active.add("file");
...
export async function createAgent({ root, version, clientName = "Aperio-agent", providerConfig = null, persona = null } = {}) {
  ...
  logger.info(`[agent] model="${provider.model}" adapter="${reasoningAdapter.match}" thinks=${state.thinks} noTools=${state.noTools}${personaTag}`);
  ...
  const getSystemPrompt = (userMessage = "", lang = "en", extraSystem = "", messages = []) => {
    ...
    // Use combined text from conversation history so the skill matched in an earlier
    // turn (e.g. "create pptx") stays active on short follow-ups ("ok", "proceed")
    const historyText = messages
      .filter(m => m.role === "user")
      .flatMap(m => { const c = m.content; if (typeof c === "string") return [c]; if (Array.isArray(c)) { const t = c.find(b => b.type === "text")?.text; return t ? [t] : []; } return []; })
      .join(" ");
    const skillContext = [userMessage, historyText].filter(Boolean).join(" ");
    const onDemand = matchSkill(skillContext, skillIndex);
  ...
  function resolveToolNames(messages, userText) {
    ...
    // Accumulate text from all user turns so profiles set in an earlier message
    // (e.g. "create pptx") remain active on short follow-ups ("ok", "proceed")
    const historyText = messages
      .filter(m => m.role === "user")
      .flatMap(m => { const c = m.content; if (typeof c === "string") return [c]; if (Array.isArray(c)) { const t = c.find(b => b.type === "text")?.text; return t ? [t] : []; } return []; })
      .join(" ");
    const combined = [userText, historyText].filter(Boolean).join(" ");
    const active = classifyProfiles(combined);
  ...
  return {
    ...
    async runAgentLoop(messages, emitter, opts = {}, getAbort = () => null, setAbort = () => {}) {
      const callToolHooked = async (name, input) => {
        ...
        if (name === "generate_xlsx" && typeof result === "string" && result.startsWith("APERIO_FILE:")) {
          ...
          } catch { /* malformed marker — fall through and return raw result */ }
        }
        return result;
      };
      const hookedCtx = { ...ctx, callTool: callToolHooked };
      if (provider.name === "anthropic") return runAnthropicLoop(messages, emitter, opts, hookedCtx);
      if (provider.name === "gemini") return runGeminiLoop(messages, emitter, opts, hookedCtx);
      if (provider.name === "deepseek") return runDeepSeekLoop(messages, emitter, opts, getAbort, setAbort, hookedCtx);
    ...
    async buildGreeting(lang = "en") {
      // memCtx is returned separately so callers can inject it via extraSystem
      // rather than appending it to the user message. This prevents the memory
      // text from being scanned by matchSkill() and triggering unintended skill
      // injection that inflates startup token count.
      let memCtx = "";
      let preloadedMemCount = 0;
      try {
        const raw = await callTool("recall", { limit: 20 });
