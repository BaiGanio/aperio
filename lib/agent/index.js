...
import { isTaskShapedVisionRequest } from "../helpers/imageBridge.js";
...
export async function createAgent({ root, version, clientName = "Aperio-agent", providerConfig = null, persona = null, character = null, spec = null, bundleDir = null, hostTools = [], mcpConnection = null } = {}) {
  ...
  // activeTurns: incremented/decremented around runAgentLoop below (issue
  // #454) — the idle-shutdown watchdog reads it via getActiveTurnCount() so
  // it never kills the server mid-generation just because the browser tab's
  // heartbeat ping was starved by normal tab-backgrounding on a long turn.
  const state = { thinks: reasoningAdapter.thinks === true, noTools: reasoningAdapter.noTools === true, toolWarningEmitted: false, noToolStreak: 0, slowTurnStreak: 0, slowTurnWarningEmitted: false, activeTurns: 0 };
  // Every model gets tools + the memory pointer now (2026-07-11 decision — no
  // more model-name capability gate). The only reason a model gets neither is
  // `noTools`: a reasoning adapter that declares the model toolless, or (for a
  // local llama.cpp model) its own chat template proving it can't render a
  // tools block — set from modelCapabilities().tools in the llamacpp provider.
  const modelIsCapable = () => !state.noTools;
  ...
  // Identity files ship read-only in id/; a user edit lands in var/id/ and wins.
  // Same overlay shape as the skills tree below (skillsDir + overlayDir): the
  // bundled default stays pristine and deleting the override restores it. The
  // writable half must live under var/ because the production container and the
  // k8s Deployment run on a read-only root filesystem — saving into id/ would
  // fail with EROFS — and because only var/ survives container recreation.
  const idOverlayDir = resolve(root, "var", "id");
  const readIdFile = f => {
    for (const p of [resolve(idOverlayDir, f), resolve(root, "id", f)]) {
      try { return readFileSync(p, "utf-8"); } catch { /* try the bundled default */ }
    }
    return "";
  };
  ...
  const {
    mcp, mcpConnection: activeMcpConnection, ownsMcpConnection,
    mcpTools, hostToolHandlers, toolSchemas, allowedToolNames,
  ...
  // The skill block this conversation last resolved (names only), fed back
  // into planTurnTools so a live flow re-sends a byte-identical block instead
  // of a recomputed one — the cached-prefix half of the llama.cpp KV-reuse fix
  // (#250; see resolvePinnedSkills in turn-planner.js for the mechanism and
  // its bounds). Keyed by the same `messages` array reference and for the same
  // reason as turnCacheByMessages above: one agent instance serves every
  // WebSocket connection, so this state must never be a single shared slot.
  // A WeakMap also means no cleanup on disconnect.
  const skillPinByMessages = new WeakMap();
  ...
  function ensureTurn(messages, userText, imageMessages) {
    ...
    const planned = planTurnTools(messages, userText, {
      ...
      pinnedSkillNames: skillPinByMessages.get(messages) ?? [],
      providerName: provider.name,
      imageMessages,
    });
    // null = a synthetic turn, which must not overwrite the flow's pin.
    if (planned.skillPinNames) skillPinByMessages.set(messages, planned.skillPinNames);
    ...
    const turn = {
      ...
      skillsPinned: planned.skillsPinned,
  ...
  function logTurnOnce(t) {
    if (t.logged) return;
    // `skills=` names the resolved block (and whether it came from the pin) so
    // a cache-fingerprint log can be read against it: a sysHash that moves
    // while this line is unchanged is NOT a skill-set change (#250).
    const skillNames = (t.skills ?? []).map(s => s.name).join(",");
    // `t.names` is the intent plan, before the model-capability and vision
    // gates. Report the post-gate count as attached; otherwise an incapable
    // local model can misleadingly log dozens of schemas while receiving [].
    const attachedCount = getSelectedTools(t).length;
    const planned = attachedCount === t.names.size ? "" : ` planned=${t.names.size}`;
    logger.info(`[tools] turn=${t.turnNum} profiles=[${[...t.profiles].join(",")}] attached=${attachedCount}/${mcpTools.length} schemas${planned} (re-sent each turn — LLM APIs are stateless) skills=[${skillNames}]${t.skillsPinned ? " (pinned)" : ""}`);
    t.logged = true;
  ...
  function finishSystemPrompt(parts, messages, extraSystem = "") {
    ...
    const providerVision = modelCapabilitiesSync(provider.model, process.env, {
      provider: isLocalProvider(provider.name) ? undefined : provider.name,
    }).vision;
    if (hasImage && providerVision && isTaskShapedVisionRequest(messages.at(-1)?.content?.find?.(b => b.type === "text")?.text ?? "", { hasImage: true })) {
  ...
  function resolveToolNamesForTurn(t) {
    ...
    const visionFiltered = filterVisionTools(t.names, {
      ...
      modelHandlesInlineImage: modelCapabilitiesSync(ctx.provider?.model, process.env, {
        provider: isLocalProvider(ctx.provider?.name) ? undefined : ctx.provider?.name,
      }).vision,
    });
  ...
  function createPrepareModelContext(emitter, lifecycleTrace, planningState = null, onModelContextShed = null, untrimmedMessages = null) {
    ...
    return async function prepareModelContext({
      ...
      const skillPromptParts = request.skillPromptParts ?? [];
      return {
        ...
        // Skills now live in the system prompt (see the skill-injection
        // middleware), so the skill-free variant is a second *system prompt*
        // rather than a second message array. Identity-filtered against the
        // exact strings the middleware recorded, so nothing else in
        // promptParts can be dropped by accident.
        systemPromptNoSkills: skillPromptParts.length
          ? finishSystemPrompt(
            request.promptParts.filter(part => !skillPromptParts.includes(part)),
            request.messages,
            extraSystem,
          )
          : null,
  ...
  async function callTool(name, input, meta) {
    ...
    // authoritative; a failed call just gets a pointed hint so the model can
    // self-correct on retry.
    const toolSchema = toolSchemas.get(name);
    const schemaIssues = checkArgs(args, toolSchema, name);
    // The hint is advisory TEXT handed back after a call has already failed. It
    // was previously suppressed for destructive tools, which conflated two
    // different guards: `parseArgs` refuses to REPAIR malformed args for those
    // tools because a regex repair can shift string boundaries and land a
    // corrupted write (lib/tools/executor.js) — that guard is untouched and must
    // stay. Declining to EXPLAIN the failure protects nothing; it only costs the
    // model the one piece of information it needs to get the retry right. The
    // 2026-08-13 db_execute run is the case in point: three identical malformed
    // calls, each told only "`sql` is required" and never that a lost-sync parse
    // had folded the statement into a key.
    const repairHint = () => hintFromIssues(name, schemaIssues, Object.keys(toolSchema?.properties ?? {}));
    let result;
    try {
  ...
  const agentObj = {
    ...
    getMcpConnection() { return activeMcpConnection; },
    async close() {
      if (!ownsMcpConnection || mcpClosed) return;
      mcpClosed = true;
      await mcp.close();
    },
    saveWhoamiContent(content) {
      mkdirSync(idOverlayDir, { recursive: true });
      writeFileSync(resolve(idOverlayDir, "whoami.md"), content, "utf-8");
    ...
    getActiveTurnCount() { return state.activeTurns; },
    ...
    async runAgentLoop(messages, emitter, opts = {}, getAbort = () => null, setAbort = () => {}) {
      ...
      // Counted for the duration of real work only — the early return
      // above (no provider configured) never starts a turn.
      state.activeTurns++;
      try {
        ...
        const { callToolHooked, surfaceScratchArtifacts,
                flushDownloadCards, verifyFileClaims, verifyCurrencyClaims, workflowSequence } = hooks;
        ...
        // Also requires modelIsCapable(): for a toolless model (state.noTools),
        // getSelectedTools()/
        ...
        // Final-answer currency-blend guard — unrelated to tool use, so it runs
        // regardless of opts.noTools.
        if (typeof finalText === "string" && finalText) {
          try { verifyCurrencyClaims(finalText); } catch (err) {
            logger.error(`[verifyCurrencyClaims] threw: ${err.message}`);
          }
        }
        // ── Workflow detection ──────────────────────────────────────────────
