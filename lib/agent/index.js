...
import {
  ...
  SYNTHETIC_USER,
  TOOL_PROFILES, HOST_TOOL_PROFILES, FIRST_TURN_TOOLS,
  SELF_MEMORY_TOOLS,
  SELF_WIKI_TOOLS,
  isShellAllowedFor,
  isCapableModel,
 ...
 * file.
 *
 * Returns one descriptor per written file, in the same order the client's
 * _stripDeliverables finds them, so the client can attach the real workspace URL
 * to the card it built from the message text. Without that URL the card can only
 * offer preview/download from the in-memory string — "open in browser" and "show
 * in folder" have nothing to point at.
 */
export function persistAnswerArtifacts(text, scratchDir) {
  if (!text || !scratchDir) return [];
  const written = [];
  const save = (ext, code) => {
    let base = ext === "html" ? "index.html" : `build-${written.length + 1}.${ext}`;
    if (ext === "html") {
    ...
    try {
      ...
      const abs = join(scratchDir, `${prefix}-${base}`);
      writeFileSync(abs, code, "utf8");
      // Same /scratch/<session>/<file> shape the download-card path uses, so
      // both kinds of card resolve through the one artifact route.
      const rel = abs.split(`${sep}var${sep}scratch${sep}`)[1];
      written.push({
        filename: base,
        url: rel ? `/scratch/${rel.split(sep).join("/")}` : null,
        sizeKb: Math.max(1, Math.round(Buffer.byteLength(code, "utf8") / 1024)),
      });
    } catch (err) {
...
export async function createAgent({ root, version, clientName = "Aperio-agent", providerConfig = null, persona = null, character = null, spec = null, bundleDir = null, hostTools = [] } = {}) {
  ...
  mcpTools = [...mcpTools, ...hostToolSchemas];
  ...
  const agentObj = {
    ...
    async runAgentLoop(messages, emitter, opts = {}, getAbort = () => null, setAbort = () => {}) {
      ...
      if (!opts.noTools && modelIsCapable() && mcpTools.some(tool => tool.name === "doc_repos")) {
        const lastUser = [...messages].reverse().find(message => message.role === "user" && !message[SYNTHETIC_USER]);
        const query = lastUser ? extractUserText(lastUser) : "";
        if (isDocRepoInventoryIntent(query)) {
      ...
      //
      // A turn whose code block we captured and wrote to the workspace is NOT an
      // instance of this failure: the file exists on disk, so telling the user the
      // model "answered with code instead of writing files" is simply false.
      // Treat it like a tool call and clear the streak.
      if (!opts.noTools && !state.noTools) {
        const proseWithCode =
          hooks.toolSeq.value === 0 &&
          answerArtifacts.length === 0 &&
          typeof finalText === "string" &&
          finalText.includes("```");
        if (hooks.toolSeq.value > 0 || answerArtifacts.length > 0) {
