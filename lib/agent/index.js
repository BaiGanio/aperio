...
import { runLlamaCppLoop } from "./providers/llamacpp.js";
...
import { getProviderSessionId, updateProviderSessionId } from "../helpers/sessions.js";
...
export async function createAgent({ root, version, clientName = "Aperio-agent", providerConfig = null, persona = null, character = null, spec = null, bundleDir = null } = {}) {
  ...
  function buildProviderTag(p) {
    const label = p.name === "ollama" ? `Ollama (${p.model})` : p.name === "llamacpp" ? `llama.cpp (${p.model})` : p.name === "deepseek" ? `DeepSeek (${p.model})` : p.name === "gemini" ? `Google Gemini (${p.model})` : p.name === "claude-code" ? `Anthropic Claude via subscription (${p.model})` : `Anthropic Claude (${p.model})`;
  ...
  function ensureTurn(messages, userText) {
    ...
    const cappedNames = capToolsForWindow(names, provider.contextWindow);
    ...
    const specNames = allowedToolNames
      ...
      : cappedNames;
    turnCache = { key, turnNum, profiles, names: specNames, skills, logged: false };
    return turnCache;
  }
  ...
  function getSelectedTools(turn) {
    ...
    return mcpTools.filter(tool => names.has(tool.name));
  }
  ...
  async function buildGreetingMemBlock() {
    ...
    try {
      ...
      const parts = [
        ...
        "Open with a warm 1–2 sentence greeting that reflects genuine continuity — what you " +
        ...
        "These notes reflect what was true when written and may now be stale, so stay tentative about specifics " +
        "(\"last I remember…\") rather than asserting them as current fact, and never invent details not present here.",
      ];
  ...
  const ctx = {
    provider, callTool, getSystemPrompt, getAnthropicTools, getOllamaTools,
  ...
  const agentObj = {
    ...
    async runAgentLoop(messages, emitter, opts = {}, getAbort = () => null, setAbort = () => {}) {
      ...
      const appendArtifactTool = (tools, kind) => {
        return appendArtifactReadTool(tools, kind, hooks.hasOffloadedArtifacts());
      };
      ...
      const hookedCtx = {
        ...
        getOllamaTools: (...args) => appendArtifactTool(ctx.getOllamaTools(...args), "ollama"),
        ...
        if (q) {
          try {
            ...
            if (typeof raw === "string" && raw.trim() && !raw.includes("No memories") && raw !== "No result") {
              // Parse recall text blocks for scope: tags
              const blocks = raw.split("\n---\n");
              const scopeHints = [];
              for (const block of blocks) {
                const lines = block.split("\n");
                // Find the Tags: line and look for scope: trigger
                const tagLine = lines.find(l => /^Tags:\s/i.test(l));
                if (!tagLine) continue;
                const scopeTag = tagLine.match(/scope:(\S+)/i);
                if (!scopeTag) continue;
                const trigger = scopeTag[1].replace(/[,;]$/, "").trim().toLowerCase();
                if (!trigger) continue;
                // The content lines are between the title line and the Tags line
                const titleMatch = block.match(/^\[PREFERENCE\]\s+(.+?)\s+\(/m);
                const title = titleMatch ? titleMatch[1].trim() : "";
                // Content = everything between title line and Tags line
                const afterTitle = block.replace(/^\[PREFERENCE\].+/m, "").trim();
                const content = afterTitle.replace(/\nTags:.*$/s, "").trim();
                // Extract a filesystem path from the content (first path-like string)
                const pathMatch = content.match(/(\/[\w./-]+)+/);
                const scopePath = pathMatch ? pathMatch[1] : null;

                const scope = { trigger, content, title, path: scopePath || null };
                hooks.activeScopes.push(scope);
                scopeHints.push(`• "${trigger}": ${scopePath ? `search ${scopePath}` : `scope: ${title}`}`);
              }
      ...
      // loops mutate in place stays intact. Local providers (Ollama, llama.cpp)
      // skip it.
      let finalText;
      ...
      if (!opts.noTools && workflowSequence.length >= 2 && emitter) {
        const toolNames = [...new Set(workflowSequence.map(t => t.name))];
        logger.info(`[agent] workflow detected: ${toolNames.join(" → ")}`);
        emitter.send({
          type: "workflow_suggestion",
          tools: workflowSequence.map(t => ({ name: t.name, summary: t.summary })),
          names: toolNames,
        });
      }
      flushDownloadCards();
    ...
    setProvider(config) {
      ...
      // state. Both flags are reset unconditionally — cheap, and correct
      // whichever local provider (if either) is now active.
