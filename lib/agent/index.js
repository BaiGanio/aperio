...
export async function createAgent({ root, version, clientName = "Aperio-agent", providerConfig = null, persona = null, character = null } = {}) {
  ...
  function createPrepareModelContext(emitter) {
  ...
  const agentObj = {
    ...
    async runAgentLoop(messages, emitter, opts = {}, getAbort = () => null, setAbort = () => {}) {
      ...
      const hooks = makeTurnHooks(emitter, turnStartMs, artifactContext);
      ...
      const prepareModelContext = createPrepareModelContext(emitter);
      const hookedCtx = {
        ...ctx,
