...
const CONFIRM_TOOLS = new Set(["create_github_issue", "update_github_issue", "delete_file"]);
...
const TOOL_PROFILES = {
  ...
  "file-generate": new Set(["write_file", "generate_xlsx", "generate_docx", "run_node_script", "run_python_script"]),
  ...
  // GitHub issues: read an issue, or (write) create/update one. The write tools
  // are confirm-before-write — they preview first and only post on confirm:true.
  github:        new Set(["fetch_github_issue", "create_github_issue", "update_github_issue"]),
};
...
export async function createAgent({ root, version, clientName = "Aperio-agent", providerConfig = null, persona = null, character = null } = {}) {
  ...
  const CHARACTER_PROMPT = character
    ...
    : "";
  // Preloaded memory snapshot for THIS session, set once by buildGreeting and
  // appended to the system prompt on every turn (not just the greeting) so the
  // model keeps knowing what it knows about the user. Without this the model goes
  // amnesiac after the greeting and denies having any memories. Stable within a
  // session, so it sits in the cache-friendly prefix.
  let sessionMemCtx = "";
  ...
  const transport = new StdioClientTransport({ command: "node", args: [resolve(root, "mcp/index.js")], env: { ...process.env, APERIO_PROC_ROLE: "mcp" } });
  ...
  // The few highest-impact memories preloaded into every turn's system prompt
  // (sessionMemCtx). A no-query recall lists by importance DESC, so this is the
  // top-N. refreshSessionMemCtx() (re)loads them — at greeting, and again after
  // any memory write so saves made mid-session show up on later turns. Returns
  // the count for the startup token breakdown. On a transient recall failure it
  // leaves the previous snapshot intact rather than wiping it.
  ...
  async function refreshSessionMemCtx() {
    try {
      ...
      if (raw && raw.trim() && !raw.includes("No memories")) {
        // Frame the snapshot as a PREVIEW, not the whole store. Weak agentic
        // models (e.g. QWEN3:30B-A3B) otherwise read "what you know" literally,
        // treat these few rows as exhaustive, and deny having more memories
        // instead of calling recall. The raw text already carries a "showing N
        // of M" footer from the recall handler.
        sessionMemCtx =
  ...
  async function callTool(name, input) {
    ...
    // A successful memory write makes the session snapshot stale — refresh it so
    // mid-session saves appear in later turns' system prompt. recall is not a
    // write tool, so refreshSessionMemCtx's own recall won't recurse here.
  ...
  const agentObj = {
    ...
    greetingToolCount,
    // Exposed (same fn the providers get via ctx) so callers/tests can inspect
    // the composed system prompt, e.g. to confirm the session memory snapshot
    ...
    // Serve a resolved skill's SKILL.md body on demand (the chip fetches it only
    // when a user expands a row), so the full content isn't pushed over the WS
    // every turn. Frontmatter is stripped so it renders as a clean document.
    getSkillDoc(name) {
    ...
    async runAgentLoop(messages, emitter, opts = {}, getAbort = () => null, setAbort = () => {}) {
      ...
      // Wall-clock start of this turn. Used to gate scratch-dir artifact
      // surfacing to files actually produced this turn (see surfaceScratchArtifacts).
      ...
      // Download cards are COLLECTED during the turn and emitted only after the
      // provider loop has produced the final answer (see flushDownloadCards).
      // Emitting mid-turn was unreliable: the card raced the multi-turn stream
      // boundaries and the frontend would attach it to an intermediate/empty
      // bubble that then got discarded, so the button silently vanished whenever
      // the model ran any tool turn after creating the file.
      const downloadCards = [];

      // Queue a download card for a generated artifact that lives in the session
      ...
      const surfaceArtifact = (absPath) => {
        ...
        // Strip the 8-hex `uuid-` prefix that generate_docx/xlsx (and soffice
        // output derived from them) add to the on-disk name, so the download
        // card shows the clean filename. The url keeps the real on-disk name.
      ...
      // Emit every queued download card. Called once, after the final answer has
      // streamed, so the frontend can attach each card to the now-rendered answer
      // bubble instead of guessing at a mid-turn stream boundary.
      const flushDownloadCards = () => {
        const seen = new Set();
        for (const card of downloadCards) {
          if (seen.has(card.url)) continue; // generate_xlsx + claim-scan can queue the same file
          seen.add(card.url);
          emitter.send(card);
        }
      ...
      //
      // Only files created or modified during THIS turn are surfaced. The
      // per-turn dedup Set is reset every runAgentLoop call, so without this
      // mtime gate a file produced in an earlier turn would be re-emitted as a
      // fresh download card on every later turn that runs a node script —
      // duplicate cards plus needless work. A small clock-skew tolerance keeps
      // files written microseconds before turnStartMs from being missed.
      const surfaceScratchArtifacts = () => {
        ...
        for (const e of entries) {
          ...
          if (st.mtimeMs < turnStartMs - 1000) continue; // leftover from a prior turn
          surfaceArtifact(abs);
        }
      };
      ...
      const callToolHooked = async (name, input) => {
        ...
        // Without this a model-named file like "create_security_pdf.js" resolves
        // against the MCP's cwd — the project root — and pollutes it. write_file
        // creates, so its target always goes to scratch; the read/run/edit tools
        // only redirect when the file actually exists there, so a project file
        // referenced by a relative path (e.g. a skill script) still resolves
        // against the project root. Mutates callArgs in place so the MCP call,
        // the post-write validator, and the UI summary all see the resolved path.
        if (name === "write_file") {
          ...
          // run_shell runs in the MCP subprocess, which has no scratch context,
          // so its own getActiveScratchDir() is always null — it can't default
          // its own cwd. Inject one here (main process) when the model omits it:
          // the session scratch dir if it actually exists yet, else the project
          // root. Both are within the allowed write floor, so the subprocess's
          // cwd guard accepts them and project-inspection commands (find/git/grep
          // over the repo) work without the model needing to pass a cwd.
        ...
        if ((name === "generate_xlsx" || name === "generate_docx") && typeof result === "string" && result.startsWith("APERIO_FILE:")) {
          try {
            ...
            // Surface the real on-disk path so the model can operate on the file
            // (e.g. convert docx→pdf via soffice). The user-facing card uses the
            // clean filename; the saved path carries the uuid-prefixed scratch name
            // the model would otherwise have no way to guess.
        ...
        if ((name === "run_node_script" || name === "run_python_script") && typeof result === "string") {
          ...
          // Python skill scripts (e.g. soffice.py docx→pdf) print no marker, but
          // surfaceScratchArtifacts() below still catches whatever PDF/xlsx/etc.
          // they leave in the session workspace — so this block runs for both.
        ...
        // Confirm-before-write tools, phase 1: strip the token from the model's
        // view so it can't self-confirm, and emit a single action_confirm_pending
        // event the clients render (web: a button that makes the SERVER execute
        // the stashed action on click; terminal: a token to reply with). The
        // model just ends its turn either way.
      ...
      // Weak local models (e.g. QWEN3:30B-A3B, a 3B-active MoE) won't reliably
      // call recall on their own — they answer "I have no memory of that" from
      // the small preloaded preview. So when the user's message is clearly a
      // memory lookup, we search the store ourselves and inject the hits as
      // context for THIS turn via extraSystem (honored by every provider loop).
      // Routed through callToolHooked so the recall shows up as a normal tool
      // card and emits recall_result for the UI. Skipped on no-tool turns
      // (greeting), which already preload memories.
      //
      // Gated to Ollama (local models). Cloud providers (Anthropic, Gemini,
      // DeepSeek, claude-code) run capable models that call recall on their own,
      // so the pre-fetch would only add latency/tokens there — whereas an extra
      // local recall is free, which is why the trigger regex can stay inclusive.
      // ctx.provider (not the boot-time `provider`) so a mid-session setProvider
      // switch is respected.
      ...
      // Final-answer hallucination guard. The provider loop has returned the
      // definitive final assistant text (not an intermediate tool-calling turn),
      // so this is the one place we can reliably check file-creation claims. If
      // the model says it created/saved a downloadable file (pdf/docx/xlsx/…)
      // but that file is not in the session workspace, surface a correction to
      // the user and record it in history so the model doesn't double down. This
      // generalises the pptx APERIO_PPTX disk-verify (which only fires when a
      // script prints a success marker) to the common case where the model lies
      // in prose after its tools failed or it skipped them entirely.
      ...
      // Emit any download cards now that the final answer (and any correction)
      // has streamed, so the frontend attaches them to the rendered answer.
      flushDownloadCards();
      return finalText;
      ...
      function verifyFileClaims(text) {
        const scratch = getActiveScratchDir();
        if (!scratch) return;
        // Match creation/save/conversion language followed by a downloadable
        // filename on the same line. Anchored on a verb so plain references to an
        // input file ("based on report.docx") aren't flagged as creation claims.
        // Filename class excludes spaces so the lazy lead-in can't bleed
        // backward across words into the capture (which would invent a bogus
        // "missing file" name). Generated artifacts use underscores/hyphens.
        ...
        // The model says it created these and they really are on disk — make sure
        // each has a download card. This is the tool-agnostic catch-all: it fires
        // no matter which tool produced the file (run_node_script, run_python_script,
        // run_shell, …), so the button never goes missing just because the file was
        // generated by a path that doesn't scan the workspace itself. Deduped via
        // surfaceArtifact's key set, so files already queued aren't doubled.
        ...
        // Render as a fresh assistant bubble. After the turn's stream_end the
        // frontend has reset its streaming state, so a new stream_start/token/
        // stream_end cycle cleanly appends a follow-up message. No usage payload
        // → the frontend skips token-badge annotation for this correction.
        ...
        // Record the correction in history so later turns don't repeat the lie.
        // Append it INTO the assistant message the provider loop just pushed,
        // rather than adding a new message — a standalone turn would create
        // consecutive same-role messages, which Gemini rejects. Handles both
        // content shapes: Anthropic's content-block array and the plain string
        // the other providers store.
        const note = `\n\n[System correction: the file(s) ${missing.join(", ")} were NOT actually written to the workspace. Do not claim they exist. If the user still wants them, generate them and verify they are on disk before reporting success.]`;
        ...
        if (last?.role === "assistant") {
          if (typeof last.content === "string") {
            ...
            else last.content.push({ type: "text", text: note.trimStart() });
          }
        }
      }
    },
    ...
    async buildGreeting(lang = "en") {
      // Preload the few highest-impact memories so the model isn't amnesiac at
      // session start, without the old ~2K-token cost of injecting 20. The load
      // (and the every-turn persistence) lives in refreshSessionMemCtx; here we
      // just read the resulting snapshot back for the startup token breakdown.
