  ...
  const agentObj = {
    ...
    getStartupBreakdown() {
      ...
      return {
        ...
        // The recall pointer (capable models only); 0 for weak models.
        memoryTokens: est(sessionMemCtx),
        toolSchemas,
      };
    },
    ...
    async buildGreeting(lang = "en") {
      ...
      // A persona or character is worth an in-character, model-generated intro;
      // the plain default assistant is not — its greeting is a throwaway "Hi!",
      // so we skip the whole inference and show a static localized line instead.
      // `staticGreeting` is null when we want the LLM to greet, the text otherwise.
      const staticGreeting = (persona || character) ? null : staticText;
      return { prompt: greetingPrompt, memCtx, preloadedMemCount, staticGreeting };
    },
    setProvider(config) {
