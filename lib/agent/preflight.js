...
export async function runPreflight({
  ...
  if (!opts.noTools && modelIsCapable() && isDocumentAggregationIntent(lastUserText()) &&
    ...
    messages.push({ role: "assistant", content: [{ type: "tool_use", id: manifestId, name: "doc_manifest", input: { query } }], [PREFLIGHT_TOOL_USE]: true });
    ...
    messages.push({ role: "user", content: [{ type: "tool_result", tool_use_id: manifestId, content: manifestRaw }], [SYNTHETIC_USER]: true });
    ...
    if (Array.isArray(manifest?.candidates) && manifest.candidates.length > 0) {
      ...
      messages.push({ role: "assistant", content: [{ type: "tool_use", id: batchId, name: "doc_batch", input }], [PREFLIGHT_TOOL_USE]: true });
      ...
      const batchContent = remaining > 0
        ...
        : batchRaw;
      messages.push({ role: "user", content: [{ type: "tool_result", tool_use_id: batchId, content: batchContent }], [SYNTHETIC_USER]: true });
