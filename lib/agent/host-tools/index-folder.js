...
export function createIndexFolderTool(folderIndexer, {
  ...
  function authorizationProposal(args, path) {
    ...
    return [
      ...
      `Index: ${panels}`,
      "",
      // The confirm grants app-wide, persistent read AND write under `path`
      // (authorizePath → setAllowlist, one list serving both ceilings). Say so
      // here: this line is where the user actually decides. See SECURITY.md.
      `Action: Allow Aperio to read AND write anything under ${path} (permanent, all future sessions), then index it`,
      `Token: ${token}`,
    ].join("\n");
  }
