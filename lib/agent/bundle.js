...
/**
 * Shared tail of the narrowing pipeline: normalize the merged spec shape, then
 * — when an adminSpec ceiling is given — assert the resulting filesystem/
 * memory-scope permission rules never widen beyond it. Used both by
 * loadAgentBundle (disk-sourced patches) and narrowAgentSpec (in-memory
 * patches, e.g. lib/agent/spawn.js's sub-agent delegation).
 */
function finalizeNarrowedSpec(merged, adminSpec) {
...
/**
 * Applies a single security-sensitive patch (the same shape as a bundle's
 * permissions.json — see PERMISSION_KEYS) to a base AgentSpec, in memory,
 * with the same narrowing guarantees loadAgentBundle enforces for on-disk
 * bundles: an adminSpec ceiling (when given) can only be narrowed, never
 * widened. This is the "future delegated agents" hook referenced above —
 * lib/agent/spawn.js uses it to turn a parent AgentSpec into a child spec.
 */
export function narrowAgentSpec(baseSpec, patch = null, adminSpec = null) {
  const merged = cloneJson(normalizeAgentSpec(baseSpec));
  if (patch) applyPatch(merged, patch, adminSpec);
  return finalizeNarrowedSpec(merged, adminSpec);
}
...
export function loadAgentBundle({ root = process.cwd(), bundleDir = null, baseSpec, adminSpec = null } = {}) {
  ...
  const spec = finalizeNarrowedSpec(merged, adminSpec);
