...
export function createToolHooks({
  ...
  return function makeTurnHooks(emitter, turnStartMs) {
    ...
    async function callToolHooked(name, input) {
      ...
      const { ok, summary, details, detail } = summarizeResult(name, result);
      emitter.send({ type: "tool_result", seq, name, ok, summary, ms: Date.now() - startedAt, ...(details ? { details } : {}), ...(detail ? { detail } : {}) });
