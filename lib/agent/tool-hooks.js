  ...
  return function makeTurnHooks(emitter, turnStartMs, artifactContext = null, lifecycleTrace = null) {
    ...
    function verifyFileClaims(text) {
      ...
      for (const diskName of onDisk) {
        scratchFiles.set(diskName, diskName);
        scratchFiles.set(diskName.replace(/^[0-9a-f]{8}-/i, ""), diskName);
      }
      ...
      const check = (name) => {
        ...
        if (diskName) {
          present.add(name);
          surfaceArtifact(join(scratch, diskName));
          return;
        }
        missing.add(name);
      };
      ...
      for (const name of present) missing.delete(name);
