...
import { createPlanningToolMiddleware } from "./planning-middleware.js";
...
export function createToolHooks({
  ...
  clearDocSessionCache = null, // invalidates docgraph's doc_batch session dedup cache (see createToolResultOffloadMiddleware)
   ...
   * @param {object|null} [toolCallMeta] — per-turn metadata forwarded verbatim
   *   to every callTool() call this turn (e.g. docSessionId for docgraph's
   *   session-scoped dedup). Captured in THIS call's own closure — never a
   *   shared/module-level variable — so concurrent turns on the same shared
   *   agent instance can never cross-contaminate each other's metadata.
  ...
  return function makeTurnHooks(
    ...
    // Runs strictly after safetyRunner's afterTool below (WS1 lifecycle-trace
    // ordering requirement) — never before, and never able to block a call.
    const plannerRunner = planningState
      ? createLifecycleRunner([createPlanningToolMiddleware({ state: planningState, emitter, logger })], { trace: lifecycleTrace })
      : null;
    ...
    const offloadRunner = createLifecycleRunner([
      createToolResultOffloadMiddleware({
        ...
        docSessionId: toolCallMeta?.docSessionId ?? null,
      }),
    ...
    // Scratch code files the model wrote BY NAME this turn (write_file/edit_file/
    // append_file). Unlike the directory sweep these are unambiguous: the model
    // chose the path and handed it to a write tool, so they surface as cards
    // whether or not the prompt matched the intent heuristic. Execution is still
    // the exclusion gate — a script it ran is a generator, not the result.
    const writtenCodeFiles = new Set();
    ...
    function surfaceCodeArtifacts() {
      // A file the model wrote by name and never ran is a deliverable by
      // construction. It surfaces even when the prompt never asked for a file:
      // the model saying "I saved it to parse-numeric-config.js" with no card
      // beneath leaves the user with a claim they cannot open. The heuristic
      // below still gates the broad sweep, where the same certainty is absent.
      for (const abs of writtenCodeFiles) {
        if (executedScripts.has(abs)) continue;
        pushCard(abs);
      }
    ...
    async function callToolHooked(name, input) {
      ...
      const result = isArtifactRead
        ...
        : await callTool(name, callInput, toolCallMeta);

      // Computed once from the untouched raw `result` (matching pre-WS3
      // behavior byte for byte — offload/fencing below never rewrite `result`
      // itself). `ok` is also used by the write-tool branches further down.
      const { ok, summary, details, detail, memories } = summarizeResult(name, result);

      // Emits exactly once per call, right before whichever return statement
      // this call takes. `artifact` is the only thing that varies by exit
      // path, since offload (below, via finalizeToolResult) only ever runs on
      // the two paths that reach it — everywhere else it stays undefined,
      // same as before offload existed at all. Merged in here rather than
      // reparsed from `result` text, since offload status isn't recoverable
      // that way once the pointer has been folded into a preview string.
      function sendToolResult(artifact) {
        artifactLifecycle.recordToolResult(name, callArgs, result);
        emitter.send({
          type: "tool_result", seq, name, ok, summary, ms: Date.now() - startedAt,
          // An offloaded result was too big/sensitive to keep in context and
          // got redacted before storage — detail/details/memories are all
          // built from the *raw, pre-redaction* result (see the comment
          // above), so once there's an artifact they must NOT also ship over
          // this event: that would leak un-redacted content the offload step
          // was specifically meant to keep out of the socket. The artifact
          // pointer + summary are what the card gets instead.
          ...(artifact ? {} : {
            ...(details ? { details } : {}), ...(detail ? { detail } : {}), ...(memories ? { memories } : {}),
          }),
          ...(artifact ? { artifact } : {}),
        });
      ...
      // Offloads an oversized final result to the artifact store — must run
      // AFTER provenance fencing/taint (below), never before: the artifact
      // stored (and any UI/harness pointer to it) has to reflect the fenced
      // text, not the pre-fence raw content, or the fence protecting the model
      // from untrusted-content injection would be bypassable by reading the
      // artifact back. Returns the artifact pointer (if any) for sendToolResult.
      async function finalizeToolResult(finalText) {
        const offloaded = await offloadRunner.run("afterTool", { name, result: finalText });
        const artifact = offloaded.request.artifact;
        if (artifact && !QUERYABLE_OFFLOAD_TOOLS.has(name)) retrievableOffloadSources.add(name);
        return { text: offloaded.request.result, artifact };
      }
      ...
      if (afterSafety.stopped) {
        sendToolResult();
        return afterSafety.value;
      }
      let modelResult = afterSafety.request.modelResult;

      // Observation only — never blocks, never rewrites the result.
      if (plannerRunner) await plannerRunner.run("afterTool", { name, callArgs, result });
      ...
      // Source code the model wrote by name: remembered here, decided at turn
      // end (surfaceCodeArtifacts), once it is known whether it was executed.
      if (WRITE_TOOLS.has(name) && ok && typeof callArgs.path === "string"
          && callArgs.path.replace(/\\/g, "/").includes("/var/scratch/") && CODE_EXT.test(callArgs.path)) {
        writtenCodeFiles.add(callArgs.path);
      }
        ...
        if (scratchDir) {
          ...
          if (imgPaths.length) {
            ...
            if (urls.length) {
              ...
              const finalized = await finalizeToolResult(patched);
              sendToolResult(finalized.artifact);
      ...
      if (typeof result === "string") {
        const tokenMatch = result.match(/\bToken:\s*((?:iss|del|wr|db|idx|tpl)_[a-z0-9]+)\b/i);
      ...
      const finalized = await finalizeToolResult(modelResult);
      sendToolResult(finalized.artifact);
      return finalized.text;
    }
    ...
    // ── Final-answer currency-blend guard ───────────────────────────────────
    // SKILL.md §6 ("never blend, never convert") has been re-worded twice
    // (once with a worked counter-example, once with a pre-send self-check
    // imperative) and both still failed live — a model that got every
    // per-currency line right still added a "Grand total" summing BGN + EUR
    // as one number (id/reference/tech-debt.md, "Document Intelligence —
    // save/insert mechanics on gemma4"). Prose has a demonstrated ceiling
    // here, so this is a deterministic check instead: it never edits or
    // blocks the model's own answer (already streamed by the time this
    // runs), it only appends a correction, exactly like verifyFileClaims.
    function verifyCurrencyClaims(text) {
      if (typeof text !== "string" || !text) return;
      const NUM = "\\d[\\d,]*(?:\\.\\d+)?";
      // Up to 10 non-digit, non-paren filler characters between the total and
      // its breakdown — markdown bold/italic closers ("**893.24** (...)") sit
      // right there in the recorded regression this guard targets.
      const OUTER_RE = new RegExp(`(${NUM})[^\\d()\\n]{0,10}\\(([^()]{0,120})\\)`, "g");
      const TERM_RE = new RegExp(`(${NUM})\\s*([A-Z]{3})\\b`, "g");
      const toNumber = raw => {
      ...
      const blends = [];
      for (const m of text.matchAll(OUTER_RE)) {
        const outer = toNumber(m[1]);
        if (outer == null) continue;
        const terms = [...m[2].matchAll(TERM_RE)];
        if (terms.length < 2) continue;
        const amounts = terms.map(t => toNumber(t[1]));
        if (amounts.some(a => a == null)) continue;
        const currencies = new Set(terms.map(t => t[2].toUpperCase()));
        if (currencies.size < 2) continue; // same currency twice isn't a blend
        const sum = amounts.reduce((a, b) => a + b, 0);
        // 0.02 tolerance absorbs decimal-string rounding, not a real mismatch —
        // a genuine coincidence (unrelated numbers that happen to add up) is
        // vanishingly unlikely given two independent currency-tagged terms.
        if (Math.abs(sum - outer) > 0.02) continue;
        blends.push(`${m[1].trim()} (${m[2].trim()})`);
      }
      if (!blends.length) return;

      logger.warn(`[verifyCurrencyClaims] blended-currency total(s) detected: ${blends.join("; ")}`);
      const list = blends.map(b => `\`${b}\``).join(", ");
      const warning =
        `\n\n⚠️ **Correction:** ${list} adds amounts in different currencies into ` +
        `one figure — that total is not valid, it implies a 1:1 exchange rate that ` +
        `was never applied. Please disregard it; only the separate per-currency ` +
        `totals are correct.`;
      emitter.send({ type: "stream_start" });
      emitter.send({ type: "token", text: warning.trimStart() });
      emitter.send({ type: "stream_end", text: warning.trim() });
