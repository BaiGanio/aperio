...
const UNTRUSTED_CONTENT_TOOLS = new Set([
  "fetch_url", "fetch_github_issue", "read_file", "read_docx", "scan_project",
]);
...
export function createToolHooks({
  ...
  return function makeTurnHooks(emitter, turnStartMs) {
    ...
    async function callToolHooked(name, input) {
      ...
      const isPendingConfirm = typeof result === "string" && /\bToken:\s*(?:iss|del|wr)_[a-z0-9]+\b/i.test(result);
      ...
      if (typeof result === "string") {
        const tokenMatch = result.match(/\bToken:\s*((?:iss|del|wr)_[a-z0-9]+)\b/i);
