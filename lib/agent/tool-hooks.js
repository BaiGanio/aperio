...
export function createToolHooks({
   ...
   */
  return function makeTurnHooks(emitter, turnStartMs, artifactContext = null) {
    let toolSeq = 0;
    ...
    const safetyRunner = createLifecycleRunner(safety.middleware);
    ...
    function finalizeToolResult(name, result) {
      if (!offloadToolResult || !artifactContext) return result;
      try {
        const offloaded = offloadToolResult(result, {
          ...artifactContext,
          toolName: name,
        });
        for (const artifact of offloaded.artifacts) {
          ...
          logger.info(
            `[tool-result-offload] tool=${name} artifact=${artifact.id} ` +
        ...
        return offloaded.result;
      } catch (err) {
        // Fail open: context pressure is preferable to silently losing a tool
        // result when local storage is unavailable.
        logger.warn(`[callToolHooked] result offload failed for ${name}: ${err.message}`);
        return result;
      }
    }
