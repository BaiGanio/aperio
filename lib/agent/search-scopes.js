...
// Three shapes of absolute path: drive-rooted (C:\x), separator-rooted Windows and
// UNC (\srv\share, \\srv\share) and POSIX (/x). The Windows-rooted alternative
// deliberately requires two or more segments so a stray backslash in prose ("\n")
// is not mistaken for a scope path — under-matching leaves the scope ignored, while
// over-matching would point the agent at the wrong folder.
const PATH_PATTERN = /(?:[A-Za-z]:[\\/][^\s,;]+|\\{1,2}[^\s,;\\/]+(?:[\\/][^\s,;]+)+|\/[\w./-]+)/;
