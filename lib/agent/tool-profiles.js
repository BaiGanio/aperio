...
// Below this served window (tokens), the full tool-schema set — re-sent every
// turn because LLM APIs are stateless — crowds out the tool result and reasoning
// the model needs to answer. It then trims the result and re-issues the same
// call in a loop (observed on gemma4:12b, whose 1.5 MB/token KV cache caps the
// window at ~6k on 32 GB). Cap the attached tools to a small budget, keeping
// THIS turn's intent tools (what the user asked for) over the always-on
// memory/self/data baseline. Larger windows are never touched.
...
export function capToolsForWindow(names, contextWindow, { schemaTokenCosts } = {}) {
  if (!contextWindow || contextWindow > SMALL_WINDOW_TOKENS) return names;
  const arr = [...names];
  ...
  const maxTools = Math.max(SMALL_WINDOW_MAX_TOOLS, Math.floor(contextWindow / 1024));
...
export function classifyProfiles(text) {
  ...
  const fileEditIntent = /\b(write|edit|modify|append|save|create|new file|rename)\b/.test(t);
  const explicitFileTarget = /\b(file|files|path|folder|directory|disk|locally|markdown|\.md)\b/.test(t);
