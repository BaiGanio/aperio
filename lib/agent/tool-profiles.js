...
// — unlike the small-window tunables above, `Number(...) || 3` would silently
// replace an explicit 0 with the default, so this needs real finite-number
// validation instead of relying on falsiness. Must also be a whole number
// (P2 review finding): turn-planner.js's `sinceLastToolUse < TOOL_PIN_TURNS`
// check advances one whole turn at a time, so a fractional value like 1.5
// doesn't mean "1.5 turns" — it silently pins for 2 follow-up turns (both
// 0 < 1.5 and 1 < 1.5), a different count than anything the config actually
// asked for. `Number.isInteger` also already excludes NaN/Infinity, so it
// subsumes the old finite check.
