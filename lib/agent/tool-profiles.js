...
import { CONFIRMABLE_TOOLS } from "../helpers/confirmableTools.js";
...
// Imported from confirmableTools.js so the emit side (this module) and the
// accept side (WS handler + HTTP interrupts) share the same canonical Set.
export const CONFIRM_TOOLS = CONFIRMABLE_TOOLS;
// The file-*persistence* subset of TOOL_PROFILES["file-edit"]/["file-generate"]
// below — deliberately narrower than either full profile (excludes read_file/
// read_docx/syntax_check/run_node_script/run_python_script, none of which
// write a file themselves). Used by checkNoToolUse (turn-diagnostics.js) to
// tell whether the model actually had a file-write tool in hand this turn,
// which must be checked against the turn's FINAL attached tool-name set
// (post allowlist/schema-budget capping), not the pre-cap profile plan —
// a profile can be selected and then have every one of its tools capped
// away, in which case the model genuinely had nothing to write with.
export const MUTATION_FILE_TOOLS = new Set([...WRITE_TOOLS, "generate_xlsx", "generate_docx"]);
...
// Marker for the synthetic assistant tool_use messages preflight.js injects
// (doc_repos/doc_manifest/doc_batch auto-execution). finalizeTurnTools
// (lib/agent/index.js) withholds those tools' schemas from the very request
// they were pre-executed for — preExecutedTools is a fresh Set every turn, so
// the schema omission never reaches the next turn. But the synthetic
// `{ role: "assistant", content: [{ type: "tool_use", ... }] }` message DOES
// persist in history, and turn-planner.js's sticky fold (toolNamesCalled)
// scans exactly that shape to reconstruct what a historical turn's request
// actually included. Left unmarked, it would pin schemas into a follow-up
// turn's tool array that llama-server never actually cached for that
// historical turn — defeating the prefix-stability the pin mechanism exists
// to protect (P2 review finding). A Symbol keeps it off the wire, same as
// SYNTHETIC_USER.
export const PREFLIGHT_TOOL_USE = Symbol("aperio.preflight_tool_use");

// Drops the schemas of tools preflight already auto-executed this turn, so the
// model isn't offered a tool whose result it has already been handed. Pure and
// exported (rather than a closure in index.js) so the `keepStable` escape
// hatch below is directly testable.
//
// `keepStable` disables the omission entirely. llama.cpp's Jinja tool-calling
// templates render the tools block at the very front of the prompt, so
// removing a schema mid-conversation shifts everything after it and
// invalidates the whole KV prefix. Measured on the 2026-08-13 WS2 T-G2.3 gate:
// on the two turns preflight fired (withholding exactly doc_manifest +
// doc_batch, 40 schemas → 38), llama-server reported `cache_n=12577` against
// `prompt_n=31067` and **262 s of prefill on one turn** — which is what pushed
// that turn past the 600 s per-turn ceiling and failed the gate. The omission
// is a nicety; the cache loss is not. A model that re-calls the pre-executed
// doc_batch pays one dedup-cached round trip (tens of ms) instead.
export function filterPreExecutedTools(tools, kind, preExecutedTools, { keepStable = false } = {}) {
  if (keepStable || !preExecutedTools?.size) return tools;
  if (kind === "gemini") {
...
export const TOOL_PROFILES = {
  ...
  codegraph: new Set(["code_repos", "code_search", "code_outline", "code_context", "code_callers", "code_callees", "code_neighbors", "code_path", "code_insights"]),
  ...
  docgraph: new Set(["doc_search", "doc_repos", "doc_manifest", "doc_batch", "doc_outline", "doc_context", "doc_refs"]),
  ...
  database:      new Set(["db_connections", "db_schema", "db_query", "db_execute", "db_normalize_amount"]),
  // Document Intelligence WS3 (issue #250): recognizes and reuses learned
  // document shapes on top of docgraph/database — a document that matches no
  // template still falls through to the ad-hoc db_execute flow, so this
  // profile is loaded alongside docgraph/database, never instead of them.
  extraction:    new Set([
    "extraction_template_list", "extraction_template_get", "extraction_template_match",
    "extraction_template_propose", "extraction_template_delete",
...
// Multi-turn tool-set stability (llamacpp-multiturn-latency plan, Step 2):
// once a turn's assistant response actually calls a tool, turn-planner.js's
// computeStickyProfiles keeps unioning (never dropping) tool profiles for up
// to this many follow-up turns before falling back to a fresh, narrow
// classification. A stable, growing tool-schema array lets llama-server reuse
// its prompt/KV-cache prefix across a flow; Step 1's probe measured that even
// a single added/removed tool fully defeats that cache (0% hit).
//
// `0` is a valid, meaningful value here (disable follow-up pinning entirely)
// — unlike the small-window tunables above, `Number(...) || default` would
// silently replace an explicit 0 with the default, so this needs real
// finite-number validation instead of relying on falsiness. Must also be a
// whole number (P2 review finding): turn-planner.js's
// `sinceLastToolUse < TOOL_PIN_TURNS` check advances one whole turn at a
// time, so a fractional value like 1.5 doesn't mean "1.5 turns" — it
// silently pins for 2 follow-up turns (both 0 < 1.5 and 1 < 1.5), a
// different count than anything the config actually asked for.
// `Number.isInteger` also already excludes NaN/Infinity, so it subsumes the
// old finite check.
//
// Default raised 3 → 8 (2026-08-13, tech-debt.md "Tool profiles / schema
// budgeting"): the T-L4.3 gemma4 run root-caused the dominant multi-minute
// turn latency in the llamacpp-multiturn-latency epic to this exact
// mechanism's periodic reset — once `sinceLastToolUse` reaches the pin
// count, the carried tool set intentionally narrows, and any schema-set
// change collapses llama-server's prefix-cache reuse (observed ~99% -> ~20%
// sim_best), forcing a near-full reprocess of the growing conversation. A
// longer pin window means resets happen far less often for a typical
// conversation. This is a mitigation, not a fix — the reset itself still
// exists and will still bust the cache when it fires — and growth is still
// bounded independently by capToolsForWindow's own 20%-of-context-window
// schema-token budget (and, on small windows, its tool-count cap), which
// apply every turn regardless of TOOL_PIN_TURNS.
export function parsePinTurns(raw) {
  if (raw === undefined || raw === "") return 8;
  const n = Number(raw);
  return Number.isInteger(n) && n >= 0 ? n : 8;
}
export const TOOL_PIN_TURNS = parsePinTurns(process.env.APERIO_TOOL_PIN_TURNS);

// Same shape as parsePinTurns, different default and a different thing being
// pinned — see computeStickySkills in turn-planner.js. Skills are a heavier,
// user-visible injection than tool schemas, so this pins for fewer turns than
// TOOL_PIN_TURNS: long enough to carry a workflow across its own follow-ups,
// short enough that a stale skill cannot outlive the flow that earned it.
export function parseSkillPinTurns(raw) {
  if (raw === undefined || raw === "") return 4;
  const n = Number(raw);
  return Number.isInteger(n) && n >= 0 ? n : 4;
}
export const SKILL_PIN_TURNS = parseSkillPinTurns(process.env.APERIO_SKILL_PIN_TURNS);
...
//
// Within the intent bucket, `currentTurnNames` (when given) splits it further
// into this-turn-relevant vs. carried-over-from-a-pinned-flow (TOOL_PIN_TURNS)
// tools, with this turn's own tools prioritized first. This guarantees a
// genuine topic pivot is never silently budget-truncated in favor of stale
// intent from earlier in a multi-turn flow — correctness of tool availability
// must not be traded for cache stability. Falls back to plain insertion order
// (today's behavior) when the caller doesn't distinguish the two.
export function capToolsForWindow(names, contextWindow, { schemaTokenCosts, currentTurnNames } = {}) {
  ...
  const intentCurrent = currentTurnNames ? intent.filter(n => currentTurnNames.has(n)) : intent;
  const intentStale   = currentTurnNames ? intent.filter(n => !currentTurnNames.has(n)) : [];
  const ordered = [...first, ...intentCurrent, ...intentStale, ...core];
  ...
  for (const name of ordered) {
    ...
    if (Number.isFinite(cost) && cost > 0 && usedSchemaTokens + cost > schemaBudget) break;
    selected.push(name);
...
// Estimated per-tool schema-token cost, used by capToolsForWindow's budget.
// Deliberately conservative: local tokenizers often split JSON/tool schemas
// more densely than the usual four-characters-per-token rule.
export function computeSchemaTokenCosts(names, openaiByName) {
...
export function filterVisionTools(names, { hasInlineImage, standaloneVision, providerIsLocal, modelHandlesInlineImage }) {
  const result = new Set(names);
  if (!(providerIsLocal && hasInlineImage && modelHandlesInlineImage)) return result;
  result.delete("read_image");
  result.delete("preprocess_image");
  result.delete("describe_image");
  ...
  if (standaloneVision) result.clear();
  return result;
}
...
export function filterSelfMemoryTools(names, { providerIsLocal }) {
  if (providerIsLocal) return new Set(names);
  const result = new Set(names);
  for (const n of SELF_MEMORY_TOOLS) result.delete(n);
  for (const n of SELF_WIKI_TOOLS) result.delete(n);
  return result;
}
...
// Money questions are often document questions without saying "document".
// Keep this narrow: an amount/expense term must be paired with aggregation or
// period language, so ordinary financial conversation is not routed to the
// document graph by accident. Money + aggregate alone still matches generic,
// non-personal questions like "How much does fuel cost each month?" — a third
// personal/corpus cue (first-person reference, or an explicit indexed-folder
// mention) distinguishes "tell me about my records" from "tell me a fact".
export function isDocumentAggregationIntent(text = "") {
  const t = String(text).toLowerCase();
  const money = /\b(utilities?|bills?|expenses?|spend(?:ing|s)?|spent|paid|pay|cost|amount|receipt|invoice|fuel|grocer(?:y|ies)|transport|internet)\b/.test(t);
  const aggregate = /\b(how much|how many|total|sum|aggregate|all|each|monthly|month|last month|category|categories)\b/.test(t);
  const personal = /\b(i|my|mine|our|ours|we|me)\b/.test(t) || /\bindexed\b/.test(t);
  return money && aggregate && personal;
}

function hasDocumentGraphIntent(text = "") {
  const t = String(text).toLowerCase();
  const explicitDocTool = /\bdoc(?:_|[\s-])(search|repos?|outline|context|refs|manifest|batch)\b/.test(t);
  ...
  return explicitDocTool || isDocRepoInventoryIntent(t) || isDocumentAggregationIntent(t) || (corpusRetrieval && !explicitWeb);
}

// Document Intelligence WS3 (issue #250): recognize/reuse a learned document
// shape. Primary trigger is the SAME signal that already unlocks docgraph for
// financial-document turns (isDocumentAggregationIntent) — WS3 is explicitly
// a layer on top of that same document-aggregation workflow, not a separate
// one, so it activates alongside docgraph rather than needing its own
// unrelated heuristic. Explicit template/extraction language is included
// too, for a direct "learn this bill's shape" request that might not carry
// aggregate phrasing ("how much", "total", ...) at all.
function hasExtractionIntent(text = "") {
  const t = String(text).toLowerCase();
  const explicitExtractionTool = /\bextraction_(?:template|apply|log)\b/.test(t);
  const explicitTemplateLanguage =
    /\b(extract(?:ion)?|template)\b[\s\S]{0,80}\b(bill|invoice|receipt|statement|document|field|shape)\b/.test(t) ||
    /\b(bill|invoice|receipt|statement|document)\b[\s\S]{0,80}\b(extract(?:ion)?|template|fields?)\b/.test(t);
  return explicitExtractionTool || explicitTemplateLanguage || isDocumentAggregationIntent(t);
}
...
export function classifyProfiles(text) {
  ...
  // extraction's own documented lifecycle (extraction_apply → db_execute →
  // extraction_log_record) is unreachable in the same turn without
  // database's db_execute — extraction_log_record even requires a genuine
  // db_execute_token as of the round-3 review fix. An aggregation-only
  // prompt ("how much did I spend on utilities...") carries no SQL/database
  // keyword of its own, so database must be added explicitly here rather
  // than left to its own separate keyword regex below.
  if (hasExtractionIntent(t)) { active.add("extraction"); active.add("database"); }
  // Edits / writes / creation. "Write/create a function" describes answer
  // content, not where to persist it; only offer filesystem mutation when the
  // user names a file-like target or uses an unambiguous persistence verb.
  const namedFileTarget = /\b[\w./-]+\.(?:js|jsx|ts|tsx|cjs|mjs|py|rb|go|rs|java|kt|swift|php|scala|c|cc|cpp|h|hpp|cs|sh|bash|sql|css|scss|less|vue|svelte|html|svg|md|txt|json|ya?ml|toml|csv|tsv)\b/.test(t);
  const explicitFileTarget =
    /\b(file|files|path|folder|directory|disk|locally|markdown|\.md|csv|tsv|\.csv|\.tsv)\b/.test(t) ||
    namedFileTarget;
  const explicitPersistenceIntent = /\b(edit|modify|append|save|rename)\b/.test(t);
  const ambiguousCreationIntent = /\b(write|create|new file)\b/.test(t);
  const fileEditIntent =
    explicitPersistenceIntent ||
    (ambiguousCreationIntent && explicitFileTarget) ||
  ...
  // A bare "run"/"execute"/"test"/"verify"/"convert"/"qa" alongside database
  // intent is ordinary SQL language ("run this query", "execute the SELECT",
  // "verify the total"), not a shell request — the same false-positive class
  // the docGraph narrowing below already guards against, extended here since
  // database questions hit it just as often and docGraphIntent is false for
  // them. Keep `shell` only when a stronger, unambiguous shell/QA signal is
  // also present. Recorded live: "run SELECT category, currency,
  // SUM(amount) GROUP BY category, currency against the extraction table"
  // added `shell` on a plain SQL follow-up, perturbing the tool-schema set
  // turn to turn and defeating llama.cpp prompt-cache reuse (open tech debt,
  // "Tool profiles / schema budgeting").
  if (active.has("database") &&
      !/\b(command|terminal|render|grep|libreoffice|soffice|pdftoppm|thumbnail|pptx|slide|slides|presentation|powerpoint|deck|xlsx|spreadsheet)\b/.test(t)) {
    active.delete("shell");
  }
  ...
  if (docGraphIntent) {
    ...
    if ((docRepoInventoryIntent || isDocumentAggregationIntent(t)) && !/\b(project|codebase|repos?|repositories|scan|tree|grep)\b/.test(t)) {
...
const RETRIEVAL_RE = /\b(do you (remember|recall)|what do you (know|remember)|do i have (any|a |an )|did i (tell|say|mention|ask)|have we (discuss|talk|spoke|cover|mention)|recall (any|my|the|some|memor)|any (memor|notes?|reminders?|records?)|what(?:'s| is| are) (stored|saved|in (memory|your memory)))\b/i;

// Bare "remember ..." (no "do you") is structurally ambiguous: the same
// phrasing is used both to ask a retrieval question ("remember when we
// discussed the trip?") and to issue a save command ("Remember that I like
// tea.", "... Remember that."). Observed 2026-07-22: a save-command turn
// ending in "Remember that." matched this branch, triggered the auto-recall
// override instead of leaving `remember` for the model to call, and derailed
// the whole turn. Only treat the bare form as retrieval when it is phrased as
// a question — genuine save commands don't end in "?".
const BARE_REMEMBER_QUESTION_RE = /\bremember (any|that|when|my|if|what)\b[\s\S]{0,60}\?/i;

export function isRetrievalQuestion(text) {
  if (!text || text.length > 400) return false;
  return RETRIEVAL_RE.test(text) || BARE_REMEMBER_QUESTION_RE.test(text);
}
