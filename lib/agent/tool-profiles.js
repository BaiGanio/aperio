...
export const SMALL_WINDOW_TOKENS    = Number(process.env.APERIO_SMALL_WINDOW_TOKENS)    || 8192;
...
export function capToolsForWindow(names, contextWindow) {
  if (!contextWindow || contextWindow >= SMALL_WINDOW_TOKENS) return names;
  if (names.size <= SMALL_WINDOW_MAX_TOOLS) return names;
  const arr = [...names];
  ...
  return new Set([...first, ...intent, ...core].slice(0, SMALL_WINDOW_MAX_TOOLS));
}
