...
async function connectMcp({ root, clientName, version, provider }) {
  const providerIsLocal = isLocalProvider(provider.name);
  const transport = new StdioClientTransport({
    ...
    env: {
      ...
      APERIO_PROVIDER_LOCAL: providerIsLocal ? "1" : "0",
    },
    stderr: "pipe",
  });
  ...
  try {
    ...
    return { mcp, tools, providerIsLocal };
  } catch (err) {
...
function registerHostTools(mcpTools, hostTools) {
  ...
  for (const tool of hostTools) {
    ...
    const mcpTool = mcpTools.find((candidate) => candidate.name === tool.name);
    if (hostToolHandlers.has(tool.name) || (mcpTool && tool.override !== true)) {
      throw new Error(`Duplicate tool name: ${tool.name}`);
    }
    // Overrides inherit the MCP tool's authoritative schema. If a test fixture
    // or reduced MCP server does not advertise that tool, do not invent it.
    if (tool.override === true && !mcpTool) continue;
 ...
 */
export async function createToolCatalog({ root, clientName, version, provider, hostTools, toolAllowlist, mcpConnection = null }) {
  const providerIsLocal = isLocalProvider(provider.name);
  if (mcpConnection && mcpConnection.providerIsLocal !== providerIsLocal) {
    throw new Error("Cannot share an MCP connection across the local/cloud privacy boundary.");
  }
  const connection = mcpConnection ?? await connectMcp({ root, clientName, version, provider });
  const { mcp, tools } = connection;
  ...
  const geminiDeclsAll = mcpTools.map(t => ({ name: t.name, description: t.description, parameters: zodToJsonSchema(t.inputSchema, { forGemini: true }) }));

  return {
    mcp,
    mcpConnection: connection,
    ownsMcpConnection: !mcpConnection,
    mcpTools,
    hostToolHandlers,
    toolSchemas,
    allowedToolNames,
