 ...
 *
 * Both functions are pure and never expose raw payloads in bulk: the argument
 * summary picks one salient field and caps it (generously for commands/paths so
 * they stay readable, like a terminal), and the result summary reports a
 * size/count/status rather than the content. This keeps the WebSocket frames
 * small and avoids leaking page bodies or memory text into the browser DOM.
 */
...
export function summarizeArgs(name, args) {
  ...
  if (path) return truncMid(path, 120);
