...
import { countTokens } from "../memory/tokenCount.js";
...
const TEXT_READ_TOOLS = new Set(["read_file", "read_docx", "doc_batch", "doc_context", "read_artifact"]);
...
// This uses Aperio's GPT tokenizer as a useful estimate. The active provider
// may tokenize the same text slightly differently, so keep the approximation
// marker visible in the card.
function formatReadResultSize(name, text) {
  return `${formatBytes(Buffer.byteLength(text, "utf8"))} · ~${formatTokens(countTokens(text))} tok`;
}
...
export function summarizeArgs(name, args) {
  ...
  if (name === "run_shell" && Array.isArray(args.steps)) {
    const chain = args.steps
      .filter(step => step && typeof step.program === "string")
      .map(step => [step.program, ...(Array.isArray(step.args) ? step.args : [])].join(" "))
      .join(" → ");
    if (chain) return trunc(chain, 160);
  }
...
/**
 * { ok, summary, detail? } describing the outcome (the "what happened") for
 * the tool activity card. The full envelope the card actually receives is
 * this plus an optional `artifact: {id, tokenCount, byteCount}` — the pointer
 * to a result offloaded to the artifact store — which the caller (tool-hooks.js,
 * the only place that knows whether *this* call was offloaded) merges in,
 * since offload status isn't recoverable by re-parsing `result` text.
 */
export function summarizeResult(name, result) {
  ...
  if (TEXT_READ_TOOLS.has(name)) {
    return withDetail(text, { ok: true, summary: formatReadResultSize(name, text) });
  }
  if (name === "fetch_url")
