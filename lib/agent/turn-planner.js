...
function computeStickySkills(messages, skillIndex) {
  if (!SKILL_PIN_TURNS || !skillIndex?.length) return [];
  const turns = realUserTurns(messages);
  const historical = turns.slice(0, -1);
  let carried = [];
 ...
 * notFound, hasInlineImage, standaloneVision }. `names` is NOT yet capped to a
 ...
 * — every other provider gets the pre-Step-2 per-turn classification.
...
export function planTurnTools(messages, userText, {
  ...
  // always outranks a skill still riding a pinned flow (see computeStickySkills).
  const carriedSkills = currentIsSynthetic ? [] : computeStickySkills(messages, skillIndex);
  for (const s of [...alwaysOn, ...matched, ...semanticMatched, ...carriedSkills]) {
  ...
  return { turnNum, profiles, names, currentTurnNames, skills, notFound, hasInlineImage, standaloneVision };
}
