...
export async function runAnthropicLoop(messages, emitter, opts = {}, ctx) {
  ...
  while (true) {
    ...
    try {
      ...
      stream = provider.client.messages.stream({ model: provider.model, max_tokens: 8192, system: getSystemPrompt(lastUserText, opts.lang, opts.extraSystem, messages), tools, messages: redactMessages(trimmed) });
    } catch (e) {
