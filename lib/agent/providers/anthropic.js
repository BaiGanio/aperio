...
import { estimateMsgTokens, trimByTokens } from "../../context/trim.js";
...
export async function runAnthropicLoop(messages, emitter, opts = {}, ctx) {
  ...
  while (true) {
    ...
    const stream = provider.client.messages.stream({ model: provider.model, max_tokens: 8192, system: getSystemPrompt(lastUserText, opts.lang, opts.extraSystem), tools, messages: trimmed });
