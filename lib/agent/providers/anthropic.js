...
export async function runAnthropicLoop(messages, emitter, opts = {}, getAbort = () => null, setAbort = () => {}, ctx) {
  const { provider, callTool, getSystemPrompt, getAnthropicTools, prepareModelContext } = ctx;
  let tokenHWM = 0;
  ...
  while (true) {
    ...
    try {
      ...
      stream = provider.client.messages.stream({ model: provider.model, max_tokens: 8192, system: systemPrompt, tools, messages: redactMessages(trimmed) }, { signal: controller.signal });
    } catch (e) {
