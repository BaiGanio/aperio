...
export async function runAnthropicLoop(messages, emitter, opts = {}, ctx) {
  ...
  while (true) {
    ...
    let stream;
    try {
      stream = provider.client.messages.stream({ model: provider.model, max_tokens: 8192, system: getSystemPrompt(lastUserText, opts.lang, opts.extraSystem), tools, messages: trimmed });
    } catch (e) {
