...
export async function runAnthropicLoop(messages, emitter, opts = {}, ctx) {
  ...
  while (true) {
    ...
    // Trimming (token-pressure drops or the MAX_HISTORY window) can begin on a
    // user message whose tool_result's matching assistant tool_use was cut off.
    // The API rejects such orphans ("unexpected tool_use_id"), so scrub them —
    // same guard gemini/ollama/deepseek already apply.
    ...
    const tools = getAnthropicTools(lastUserText, trimmed);
    let stream;
    try {
      // PRIVACY-01: scrub secrets from the outgoing (derived) array; the
      // persistent `messages` history is left intact.
      stream = provider.client.messages.stream({ model: provider.model, max_tokens: 8192, system: getSystemPrompt(lastUserText, opts.lang, opts.extraSystem, trimmed), tools, messages: redactMessages(trimmed) });
    } catch (e) {
