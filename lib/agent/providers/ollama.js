...
export async function runOllamaLoop(messages, emitter, opts = {}, getAbort = () => null, setAbort = () => {}, ctx) {
  ...
  if (mainIsVision) {
    // VLM-only models (qwen2.5vl, llava…) reject the `tools` param; full
    // multimodal models (gemma4…) keep tools. Either way the bridge is skipped
    // because the model sees images natively. Only the tool-suppression
    // transition is worth logging — and only once, not every turn.
  ...
  while (true) {
    ...
    let systemPrompt = getSystemPrompt(lastUserText, opts.lang, opts.extraSystem, trimmed);
    ...
    const ollamaTools = getOllamaTools(lastUserText, trimmed);
    ...
    const suppressThinking = forceSuppressThinking;
    let response;
    ...
    if (corruptToolCall) {
      ...
      if (leakRetries < 1) {
        ...
        logger.warn(`[agent] corrupted tool name from model="${provider.model}" — retrying with nudge`);
        continue;
      }
      logger.error(`[agent] corrupted tool name persisted after retry from model="${provider.model}"`);
