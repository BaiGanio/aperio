...
export async function runLlamaCppLoop(messages, emitter, opts = {}, getAbort = () => null, setAbort = () => {}, ctx) {
  ...
  while (true) {
    ...
    let trimmed, systemPrompt, messagesNoSkills, llamacppTools;
    if (prepareModelContext) {
      ...
      messagesNoSkills = prepared.messagesNoSkills ?? null;
    ...
    // can inject thousands of prompt tokens (living in the tail message, not
    // the system prompt), and on a small served context llama.cpp rejects
    // the whole request (400 exceed_context_size_error) rather than
    // trimming. Skills are an enhancement, not a correctness requirement —
    // rebuild without the tail block and re-fit before sending a request we
    // already know is doomed.
    if (fitted.estimatedTokens > requestBudget && messagesNoSkills) {
      openaiMessages = toOpenAIMessages(messagesNoSkills, systemPrompt, provider.vision);
    ...
    if (LOG_CACHE_FINGERPRINT) {
      ...
      logger.info(`[llamacpp] fingerprint sysBytes=${sysMsg.length} sysHash=${sysHash} toolsBytes=${toolsJson.length} toolsHash=${toolsHash} toolCount=${(llamacppTools ?? []).length} msgCount=${openaiMessages.length}`);
    }
