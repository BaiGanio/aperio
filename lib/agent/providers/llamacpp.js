...
export async function runLlamaCppLoop(messages, emitter, opts = {}, getAbort = () => null, setAbort = () => {}, ctx) {
  ...
  while (true) {
    ...
    // can inject thousands of prompt tokens (now living in the tail message,
    // not the system prompt — see prompt-cache-tail-relocation), and on a
    // small served context llama.cpp rejects the whole request (400
    // exceed_context_size_error) rather than trimming. Skills are an
    // enhancement, not a correctness requirement — rebuild without the tail
