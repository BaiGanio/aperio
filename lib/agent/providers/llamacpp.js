...
import { ToolExecutor, extractTextToolCall, detectToolCallLeak, recoverToolName, looksLikeSystemPromptEcho, countTrailingRepeatedToolCalls, resolveTurnStepLimit, TOOL_REPEAT_LIMIT, TOOL_REPEAT_NUDGE, TURN_STEP_NUDGE } from "../../tools/executor.js";
...
import { bridgeImagesToVLM, isStandaloneVisionRequest } from "../../helpers/imageBridge.js";
import { modelCapabilitiesSync } from "../../providers/model-capabilities.js";
import { DEFAULT_LOCAL_MODEL } from "../../providers/index.js";
import { fetchServerModels } from "../../helpers/llamacpp/models.js";
import { LLAMACPP_MAIN_ALIAS, LLAMACPP_VLM_ALIAS } from "../../helpers/llamacppAliases.js";
...
import { isExplicitWikiWriteIntent } from "../tool-profiles.js";
...
// Off by default (logger.debug never surfaces — level is hardcoded 'info' in
// lib/helpers/logger.js). Opt-in diagnostic for the llamacpp-multiturn-latency
// investigation (tests/docint/harness/):
// hashes the exact system-prompt/tools bytes sent per request so a "same
// schema count, still cache_n=0" case (2026-08-13 T-L4.2 run) can be checked
// against whether the outgoing prompt was actually byte-identical, not just
// same-sized, request to request. Since round 6 it also emits `msgprint`, a
// per-message hash+length list — round 5 proved the divergence is in the
// message array, so the system/tools hashes alone can no longer localise it.
const LOG_CACHE_FINGERPRINT = process.env.APERIO_LOG_CACHE_FINGERPRINT === "on";

// llama-server never tokenizes image payloads as text — the mmproj encodes each
// image into a bounded patch budget. Counting the base64 data URL as prose put
// a single screenshot at ~1M "tokens", which popped tool schemas down to the
// floor and spuriously dropped skill prompts. Estimate images at a flat
// per-image allowance instead.
const IMAGE_TOKEN_ESTIMATE = 1024;
function estimateRequestTokens(value) {
  let images = 0;
  const json = JSON.stringify(value).replace(/data:[^"]*;base64,[^"]*/g, () => {
    images++;
    return "<image>";
  });
  return encode(json).length + images * IMAGE_TOKEN_ESTIMATE;
}
...
// Final safety net for small local windows. Normal sizing should leave the
// stable Aperio identity intact, but an explicit tiny LLAMACPP_SERVE_CTX or a
// larger customized identity can still make the first request impossible.
// Preserve the identity's beginning and provider-specific tail while removing
// its middle; sending a compact prompt is strictly better than knowingly
// sending a request llama.cpp will reject before generation.
export function compactSystemPromptToContext(messages, tools, contextWindow, {
  estimateTokens = estimateRequestTokens,
  headroomRatio = REQUEST_HEADROOM_RATIO,
  minimumSystemTokens = 256,
} = {}) {
  const estimatedTokens = estimateTokens({ messages, tools: tools ?? [] });
  const budget = Math.floor(contextWindow * headroomRatio);
  const systemIndex = messages.findIndex(message =>
    message?.role === "system" && typeof message.content === "string"
  );
  if (estimatedTokens <= budget || systemIndex < 0) {
    return { messages, estimatedTokens, removedTokens: 0 };
  }

  const withoutSystem = messages.map((message, index) =>
    index === systemIndex ? { ...message, content: "" } : message
  );
  const nonSystemTokens = estimateTokens({ messages: withoutSystem, tools: tools ?? [] });
  const allowance = Math.max(minimumSystemTokens, budget - nonSystemTokens - 128);
  const original = encode(messages[systemIndex].content);
  if (original.length <= allowance) {
    return { messages, estimatedTokens, removedTokens: 0 };
  }

  const marker = encode("\n\n… [system prompt compacted to fit this local context window] …\n\n");
  const contentBudget = Math.max(32, allowance - marker.length);
  const headCount = Math.ceil(contentBudget * 0.72);
  const tailCount = Math.max(0, contentBudget - headCount);
  const compacted = [
    ...original.slice(0, headCount),
    ...marker,
    ...(tailCount ? original.slice(-tailCount) : []),
  ];
  const fittedMessages = messages.map((message, index) =>
    index === systemIndex ? { ...message, content: decode(compacted) } : message
  );
  return {
    messages: fittedMessages,
    estimatedTokens: estimateTokens({ messages: fittedMessages, tools: tools ?? [] }),
    removedTokens: original.length - contentBudget,
  };
}
...
// Per-request prompt fingerprint for the KV-reuse investigation. Deliberately
// lives inside makeLlamaCppRequest rather than at a call site: T-G2.3 round 5
// saw a request reach the server with no fingerprint line at all, which left
// the slot in a state nothing in the logs accounted for. Every request path
// (normal, retry, warm-up) funnels through here, so none can be missed.
//
// `msgprint` is the round-6 addition. Round 5 established that reuse stops at
// `sysBytes+toolsBytes` on a collapsed request — i.e. somewhere in the message
// array — but could not name the message. Hashing each outgoing message with
// its byte length identifies both the message that changed and whether it grew,
// shrank, or moved index. Hashes and lengths only: no message content is
// logged, so this stays safe to switch on against a real conversation.
let requestSeq = 0;
function logRequestFingerprint(llamacppTools, openaiMessages, noTools) {
  const seq = ++requestSeq;
  const sysMsg = openaiMessages[0]?.role === "system" ? String(openaiMessages[0].content ?? "") : "";
  // Hashed over llamacppTools regardless of noTools so the values stay
  // comparable with the run history already recorded against them; `noTools`
  // is logged separately because it omits the tools block from the wire body
  // (see the request body below) and so changes the rendered prefix.
  const toolsJson = JSON.stringify(llamacppTools ?? []);
  const sysHash = createHash("sha1").update(sysMsg).digest("hex").slice(0, 12);
  const toolsHash = createHash("sha1").update(toolsJson).digest("hex").slice(0, 12);
  logger.info(`[llamacpp] fingerprint req=${seq} noTools=${noTools ? 1 : 0} sysBytes=${sysMsg.length} sysHash=${sysHash} toolsBytes=${toolsJson.length} toolsHash=${toolsHash} toolCount=${(llamacppTools ?? []).length} msgCount=${openaiMessages.length}`);
  const parts = openaiMessages.map((message, index) => {
    const body = JSON.stringify(message) ?? "";
    const hash = createHash("sha1").update(body).digest("hex").slice(0, 8);
    return `${index}:${message?.role ?? "?"}:${hash}:${body.length}`;
  });
  logger.info(`[llamacpp] msgprint req=${seq} ${parts.join(" ")}`);
}

async function makeLlamaCppRequest(provider, llamacppTools, openaiMessages, noTools, suppressThinking, setAbort, maxTokens) {
  if (LOG_CACHE_FINGERPRINT) logRequestFingerprint(llamacppTools, openaiMessages, noTools);
...
// Prompt-cache warm-up: fires a minimal, invisible chat-completion carrying
// the real system prompt so llama-server prefills and caches it before the
...
export async function runLlamaCppLoop(messages, emitter, opts = {}, getAbort = () => null, setAbort = () => {}, ctx) {
  ...
  // Analyze user images with the local vision model (whichever model is
  // acting as the eyes — the main model itself when it has native vision,
  // otherwise the dedicated bridge model, always defaultLocalModel()) before
  // the main model. This keeps visual grounding separate from tool
  // orchestration even when the selected main model can also see images.
  const mainCaps = modelCapabilitiesSync(provider.model);
  if (!mainCaps.tools && !state.noTools) {
    state.noTools = true;
    logger.info(`[agent] model "${provider.model}" has no tool-calling chat template — suppressing tools`);
  }
  ...
  let mainHasVision = mainCaps.vision;
  if (hasRawUserImage && !mainHasVision) {
    // mainCaps is disk-only (modelCapabilitiesSync) and can lag behind what
    // buildModelsPreset() already committed to at boot via the async,
    // Hub-aware modelCapabilities() — for an uncached native-vision model,
    // the preset can already omit aperio-vlm on Hub metadata alone, before
    // the weights/projector finish downloading and disk agrees. Ask the
    // router what it actually loaded: if aperio-vlm doesn't exist, the
    // bridge below cannot work regardless, and aperio-main is the only route
    // that can see this image.
    const serverModels = await fetchServerModels();
    if (serverModels?.includes(LLAMACPP_MAIN_ALIAS) && !serverModels?.includes(LLAMACPP_VLM_ALIAS)) {
      mainHasVision = true;
    }
  }

  if (hasRawUserImage && !mainHasVision) {
    // Only a native-vision model can take the raw image on the main request.
    // A capable but text-only model (Ornith, gpt-oss) is served without an
    // mmproj, so llama-server 500s ("image input is not supported") the
    // moment raw pixels reach it. Everything non-vision goes through the
    // bridge, which replaces image blocks with VLM text descriptions — and on
    // bridge failure strips them, so a blind model never receives raw pixels.
    ...
    if (standalone && bridge.described > 0 && bridge.displayText) {
      ...
      logger.info(`[agent] standalone visual request answered directly by ${DEFAULT_LOCAL_MODEL}`);
      return answer;
    }
  }
  ...
  // a Stop pressed while we're awaiting prepareModelContext/checkLlamaCppHealth/
  // callTool (i.e. between model requests, with no fetch currently listening
  // on the signal) would otherwise be silently lost and a follow-up request
  // would fire anyway. Latch our own copy the moment the registered controller
  // is aborted, independent of wsHandler forgetting its reference (mirrors
  ...
  while (true) {
    ...
    setAbort(controller);
    let trimmed, systemPrompt, systemPromptNoSkills, llamacppTools;
    if (prepareModelContext) {
      ...
      systemPromptNoSkills = prepared.systemPromptNoSkills ?? null;
    ...
    // can inject thousands of prompt tokens (since the round-6 KV-reuse fix
    // they live in the system prompt, not the tail message), and on a small
    // served context llama.cpp rejects the whole request (400
    // exceed_context_size_error) rather than trimming. Skills are an
    // enhancement, not a correctness requirement — rebuild without the skill
    // block and re-fit before sending a request we already know is doomed.
    if (fitted.estimatedTokens > requestBudget && systemPromptNoSkills) {
      openaiMessages = toOpenAIMessages(trimmed, appendNudges(systemPromptNoSkills), provider.vision);
    ...
    if (fitted.estimatedTokens > requestBudget) {
      const toolFree = fitToolsToContext(
        openaiMessages,
        llamacppTools,
        provider.contextWindow,
        { minimumTools: 0 },
      );
      llamacppTools = toolFree.tools;
      fitted = toolFree;
    }
    if (fitted.estimatedTokens > requestBudget) {
      const compacted = compactSystemPromptToContext(
        openaiMessages,
        llamacppTools,
        provider.contextWindow,
      );
      if (compacted.removedTokens > 0) {
        logger.warn(
          `[llamacpp] request preflight compacted the system prompt by ${compacted.removedTokens} tokens ` +
          `(${fitted.estimatedTokens}→${compacted.estimatedTokens}, budget ${requestBudget})`,
        );
        openaiMessages = compacted.messages;
        fitted = { ...fitted, estimatedTokens: compacted.estimatedTokens };
      }
    }
    if (fitted.estimatedTokens > requestBudget) {
      logger.warn(
        `[llamacpp] request remains over budget after all fallbacks ` +
    ...
    const noTools = state.noTools || opts.noTools || forceAnswerOnly;
    // Let thinking stream on the first attempt by default. This has no link to
    // the browser's "Show reasoning" preference (aperio-reasoning) — that
    // setting is UI-display-only and never reaches the agent or provider.
    // Some models (gemma) occasionally stall in their thinking channel on tool
    // turns and end empty — that case is caught below by the empty-completion
    // retry, which re-runs this turn with thinking forced off.
    ...
    // The fingerprint/msgprint pair is emitted from makeLlamaCppRequest itself
    // so every request path is covered, not just this one.
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (actualContext && actualContext !== provider.contextWindow) {
              ...
              // Deliberately sparse: this re-announce only has fresh data for the
              // context window (the reason it fires at all — the router's real
              // window differs from the preset). costRates/imageTokens/toolEligible/
              // roundtableAvailable/contextCapacityPct are omitted entirely rather
              // than sent as null/false, so the frontend's merge treats "absent" as
              // "no change" instead of clobbering boot-time values (see streaming.js
              // provider handler + provider-ux-parity plan WS1).
              emitter.send({ type: "provider", name: provider.name, model: provider.model, thinks: state.thinks, contextWindow: actualContext });
            }
          } else {
    ...
    if (!cleanText.trim() && toolCalls.length === 0 && emptyRetries < 1) {
      ...
      retryNudge = "Your previous attempt ended without any answer or tool call. Respond now: either give the final answer directly, or call the appropriate tool. If the previous tool result reports a file validation or syntax failure, repair the existing file with edit_file using a targeted replacement, then run syntax_check before continuing. Do not claim success until the check passes. Do not think further.";
      if (streamHandler.thinkingTimedOut)
        logger.info(`[agent] runaway reasoning from model="${provider.model}" — cut off after thinking timeout, retrying with thinking suppressed`);
      else
    ...
    if (toolCalls.length === 0 && cleanText.trim() &&
        detectToolCallLeak(cleanText, knownToolNames) &&
        !extractTextToolCall(cleanText, messages, knownToolNames)) {
      emitter.send({ type: "retract" });
      if (leakRetries < 1) {
        leakRetries++;
        forceSuppressThinking = true;
    ...
    const toolExecutor = new ToolExecutor(callTool, emitter, messages, knownToolNames, forceAnswerOnly);
    ...
    if (cancelled) { emitter.send({ type: "stream_end", text: "", usage: streamUsage }); return ""; }
    // Runaway-loop breaker. The dedup cache already refuses to re-run an
    // identical in-turn call, but the model can keep issuing it forever and burn
    // the whole turn budget on a result it already holds. Take the tools away for
    // one pass and make it answer instead.
    if (!forceAnswerOnly) {
    ...
    // Per-turn step cap. Counted here — reaching this line means the pass ended
    // in tool calls rather than an answer — so it bounds the loop even when the
    // calls are all different and the repeat breaker never fires.
    toolSteps++;
