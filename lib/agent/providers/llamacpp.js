...
export async function runLlamaCppLoop(messages, emitter, opts = {}, getAbort = () => null, setAbort = () => {}, ctx) {
  ...
  } else if (hasRawUserImage && !isVisionModel(provider.model)) {
    // Only a native-vision model can take the raw image on the main request.
    // The tool-capability allowlist (APERIO_CAPABLE_MODELS) is orthogonal to
    // sight: a capable but text-only model (Ornith, gpt-oss) is served without
    // an mmproj, so llama-server 500s ("image input is not supported") the
