...
import { ToolExecutor, extractTextToolCall, detectToolCallLeak, recoverToolName, looksLikeSystemPromptEcho, countTrailingRepeatedToolCalls, TOOL_REPEAT_LIMIT, TOOL_REPEAT_NUDGE } from "../../tools/executor.js";
...
export async function runLlamaCppLoop(messages, emitter, opts = {}, getAbort = () => null, setAbort = () => {}, ctx) {
  ...
  let retryNudge = null;
  // Set once by the tool-repeat breaker below; never cleared, so the turn gets
  // exactly one tool-free pass and then ends whatever the model does with it.
