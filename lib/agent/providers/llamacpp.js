...
async function makeLlamaCppRequest(provider, llamacppTools, openaiMessages, noTools, suppressThinking, setAbort) {
  ...
  try {
    const response = await fetch(`${provider.baseURL}/chat/completions`, {
      ...
      body: JSON.stringify({ model: provider.requestModel || provider.model, messages: openaiMessages, stream_options: { include_usage: true }, ...(noTools ? {} : { tools: llamacppTools }), ...(suppressThinking ? { chat_template_kwargs: { enable_thinking: false } } : {}), stream: true }),
...
export async function runLlamaCppLoop(messages, emitter, opts = {}, getAbort = () => null, setAbort = () => {}, ctx) {
  ...
  while (true) {
    ...
    const stopModelWatch = provider.name === "llamacpp"
      ? startModelProgressWatcher({ model: provider.model, emitter, routerBaseURL: provider.llamacppBaseURL })
      : null;
    let response;
    ...
    if (!response.ok) {
      ...
      if (response.status === 400 && provider.requestModel && provider.requestModel !== provider.model && err.includes(`model '${provider.requestModel}' not found`)) {
        ...
        try {
          ...
          const errorMsg = e.isTimeout ? "Request timeout" : e.message;
          logger.error(`[llamacpp] raw-model fallback failed: ${errorMsg}`, { model: provider.model });
          emitter.send({ type: "stream_start" }); emitter.send({ type: "token", text: "⚠️ " + errorMsg }); emitter.send({ type: "stream_end", text: errorMsg, usage: streamUsage }); return errorMsg;
        }
      }
      if (response.ok) {
    ...
    if (streamHandler.streamError) {
      const label = provider.name === "llamacpp" ? "llama.cpp" : provider.name === "gemini" ? "Gemini" : provider.name === "deepseek" ? "DeepSeek" : provider.name;
      logger.error(`[llamacpp] ${label} streamed error: ${streamHandler.streamError}`);
