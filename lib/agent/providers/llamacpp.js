...
import { isCapableModel } from "../tool-profiles.js";
...
export async function runLlamaCppLoop(messages, emitter, opts = {}, getAbort = () => null, setAbort = () => {}, ctx) {
  ...
  while (true) {
    ...
    const fitted = fitToolsToContext(openaiMessages, llamacppTools, provider.contextWindow);
    if (fitted.removed > 0) {
      logger.info(
        `[tools] request preflight capped schemas ${llamacppTools.length}→${fitted.tools.length} ` +
        `(${fitted.estimatedTokens}/${Math.floor(provider.contextWindow * REQUEST_HEADROOM_RATIO)} estimated tokens)`,
      );
