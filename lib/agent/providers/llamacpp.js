...
async function makeLlamaCppRequest(provider, llamacppTools, openaiMessages, noTools, suppressThinking, setAbort) {
  ...
  try {
    const response = await fetch(`${provider.baseURL}/chat/completions`, {
      ...
      body: JSON.stringify({ model: provider.model, messages: openaiMessages, stream_options: { include_usage: true }, ...(noTools ? {} : { tools: llamacppTools }), ...(suppressThinking ? { chat_template_kwargs: { enable_thinking: false } } : {}), stream: true }),
...
export async function runLlamaCppLoop(messages, emitter, opts = {}, getAbort = () => null, setAbort = () => {}, ctx) {
  ...
  while (true) {
    ...
    let trimmed, systemPrompt, llamacppTools;
    if (prepareModelContext) {
      ...
      llamacppTools = getOllamaTools(lastUserText, trimmed);
    }
    ...
    try { response = await makeLlamaCppRequest(provider, llamacppTools, openaiMessages, noTools, suppressThinking, setAbort); }
      ...
      else logger.error(`[llamacpp] request failed: ${errorMsg}`, { model: provider.model });
    ...
    if (!response.ok) {
      ...
      logger.error(`[llamacpp] ${label} API error (${response.status}): ${err.slice(0, 200)}`);
      let errMsg = err;
    ...
    if (streamHandler.timings) logger.debug(`[llamacpp] timings model="${provider.model}" prompt_tps=${streamHandler.timings.prompt_per_second ?? "?"} gen_tps=${streamHandler.timings.predicted_per_second ?? "?"}`);
    ...
    if (toolCalls.length === 0 && cleanText.trim() &&
      ...
      if (leakRetries < 1) {
        ...
        logger.warn(`[agent] tool-call leakage from model="${provider.model}" — retrying with nudge: ${cleanText.slice(0, 120).replace(/\s+/g, " ")}`);
        continue;
      }
