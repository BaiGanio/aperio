...
import { ensureVisionEngine } from "../../helpers/startLlamaCpp.js";
...
export async function runDeepSeekLoop(messages, emitter, opts = {}, getAbort = () => null, setAbort = () => {}, ctx) {
  ...
  // DeepSeek is a provider-level blind (model-capabilities.js's PROVIDER_VISION
  // map — no DeepSeek model accepts native image_url content today), so every
  // turn with an image goes through the local VLM bridge. server.js only boots
  // llama-server for AI_PROVIDER=llamacpp, so on a DeepSeek install nothing is
  // listening on it yet — start it on demand, the first time a turn actually
  // needs it. Previously this call unconditionally fetched a dead port and
  // failed silently; a DeepSeek image turn had never actually worked.
  if (!provider.vision) {
    ...
    if (hasImage) {
      try {
        await ensureVisionEngine(emitter);
      } catch (err) {
        logger.warn(`[deepseek] local vision engine failed to start: ${err.message}`);
        emitter.send({ type: "token", text: `> ⚠️ **Vision not available** — the local vision model could not start (${err.message}). Responding based on the text description only.\n\n` });
      }
    }
