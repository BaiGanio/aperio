...
function codexTurnBudgets() {
  return {
    maxToolCalls: positiveLimit(process.env.CODEX_TURN_MAX_TOOL_CALLS, 32),
    maxProcessedTokens: positiveLimit(process.env.CODEX_TURN_MAX_PROCESSED_TOKENS, 250000),
    maxSeconds: positiveLimit(process.env.CODEX_TURN_MAX_SECONDS, 900),
  };
}
...
export async function runCodexLoop(messages, emitter, opts = {}, getAbort, setAbort, ctx) {
  ...
  const budgets = codexTurnBudgets();
  ...
  const tripGuardrail = (kind, limit, value) => {
    if (guardrail) return;
    guardrail = { kind, limit, value };
    logger.warn(`[codex] turn guardrail reached: ${kind}=${value} limit=${limit}`);
    if (!abortCtrl.signal.aborted) abortCtrl.abort();
  };

  if (budgets.maxSeconds > 0) {
    elapsedTimer = setTimeout(
      () => tripGuardrail("elapsed_seconds", budgets.maxSeconds, budgets.maxSeconds),
      budgets.maxSeconds * 1000,
    );
    elapsedTimer.unref?.();
  }

  function handleEvent(ev) {
    if (guardrail && ev.type !== "turn.completed") return;
    ...
    if (ev.type === "item.started") {
      ...
      if (!described) return;
      if (budgets.maxToolCalls > 0 && toolSeq >= budgets.maxToolCalls) {
        tripGuardrail("tool_calls", budgets.maxToolCalls, toolSeq + 1);
        return;
      }
    ...
    if (ev.type === "turn.completed") {
      ...
      if (budgets.maxProcessedTokens > 0 && finalUsage.input_tokens > budgets.maxProcessedTokens) {
        tripGuardrail("processed_tokens", budgets.maxProcessedTokens, finalUsage.input_tokens);
      }
      logger.info(
  ...
  if (abortCtrl.signal.aborted) {
    ...
    const limitText = `⚠️ Codex turn limit reached (${guardrail.kind}: ${guardrail.value}/${guardrail.limit}). ` +
      "The turn was interrupted; retry with a narrower request or raise the configured limit.";
    emitter.send({ type: "token", text: limitText });
    finalText = limitText;
  }

  if (!guardrail && exitCode.error) {
  ...
  } else if (!guardrail && exitCode.code !== 0) {
    ...
    emitter.send({ type: "token", text: errText });
    finalText = streamedText || errText;
  } else if (!guardrail && turnFailed) {
    const errText = `⚠️ Codex provider error: ${turnFailed}`;
    emitter.send({ type: "token", text: errText });
    finalText = streamedText || errText;
  } else if (!guardrail && !finalText && !streamedText) {
