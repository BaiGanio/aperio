...
function buildCodexArgs({ prompt, provider, root, sessionId, images = [] }) {
...
export async function runCodexLoop(messages, emitter, opts = {}, getAbort, setAbort, ctx) {
  ...
  const args = buildCodexArgs({ prompt, provider, root, sessionId: resumeSessionId, images: imagePaths });
  const env = {
    ...process.env,
