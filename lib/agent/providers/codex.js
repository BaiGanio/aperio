...
import { mkdirSync, readdirSync, renameSync, statSync, writeFileSync, unlinkSync } from "node:fs";
...
import { createCodexTurnMeter, resolveCodexTurnBudgets } from "./codex-turn-meter.js";
...
// Extracts the image content blocks (Aperio's `{type:"image", source:{type:
// "base64", media_type, data}}` shape, already assembled by imageHandler.js)
// from the latest user message — the same message latestUserText() reads its
// text from.
function latestUserImageBlocks(messages) {
  const lastUserMsg = [...messages].reverse().find(m => m.role === "user");
  const content = lastUserMsg?.content;
  if (!Array.isArray(content)) return [];
  return content.filter(b => b.type === "image" && b.source?.type === "base64" && b.source?.data);
}

// codex exec's `-i/--image <FILE>...` flag takes real file paths, not inline
// data — decode each block to a per-turn temp file under the session's
// scratch dir (or the OS tmp dir when no session scratch dir is active, e.g.
// an isolated/one-shot call). Returns the file paths in block order.
function writeTempCodexImages(blocks, dir) {
  if (!blocks.length) return [];
  mkdirSync(dir, { recursive: true, mode: 0o700 });
  return blocks.map((block, i) => {
    const ext = IMAGE_EXT_BY_MEDIA_TYPE[block.source.media_type] || "png";
    const filePath = join(dir, `codex-image-${Date.now()}-${i}.${ext}`);
    writeFileSync(filePath, Buffer.from(block.source.data, "base64"));
    return filePath;
  });
}

function cleanupTempCodexImages(paths) {
  for (const p of paths) {
    try { unlinkSync(p); } catch (err) { logger.warn(`[codex] failed to remove temp image ${p}: ${err.message}`); }
  }
}
...
function buildCodexArgs({ prompt, provider, root, sessionId, images = [], ephemeral = false, noTools = false }) {
  ...
  // WS4/D1: without this, `--json` never emits a `reasoning` item at all (verified
  // live — the CLI's own config.toml default of "auto" only applies to the
  // interactive TUI, not `codex exec`), so the reasoning bubble stays permanently
  // empty for codex regardless of model_reasoning_effort. Free — it surfaces a
  // summary of tokens already billed as reasoning_output_tokens, not new ones.
  const reasoningSummary = enumValue(
    process.env.CODEX_REASONING_SUMMARY,
  ...
  const common = [
    ...
    "-c", `model_reasoning_summary=${tomlString(reasoningSummary)}`,
    "-c", `approval_policy=${tomlString(approvalPolicy)}`,
    // Tool-free calls (summaries, handoffs) skip the MCP server entirely —
    // no server to start, no tool schemas to spend context on.
    ...(noTools ? [] : [
  ...
  // One-shot evaluators deliberately do not resume a provider session. Avoid
  // leaving dozens of otherwise-unreferenced Codex threads behind; the eval
  // ledger and transcripts are their durable record.
  if (ephemeral) common.push("--ephemeral");

  if (process.env.CODEX_IGNORE_RULES === "1") common.push("--ignore-rules");

  // Use one `--image=<path>` occurrence per image.  Codex's variadic
  // `-i/--image <FILE>...` form greedily consumes the following positional
  // prompt as another image path, so `-i <path> ... <prompt>` silently turns
  // the prompt into stdin input.  The equals form keeps each image value
  // bounded while leaving the final positional prompt unambiguous.
  const imageArgs = images.map(path => `--image=${path}`);

  if (sessionId) {
    return [...common, ...imageArgs, "resume", sessionId, "--", prompt];
  }
  // Skill blocks begin with YAML frontmatter (`---`). Without an explicit
  // end-of-options delimiter clap parses the whole prompt as a CLI flag and
  // the model never receives the turn.
  return [...common, ...imageArgs, "--", prompt];
}
...
export async function runCodexLoop(messages, emitter, opts = {}, getAbort, setAbort, ctx) {
  ...
  const budgets = resolveCodexTurnBudgets(ctx.codexTurnEnv || process.env, {
    onInvalid: key => logger.warn(`[codex] invalid ${key}; using the safe registry default`),
  });
  const clock = ctx.codexClock?.now ? ctx.codexClock : undefined;
  const meter = createCodexTurnMeter({ budgets, now: clock?.now });
  const setTimer = ctx.codexSetTimeout || setTimeout;
  const clearTimer = ctx.codexClearTimeout || clearTimeout;
  ...
  // WS4/D1: codex's --json reasoning item is not streamed token-by-token — it
  // arrives as one `item.completed` carrying the full summary text (`item.started`
  // for it is not guaranteed, observed live to be absent) — so each reasoning item
  // opens and closes its own reasoning_start/token/done triplet immediately,
  // mirroring how the other providers re-open a bubble after a tool-call pause.
  ...
  // ctx.getSkillsBlock) instead of pointing codex's native .codex/skills/
  // discovery at Aperio's skills/ dir — that would bypass the skill panel's
  // enable/disable state, the forced /skill command, and one-shot picks.
  const skillsBlock = ctx.getSkillsBlock?.(latestUserText(messages), opts.lang, messages) || "";
  const extraSystem = [skillsBlock, opts.extraSystem].filter(Boolean).join("\n\n---\n\n");
  const prompt = buildPrompt(messages, extraSystem, !resumeSessionId);

  const imageBlocks = latestUserImageBlocks(messages);
  const imagePaths = writeTempCodexImages(imageBlocks, scratchDir || tmpdir());

  const args = buildCodexArgs({
    prompt, provider, root, sessionId: resumeSessionId, images: imagePaths,
    ephemeral: opts.isolatedProviderSession === true,
    noTools: opts.noTools === true,
  });
  const env = {
    ...process.env,
    // The Codex CLI's API-key auth path reads OPENAI_API_KEY, not the
    // Aperio-facing CODEX_API_KEY setting — map it so the advertised config
    // actually authenticates. CODEX_API_KEY is the Codex-specific credential
    // (config.js reserves it for `codex exec`; OPENAI_API_KEY is documented
    // as generic), so it takes precedence over a generic OPENAI_API_KEY that
    // may be configured for an unrelated OpenAI-compatible provider.
  ...
  try {
    ...
    cleanupTempCodexImages(imagePaths);
    const errText = `⚠️ Codex provider error: ${error.message}`;
    emitter.send({ type: "token", text: errText });
    const usage = { ...mapUsage(null), ...meter.snapshot() };
    emitter.send({ type: "stream_end", text: errText, usage });
    return errText;
  }

  const tripGuardrail = (candidate) => {
    if (!candidate || candidate.enforcement !== "live") return;
    logger.warn(`[codex] turn guardrail reached: ${candidate.kind}=${candidate.value} limit=${candidate.limit}`);
    if (!abortCtrl.signal.aborted) abortCtrl.abort();
  };

  if (budgets.timeoutMs > 0) {
    elapsedTimer = setTimer(
      () => tripGuardrail(meter.tripElapsed()),
      budgets.timeoutMs,
    );
    elapsedTimer.unref?.();
  }
  ...
  function handleEvent(ev) {
    if (meter.guardrail?.enforcement === "live" && ev.type !== "turn.completed") return;
    ...
    const itemGuardrail = meter.observeItem(ev.type, ev.item);
    tripGuardrail(itemGuardrail);
    ...
    if (ev.type === "item.completed" && ev.item?.type === "reasoning") {
      if (!reasoningOpen) { reasoningOpen = true; emitter.send({ type: "reasoning_start" }); }
      if (ev.item.text) emitter.send({ type: "reasoning_token", text: ev.item.text });
    ...
    if (ev.type === "item.started") {
      ...
      if (!described) return;
      if (meter.guardrail?.enforcement === "live") return;
    ...
    if (ev.type === "turn.completed") {
      ...
      const tokenGuardrail = meter.observeProcessedTokens(finalUsage.input_tokens);
      if (tokenGuardrail?.enforcement === "observed" && !observedWarningSent) {
        observedWarningSent = true;
        const warning = `⚠️ Codex observed aggregate-token ceiling reached (${tokenGuardrail.value}/${tokenGuardrail.limit}). ` +
          `The answer completed, but future turns may need a narrower request or a higher ${tokenGuardrail.setting} limit.`;
        emitter.send({ type: "token", text: `\n\n${warning}` });
        streamedText += `\n\n${warning}`;
  ...
  cleanupTempCodexImages(imagePaths);
  if (elapsedTimer) clearTimer(elapsedTimer);
  ...
  // A reasoning item that started (or was inferred open) but never reached
  // item.completed must not leave the collapsed bubble stuck in its "thinking…"
  // state forever.
  if (reasoningOpen) { emitter.send({ type: "reasoning_done" }); reasoningOpen = false; }

  const guardrail = meter.guardrail;
  if (abortCtrl.signal.aborted) {
    if (!guardrail || guardrail.enforcement !== "live") {
      emitter.send({ type: "stream_end", text: "", usage: finalUsage });
      return "";
    }
    const limitText = `⚠️ Codex turn limit reached (${guardrail.setting}: ${guardrail.value}/${guardrail.limit}). ` +
      `The turn was interrupted; retry with a narrower request or raise ${guardrail.setting}.`;
    emitter.send({ type: "token", text: limitText });
    finalText = limitText;
  }

  if ((!guardrail || guardrail.enforcement === "observed") && exitCode.error) {
  ...
  } else if ((!guardrail || guardrail.enforcement === "observed") && exitCode.code !== 0) {
    ...
    emitter.send({ type: "token", text: errText });
    finalText = streamedText || errText;
  } else if ((!guardrail || guardrail.enforcement === "observed") && turnFailed) {
    const errText = `⚠️ Codex provider error: ${turnFailed}`;
    emitter.send({ type: "token", text: errText });
    finalText = streamedText || errText;
  } else if ((!guardrail || guardrail.enforcement === "observed") && !finalText && !streamedText) {
