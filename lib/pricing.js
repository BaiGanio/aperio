...
let _cache = null; // { internalKey: { in, out, cacheRead, cacheWrite, contextWindow } }

// One published OpenRouter rate, converted from USD-per-token to USD-per-million.
// `null` means the catalog carries no such rate — never 0, which would be a claim
// that those tokens are free. A caller that cannot price a class must say so
// rather than bill it at zero.
...
async function fetchFromOpenRouter() {
  ...
  for (const model of json.data) {
    ...
    extracted[internalKey] = {
      ...
      // Cache reads are billed BELOW the prompt rate, cache writes ABOVE it, so
      // neither can be derived from `in`. Both are absent for providers that
      // charge no separate cache rate (OpenAI, DeepSeek) — those stay null.
      // `input_cache_write` is the 5-minute rate, which is the one that applies:
      // lib/agent/providers/anthropic.js sets a bare `{ type: "ephemeral" }`
      // breakpoint with no `ttl`, so Aperio never buys the 1-hour cache.
      cacheRead: parseRate(p.input_cache_read),
      cacheWrite: parseRate(p.input_cache_write),
...
export function getPricing(modelName) {
  ...
  return {
    in: entry.in,
    out: entry.out,
    // A cache file written before these rates existed carries neither, and
    // `null` is the same answer as "the catalog does not publish one" — the
    // caller has to handle that case regardless, so there is nothing to migrate.
    cacheRead: Number.isFinite(entry.cacheRead) ? entry.cacheRead : null,
    cacheWrite: Number.isFinite(entry.cacheWrite) ? entry.cacheWrite : null,
