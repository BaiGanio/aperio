...
export async function runReindex(store, {
  ...
  // No sqlite-vec on this platform. Every stale marker stays exactly as it is
  // — it is the record that these stores need rebuilding once vector support
  // returns — but nothing is cleared and no embedding is generated. Running
  // here would clear each store's vectors and then spend one embedding call
  // per row writing blobs into plain tables that no query can MATCH against,
  // and reconcileVecSidecars destroys them again the moment the database is
  // opened where the extension loads. This is the single choke point for that
  // rule: both the server's background driver and the CLI reach the generator
  // only through here.
  if (!vectorStorageSupported(store)) {
    logger.info(
      "[reindex] vector search is unavailable on this platform — skipping reindex; " +
      "stores stay marked stale and serve full-text results until the database is opened where sqlite-vec loads."
    );
