...
// A `lookup` option that always answers with the pinned IP. Node ≥20 sockets
// default to autoSelectFamily (Happy Eyeballs), which calls lookup with
// { all: true } and expects an ARRAY of { address, family } — the scalar
// callback form makes net read `undefined` and throw ERR_INVALID_IP_ADDRESS
// ("Invalid IP address: undefined"). Honor both shapes.
...
export async function safeFetch(url, options = {}, _redirectCount = 0) {
  ...
  const resp = await new Promise((resolve, reject) => {
    const req = mod.request({
      ...
      lookup: makePinnedLookup(ip, family),
