...
import { recommendContextLength, MODEL_FACTS, GENERIC_MODEL_FACTS, factsForHf, resolvePerfProfile, getRecommendedModel, residentFootprintGB, RAM_FIT_DEFAULTS } from "../providers/index.js";
...
function modelFactsFor(modelKey, hardware) {
  return (hardware.modelCacheDir ? inspectCachedModel(modelKey, hardware.modelCacheDir) : null)
    ?? factsForHf(modelKey)
    ?? GENERIC_MODEL_FACTS;
}

// Gemma 4's hybrid attention uses a 512-wide head that is not supported by
// every llama.cpp Flash Attention backend (notably some Metal builds). The
// backend reports the failure only during the first decode as "Compute error.",
// so keep the low-VRAM profile's Flash Attention default for other models but
// explicitly opt Gemma 4 out in its preset section.
function flashAttentionSafeFor(modelKey) {
  return !/(?:gemma[-.]?4|gemma4)/i.test(String(modelKey));
}
...
export function buildModelsPreset(env = process.env, hardware = {}) {
  ...
  const emit = (alias, name, extra = {}) => {
    ...
    if (profile === "fast-low-vram" && !flashAttentionSafeFor(name)) {
      lines.push("flash-attn = false");
    }
    if (profile === "fast-low-vram") {
...
// ── Public ─────────────────────────────────────────────────────────────────
export async function ensureLlamaCpp(_spawn = spawn, _kill = killByPid, _findPid = findLlamaCppPidOnPort) {
  ...
  if (await isLlamaCppUp()) {
    ...
    const discoveredPid = stored?.pid ?? llamaCppProc?.pid ?? _findPid();
    if (!stored?.pid && discoveredPid) {
      // Adopt the listener so Ctrl+C/browser-idle shutdown can clean up the
      // exact process we reconciled, including its router worker group.
    ...
    const killPid = stored?.pid ?? llamaCppProc?.pid ?? discoveredPid;
