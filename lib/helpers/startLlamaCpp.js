...
import { resolvePerfProfile, factsForHf, DEFAULT_LOCAL_MODEL } from "../providers/index.js";
import { resolveModelCacheDir } from "./modelCache.js";
import { resolveLlamaCppCommand } from "./llamacppBinary.js";
...
import { buildModelsPreset, buildVisionOnlyPreset, defaultMainModelHf } from "./llamacpp/preset.js";
...
// ── Public ─────────────────────────────────────────────────────────────────
// A serialized startup queue — every call chains after whichever run is
// currently in flight, rather than sharing one call's promise. Two callers
// can race here for real: e.g. two DeepSeek sessions each submit a first
// image at once while llama-server is still cold (ensureVisionEngine →
// ensureLlamaCpp for both), or a DeepSeek image bridge cold-starting the
// vision-only preset while a local wiki refresh concurrently calls the
// normal ensureLlamaCpp() for the main+VLM preset. Without serializing, both
// can pass the "not up yet" health check and spawn competing llama-server
// processes on the same port — the loser's PID can clobber the module-global
// llamaCppProc, leaving the winner untracked and unreapable. A single shared
// promise (an earlier version of this) fixed that but broke the
// mixed-preset case: the second caller would silently inherit the first
// caller's DIFFERENT preset result instead of getting its own. Chaining
// instead means every call actually runs its own ensureLlamaCppRun once its
// turn in the queue arrives — that pass's own preset-hash reconciliation
// (below) detects a mismatch against whatever the previous call left running
// and restarts with the correct preset — while never running two spawn
// attempts concurrently. The extra liveness round trip for two genuinely
// identical concurrent intents is cheap (ensureLlamaCppRun's own fast path).
let startupQueue = Promise.resolve();

// `_buildPreset` is injectable so ensureVisionEngine (below) can drive this
// exact spawn/reconcile/health-poll path against a vision-only preset instead
// of the normal main(+VLM) one, without duplicating the ~200-line body.
export async function ensureLlamaCpp(_spawn = spawn, _kill = killByPid, _findPid = findLlamaCppPidOnPort, _isOwnedPid = isLlamaServerPid, _buildPreset = buildModelsPreset) {
  const run = startupQueue.then(() => ensureLlamaCppRun(_spawn, _kill, _findPid, _isOwnedPid, _buildPreset));
  // Keep the queue itself always-resolving so one caller's rejection (e.g.
  // "did not start within 30s") never permanently blocks the next caller in
  // line — `run` still rejects for THIS caller, who sees the real error.
  startupQueue = run.catch(() => {});
  return run;
}

async function ensureLlamaCppRun(_spawn, _kill, _findPid, _isOwnedPid, _buildPreset) {
  ...
  // a custom LLAMACPP_MODEL wins) and the same runtimeHardware
  // buildModelsPreset() uses below. Passing an empty env / mis-ordered args here
  // sized LLAMACPP_SERVE_CTX for the default model while the preset loaded the
  // configured one.
  ...
  const preset = await _buildPreset(process.env, runtimeHardware);
  if (_buildPreset === buildModelsPreset) {
  ...
  if (await isLlamaCppUp()) {
    ...
    // A test or an interrupted boot can leave the module-local handle behind
    // after state.json has been removed. Do not treat that orphaned handle as
    // ownership of the currently healthy listener: it may be a stale fake PID
    // (in tests) or a recycled PID from an earlier process. A live listener or
    // persisted state is the evidence required to retain/adopt ownership.
    if (!stored && !portPid) llamaCppProc = null;
  ...
  llamaCppProc = _spawn(resolveLlamaCppCommand(), [
...
// WS4: on-demand local vision for a cloud text-only provider (DeepSeek today —
// see deepseek.js). server.js only boots llama-server for AI_PROVIDER=llamacpp,
// so a cloud install's image upload previously fetched 127.0.0.1:8080 with
// nothing listening and silently degraded to a text label — this starts the
// engine the first time a turn actually needs it.
//
// Already up AND genuinely serving a usable vision route → no-op. That route
// is either the bridge alias itself (a running main+VLM preset always
// includes aperio-vlm, or a prior ensureVisionEngine call), or a native-vision
// main-only preset that already serves images under aperio-main (see
// mainAliasEligible in mcp/tools/image.js — the same condition, checked here
// too so a live native-vision main engine — e.g. mid-round-table, or after a
// runtime provider switch — is never torn down just to load the smaller
// vision-only preset). Reconciling (restarting) either of those would
// interrupt any concurrent local turn and, once restarted to the vision-only
// preset, leave a subsequent switch back to llamacpp passing /health but
// failing model requests. A healthy /health does not by itself mean either
// route exists, though — an unmanaged process or a stale preset can be "up"
// without either — those cases fall through to ensureLlamaCpp, which
// reconciles (restarts with the vision-only preset) using its own
// ownership-verified kill path, via the exact same spawn/health-poll path
// used for the normal boot.
export async function ensureVisionEngine(emitter = null, _spawn = spawn, _kill = killByPid, _findPid = findLlamaCppPidOnPort, _isOwnedPid = isLlamaServerPid) {
  if (await isLlamaCppUp()) {
    const serverModels = await fetchServerModels();
    if (serverModels?.includes(LLAMACPP_VLM_ALIAS)) return;
    if (serverModels?.includes(LLAMACPP_MAIN_ALIAS)) {
      const { vision } = await modelCapabilities(process.env.LLAMACPP_MODEL || "");
      if (vision) return;
    }
    logger.warn(`🦙 llama-server is up on port ${LLAMACPP_PORT} but does not serve ${LLAMACPP_VLM_ALIAS} or a vision-capable ${LLAMACPP_MAIN_ALIAS} (unmanaged process or a stale preset) — reconciling with the vision-only preset.`);
  }
  if (emitter) {
    const vlmModel = DEFAULT_LOCAL_MODEL;
    const sizeGB = factsForHf(vlmModel)?.sizeGB;
    const sizeNote = Number.isFinite(sizeGB) ? `~${sizeGB.toFixed(1)} GB` : "several GB";
    emitter.send({ type: "token", text: `> 🖼️ Starting the local vision model (${vlmModel}) for this turn — first run downloads ${sizeNote} and may take a few minutes…\n` });
  }
  const started = await ensureLlamaCpp(_spawn, _kill, _findPid, _isOwnedPid, buildVisionOnlyPreset);
  // ensureLlamaCpp fails closed (returns false, does not throw) when it can't
  // safely reconcile — e.g. an unowned foreign process holding the port. That
  // must not read as success here: bridgeImagesToVLM's subsequent
  // describe_image call would otherwise fail later with a confusing HTTP/
  // connection error instead of this clear, catchable diagnostic.
  if (started === false) {
    throw new Error(`llama-server on port ${LLAMACPP_PORT} could not be reconciled to serve ${LLAMACPP_VLM_ALIAS} — see the server log for details.`);
  }
}
