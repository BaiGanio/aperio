...
import { recommendContextLength, MODEL_FACTS, factsForHf, resolvePerfProfile, getRecommendedModel, resolveModelFacts, residentFootprintGB, RAM_FIT_DEFAULTS } from "../providers/index.js";
...
function defaultMainModelHf(env, hardware, profile) {
  return getRecommendedModel(profile, hardware, env) || DEFAULT_MAIN_MODEL;
}
...
function isLlamaServerPid(pid) {
  ...
  try {
    ...
    return /(?:^|[\\/\\s])llama-server(?:$|[\\s])/i.test(command);
  } catch { return false; }
}
