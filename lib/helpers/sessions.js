...
export function finaliseSession(id, messages, attachmentsMap = null, hadAttachments = false) {
  ...
  if (!isMeaningful(messages, attachmentsMap, hadAttachments) && !scratchHasFiles(id)) {
    ...
    remove(id);
    // Truncate the shared log so the next session starts clean.
    truncateServerLog();
    return;
  }
  ...
  // Save this session's llama-server log, then truncate so the next session
  // starts clean. Best-effort — no-op when the server isn't running.
  saveServerLog(id);
  truncateServerLog();
