...
/**
 * True if the user actually engaged — at least one non-empty user turn beyond
 * the internal greeting at [0]. Deliberately laxer than isMeaningful: it draws
 * the line at "did anything happen" rather than "was it substantive", used to
 * preserve a session INTERRUPTED by shutdown (which the user didn't choose to
 * end) while still dropping a phantom session from a browser that connected but
 * never sent a turn.
 */
function hasUserEngagement(messages) {
  return messages.slice(1).some(m => m.role === "user" && extractText(m).length > 0);
}
...
export function finaliseSession(id, messages, attachmentsMap = null, hadAttachments = false, opts = {}) {
  ...
  const keepInterrupted = opts.onShutdown === true && (hasUserEngagement(messages) || hasServerLog);
  ...
  if (!keepInterrupted && !wasSummarized && !isContinuation && !isMeaningful(messages, attachmentsMap, hadAttachments) && !scratchHasFiles(id)) {
  ...
  const derivedTitle = deriveTitle(messages, s.summaries);
  if (!isContinuation || derivedTitle !== "Untitled session") {
    s.title = derivedTitle;
  }

  // Store the actual conversation — skip the internal greeting/resume-context
  // prompt at [0]. For a continuation, APPEND the new turns to the existing
  // persisted transcript instead of replacing it: `messages` here holds only
  // what happened on this connection since the resume, and replacing would
  // silently erase everything the session held before it (round 12, P1).
  const newReadable = toReadableMessages(messages.slice(1), attachmentsMap);
