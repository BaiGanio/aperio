import os from "os";
import { MODEL_FACTS, factsForHf, llamacppContextWindow } from "../providers/index.js";
...
export function estimateLlamaCppFootprintGB(model, contextWindow, env = process.env) {
  if (!model) return 0;
  const facts = factsForHf(model) ?? MODEL_FACTS[String(model)] ?? { sizeGB: 8 };
...
export function shouldEnableRoundtable({
  ...
  // Deduplicate: llama-server's router keeps ONE resident copy per model, so
  // the same model used by several agents costs its footprint once, not once
  // per agent.
  const localModels = [...new Set(
    [mainProvider, primaryConfig, verifierConfig]
  ...
  const footprintGB = localModels.reduce((sum, model) => sum + estimateLlamaCppFootprintGB(model, contextWindow, env), 0);
