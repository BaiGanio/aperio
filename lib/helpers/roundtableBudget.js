...
// An Ollama server on another host serves models from THAT machine's RAM —
// os.totalmem() here says nothing about it, so the budget check must not run.
function ollamaIsRemote(env) {
  const raw = env.OLLAMA_BASE_URL || "http://localhost:11434";
  try {
...
export function shouldEnableRoundtable({
  ...
  // Deduplicate: Ollama keeps ONE resident copy per model, so the same model
  // used by several agents costs its footprint once, not once per agent.
  const localModels = [...new Set(
    [mainProvider, primaryConfig, verifierConfig]
      .filter(c => c?.name === "ollama")
      .map(c => c.model),
  )];
