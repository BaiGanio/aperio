...
const callerFormat = format((info) => {
  ...
  // Attribute the error to OUR code, not to whatever library frame happens to sit
  // on top of the stack. logger.error is often invoked from inside an async
  // stream/transform pipeline (e.g. readable-stream), so without excluding
  // node_modules the caller resolves to
  // `node_modules/readable-stream/lib/_stream_transform.js:…` — noise that tells
  // the reader nothing about where the failure actually originated.
...
// MCP stdio servers speak JSON-RPC on stdout — any stray log line there is a
// protocol violation (the Node SDK client skips them, strict clients drop the
// connection). Detect MCP mode two ways: the role env Aperio's own spawner sets
// (lib/agent/mcp-connect.js) and direct execution of mcp/index.js (standalone
// `npm run mcp`, e.g. registered in Codex/Claude), then route the Console
// transport to stderr so the JSON-RPC channel stays clean.
const IS_MCP_PROCESS = (() => {
  if (process.env.APERIO_PROC_ROLE === "mcp") return true;
  const entry = process.argv[1];
  if (!entry) return false;
  try {
    return resolve(entry) === resolve(__dirname, "../../mcp/index.js");
  } catch {
    return false;
  }
})();

const CONSOLE_STDERR_LEVELS = IS_MCP_PROCESS
...
const logger = createLogger({
  ...
  transports: [
    new transports.Console({
      stderrLevels: CONSOLE_STDERR_LEVELS,
      format: format.combine(
        format.colorize(),
