...
// Redact one content block. A tool_result's payload lives at block.content
// (string, or a nested array of blocks — e.g. the image bridge's
// [{type:"text",...}, {type:"image",...}] shape) rather than block.text, so
// both must be scrubbed, recursively.
function redactBlock(block) {
  if (!block || typeof block !== "object") return block;
  let out = block;
  if (typeof block.text === "string") out = { ...out, text: redactSecrets(block.text) };
  if (typeof block.content === "string") out = { ...out, content: redactSecrets(block.content) };
  else if (Array.isArray(block.content)) out = { ...out, content: block.content.map(redactBlock) };
  return out;
}
...
export function redactMessages(messages) {
  ...
  return messages.map((m) => {
    ...
    if (Array.isArray(m.content)) return { ...m, content: m.content.map(redactBlock) };
    return m;
  });
}
