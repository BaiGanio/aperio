...
export function createNetGuard({ allowedHosts }) {
  return function netGuard(req, res, next) {
    ...
    // The GitHub webhook authenticates with its own HMAC (GITHUB_WEBHOOK_SECRET),
    // which GitHub signs — it cannot present X-Aperio-Client (a first-party-browser
    // -only header), so exempt it here, matching authGuard's exemption.
    if (req.path.startsWith("/api/") && STATE_CHANGING.has(req.method) && req.path !== "/api/github/webhook") {
