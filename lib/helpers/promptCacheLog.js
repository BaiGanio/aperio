...
//
// Prompt-cache hygiene: a pure parser over llama-server's own debug log
// (var/llamacpp/<session-id>.log or the shared server.log) that extracts,
// per request, the slot-cache signal llama.cpp already reports on stdout:
//
...
export function formatPromptCacheReport(records) {
  ...
  const lines = ["Prompt-cache reuse per request:", ""];
  records.forEach((r, i) => {
