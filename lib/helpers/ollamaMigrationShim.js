...
// This gate runs before the database opens, so it intentionally reads only
// model-selection defaults from the config registry. Sizing facts live in the
// database and are unavailable at this preflight stage.
function curatedModelLines() {
  return CONFIG
    .filter(entry => /^LLAMACPP_(?:MODEL_TIER_\d+|VLM_MODEL)$/.test(entry.key))
    .map(entry => `    ${entry.key.padEnd(24)} → ${entry.default}`);
}
...
export function formatOllamaMigrationMessage(detection) {
  ...
  lines.push("  files into the standard Hugging Face hub cache (~/.cache/huggingface/hub,");
  lines.push("  override with LLAMA_CACHE). Configured replacement model IDs:");
  lines.push("");
