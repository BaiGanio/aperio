...
// Hugging Face leaves *.downloadInProgress behind when a download is
// interrupted. Treat a marker that has not been touched for this long as an
// orphan; otherwise a completed model can be reported as downloading forever.
const DOWNLOAD_STALE_MS = 15 * 60 * 1000;
const GIB = 1024 ** 3;
...
export function downloadInProgressBytes(modelId, cacheRoot = resolveModelCacheDir()) {
  ...
  try {
    ...
    for (const f of readdirSync(blobsDir)) {
      ...
      try {
        const stat = statSync(join(blobsDir, f));
        if (Date.now() - stat.mtimeMs > DOWNLOAD_STALE_MS) continue;
        bytes += stat.size;
      } catch { /* race: finalised mid-scan */ }
    }
    return bytes;
  } catch { return 0; }
}

// Monotonic bytes present in this model repo's blob store, including shards
// llama.cpp has already renamed from *.downloadInProgress to their final
// digest. Bootstrap pulls can contain both the main GGUF and an mmproj, so
// watching only in-progress markers makes the displayed count jump backwards
// whenever one file finishes.
export function downloadedModelBytes(modelId, cacheRoot = resolveModelCacheDir()) {
  const dirName = cacheDirNameFor(modelId);
  if (!dirName) return 0;
  const blobsDir = join(resolve(cacheRoot), dirName, "blobs");
  try {
    ...
    for (const file of readdirSync(blobsDir)) {
      try {
        const stat = statSync(join(blobsDir, file));
        if (stat.isFile()) bytes += stat.size;
      } catch { /* race: blob renamed/finalised mid-scan */ }
    }
    return bytes;
  } catch { return 0; }
}
