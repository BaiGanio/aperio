  ...
  setImmediate(async () => {
    try {
      ...
      for (const row of pending) {
        if (aborted) break;
        // currentOp spans both ONNX inference AND the LanceDB write so that
        // shutdownEmbeddings() waits for the full operation before process.exit().
        // Calling exit() while either native layer is active destroys their mutexes.
        currentOp = (async () => {
