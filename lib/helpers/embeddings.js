...
import { signatureString, supportsVecMeta, ensureVecMeta, markStaleWhereChanged, markStoresStale, pendingStoreNames, embedForStore, readRebuiltVectorStores, clearRebuiltVectorStores } from './vecMeta.js';
...
async function checkEmbeddingProviderVecMeta(store, current, stored) {
  ...
  // Vector storage the store had to physically rebuild when it opened — a
  // SQLite database moved between a machine with the sqlite-vec extension and
  // one without it. The tables are the right kind now but empty, while the
  // signature and the dimensions are unchanged, so nothing further down would
  // ever notice. Recorded here, above the disabled check, precisely because it
  // has to outlive this process: the discovery only happens at open, and a boot
  // with embeddings disabled would otherwise drop it, leaving a later
  // re-enabled boot to read `current` over empty tables.
  //
  // Both sources are consulted: the property, for a rebuild this process's own
  // open performed, and the durable marker, for one performed by a process that
  // exited without ever reaching this function (see recordRebuiltVectorStores).
  // The marker is cleared only after the stores are actually marked, so a crash
  // in between leaves it for the next boot instead of dropping it.
  const rebuilt = [...new Set([
    ...(store.rebuiltVectorStores ?? []),
    ...await readRebuiltVectorStores(store),
  ])];
  if (rebuilt.length) {
    const marked = await markStoresStale(store, rebuilt);
    if (marked.length) {
      logger.warn(
        `[embeddings] vector storage was rebuilt at startup for: ${marked.join(", ")}. ` +
        `These stores serve full-text results only until reindexed.`
      );
    }
    await clearRebuiltVectorStores(store);
  }
  ...
  const staled = await markStaleWhereChanged(store, {
    signature,
    dims: current.dims,
    // Not widened for `rebuilt`: markStoresStale above already marked exactly
    // the stores whose storage was emptied. A resize is different — it recreates
    // every sidecar, so every store really did lose its vectors.
    allStale: resized,
  });
  ...
  setImmediate(async () => {
    try {
      ...
      for (const row of [...memPending, ...wikiPending, ...selfPending]) {
        ...
        // `deferred` above is a boot-time snapshot taken before this loop was
        // scheduled — it keeps the two backfills from duplicating work, but it
        // cannot see a reindex that starts *after* boot, and this loop can run
        // for a long time. Re-gate per row (issue #340) so a reindex claiming
        // a store mid-backfill takes its rows over rather than racing us for
        // them in a different embedding space.
        const storeName = isWiki ? "wiki" : isSelf ? "self_memories" : "memories";
        currentOp = (async () => {
          const { embedding } = await embedForStore(store, storeName,
            // SQLite's wiki adapter exposes `body_md`, while the Postgres
            // query and memory stores expose `content`.
            () => generateEmbeddingFn(`${row.title}. ${row.content ?? row.body_md ?? ""}`));
          if (embedding) {
