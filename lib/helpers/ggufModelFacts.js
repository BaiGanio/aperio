...
import { isGgufWeightsFile, isMmprojFile } from "./ggufFiles.js";
...
const RELEVANT_SUFFIXES = ["block_count", "context_length", "embedding_length", "attention.head_count", "attention.head_count_kv", "attention.key_length", "attention.value_length", "attention.sliding_window", "full_attention_interval", "expert_count", "expert_used_count", "chat_template"];
...
// Shared by findCachedGguf and hasCachedMmproj so both walk the exact same
// repo→cache-dir + refs/main→snapshots/<rev> resolution. Returns null (no
// throw) for an uncached repo, a bare alias with no "/", or a broken refs file
// — never touches disk beyond the refs/main read and the snapshot dir's own
// existence check.
export function resolveSnapshotDir(model, cacheRoot) {
  const root = repoCacheDir(model, cacheRoot);
  if (!root) return null;
  let revision;
  try { revision = readFileSync(join(root, "refs", "main"), "utf8").trim(); } catch { return null; }
  const snapshot = join(root, "snapshots", revision);
  return existsSync(snapshot) ? snapshot : null;
}
...
export function findCachedGguf(model, cacheRoot) {
  ...
  const snapshot = resolveSnapshotDir(model, cacheRoot);
  if (!snapshot) return null;
  let files;
  try { files = readdirSync(snapshot).filter(isGgufWeightsFile); } catch { return null; }
...
// True ground truth for "can this local model see images?": its cache
// snapshot (or, for a bare .gguf path, its own directory) holds a companion
// mmproj*.gguf vision projector. Returns the absolute path, or null.
export function hasCachedMmproj(model, cacheRoot) {
  const isBareFile = String(model).toLowerCase().endsWith(".gguf") && existsSync(model);
  const dir = isBareFile ? resolve(model, "..") : resolveSnapshotDir(model, cacheRoot);
  if (!dir) return null;
  let files;
  try { files = readdirSync(dir); } catch { return null; }
  if (isBareFile) {
    // An HF cache snapshot dir is exclusively one repo's own files, so any
    // mmproj found there is unambiguously that model's companion. A bare-file
    // directory carries no such guarantee — a user keeping several models
    // side by side (e.g. ~/models/llama-70b.gguf next to
    // ~/models/gemma-vlm.gguf + mmproj-gemma.gguf) would otherwise have the
    // text-only model wrongly marked vision-capable, sending it raw images it
    // cannot process. Only trust the pairing when this is the sole model
    // weights file present — an unambiguous 1:1 match.
    if (files.filter(isGgufWeightsFile).length > 1) return null;
  }
  const hit = files.find(isMmprojFile);
  return hit ? join(dir, hit) : null;
}

// Whether a GGUF's own chat template renders a tool-calling block. Permissive
// on missing/unreadable data (returns true) — this only ever narrows tool
// availability for a model whose template proves it can't accept tools
// (e.g. llava), never withholds tools for a model we simply couldn't read.
// Reads the template transiently to test it; never returns or retains the
// string itself (readGgufMetadata's `meta` is function-local here).
export function hasToolCallingTemplate(path) {
  try {
    const meta = readGgufMetadata(path);
    const template = meta["tokenizer.chat_template"];
    if (typeof template !== "string") return true;
    return /\btools\b|\btool_call\b/.test(template);
  } catch {
    return true;
  }
}
