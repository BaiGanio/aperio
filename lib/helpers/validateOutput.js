...
function sanitiseHtml(text) {
  ...
  // Use Unicode private-use chars (/) as delimiters — not in any regex above.
  const blocks = [];
...
export function validateOutputSafe(text, label = "output") {
  ...
  if (cleaned !== text) {
    ...
    logger.debug(`[validateOutput] ${label}: ${diff > 0 ? "fixed unclosed fence" : "sanitised content"} (${Math.abs(diff)} chars)`);
  }
  return cleaned;
}
