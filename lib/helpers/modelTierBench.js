...
function validateArgumentAssertions(value, label) {
  if (value === undefined) return [];
  if (!Array.isArray(value) || value.some(item => !item || typeof item !== "object" ||
    typeof item.tool !== "string" || !item.tool || !item.arguments || typeof item.arguments !== "object" ||
    Array.isArray(item.arguments))) {
    throw new Error(`${label} must be an array of tool argument assertions`);
  }
  return value.map(item => ({ tool: item.tool, arguments: structuredClone(item.arguments) }));
}
...
export function validateBenchmarkCases(input) {
  ...
  return input.map((raw, index) => {
    ...
    const argumentAssertions = validateArgumentAssertions(raw.argumentAssertions, `${at}.argumentAssertions`);
    ...
    return {
      ...
      timeoutMs: Number.isFinite(raw.timeoutMs) && raw.timeoutMs > 0 ? raw.timeoutMs : DEFAULT_CASE_TIMEOUT_MS,
      stateAssertion,
...
export function describeBenchmarkCase(caseDef) {
  return {
    ...
    argumentAssertions: (caseDef.argumentAssertions ?? []).map(assertion => ({
      tool: assertion.tool,
      arguments: structuredClone(assertion.arguments),
    })),
...
function argumentsMatch(actual, expected) {
  ...
  return expectedKeys.every(key => {
    ...
    if (left && typeof left === "object" && right && typeof right === "object") return argumentsMatch(left, right);
    return left === right;
  });
}

function isArgumentContractOnlyFailure(result) {
  return result?.status === "fail" && result.argumentAssertionsPassed === false &&
    result.completed === true && result.toolSequencePassed === true &&
    result.answerTermsPassed === true && result.toolsSuccessful === true &&
    result.statePassed === true && Array.isArray(result.argumentAssertions) &&
    result.argumentAssertions.length > 0;
}

export function rescoreBenchmarkRun(run) {
  const rescored = structuredClone(run);
  const changedCases = [];
  if (run?.status !== "complete" || !Array.isArray(run.caseResults)) {
    return { run: rescored, changedCases, rescored: false, reason: "run is not a complete persisted artifact" };
  }
  rescored.caseResults = run.caseResults.map((result, index) => {
    if (!isArgumentContractOnlyFailure(result)) return result;
    const assertions = result.argumentAssertions.map(assertion => ({
      ...assertion,
      passed: assertion.observed !== null && argumentsMatch(assertion.observed, assertion.expected),
    }));
    if (!assertions.every(assertion => assertion.passed)) return result;
    changedCases.push(result.id ?? String(index));
    return {
      ...result,
      argumentAssertions: assertions,
      argumentAssertionsPassed: true,
      status: "pass",
      rescore: { matcher: "argument-subset", sourceStatus: "fail" },
    };
  });
  return {
    run: rescored,
    changedCases,
    rescored: changedCases.length > 0,
    reason: changedCases.length ? null : "no argument-contract false negatives found",
  };
}
...
export function evaluateBenchmarkCase(caseDef, events, { statePassed = true } = {}) {
  ...
  const argumentAssertions = (caseDef.argumentAssertions ?? []).map(assertion => {
    const matching = starts.find(event => event.name === assertion.tool && event.arguments !== undefined);
    const observed = matching?.arguments ?? null;
    return {
      tool: assertion.tool,
      expected: structuredClone(assertion.arguments),
      observed: observed === null ? null : structuredClone(observed),
      passed: matching !== undefined && argumentsMatch(observed, assertion.arguments),
    };
  });
  const argumentAssertionsPassed = argumentAssertions.every(assertion => assertion.passed);
  const passed = completed && toolSequencePassed && argumentAssertionsPassed && answerTermsPassed && toolsSuccessful && statePassed;
