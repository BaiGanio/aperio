...
const VALID_PROVIDERS = new Set(["anthropic", "deepseek", "gemini", "ollama", "llamacpp", "codex"]);

// Which .env var holds the model name for each local (non-cloud) engine.
const LOCAL_MODEL_VAR = { ollama: "OLLAMA_MODEL", llamacpp: "LLAMACPP_MODEL" };
...
export function writeEnvFromWizard({ provider, apiKey, model, port }) {
  ...
  if (isCloud) {
    if (apiKey?.trim()) out = setKey(out, PROVIDER_KEY_VAR[provider], apiKey.trim());
    if (model?.trim()) {
      const MODEL_VAR = { anthropic: "ANTHROPIC_MODEL", deepseek: "DEEPSEEK_MODEL", gemini: "GEMINI_MODEL", codex: "CODEX_MODEL" };
      out = setKey(out, MODEL_VAR[provider], model.trim());
    }
  } else {
    out = setKey(out, LOCAL_MODEL_VAR[provider], model.trim());
  }
