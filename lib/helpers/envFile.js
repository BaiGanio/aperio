...
export function writeEnvFromWizard({ provider, apiKey, model, port }) {
  ...
  // Fresh installs only (WS8): run_node_script/run_python_script are gated
  // behind this same switch as run_shell (mcp/tools/shell.js), and the
  // bundled pptx/docx/pdf skills need them to build documents. Baking "1"
  // into the file this wizard creates keeps document generation working out
  // of the box; an existing install never reaches this branch (line 62
  // returns early when .env already exists), so its own setting — including
  // "never set, defaults off" — is untouched.
  out = setKey(out, "APERIO_ENABLE_SHELL", "1");
