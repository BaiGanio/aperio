...
// throwing. This mirrors the pattern used by ensureLlamaCpp() so it can be
...
export function isEphemeralPort(port) {
  return port === 0 || (typeof port === "string" && port.trim() === "0");
}
...
export async function ensurePort(port, { wait = false } = {}) {
  // Port 0 is not a concrete shared resource: it asks the OS to choose an
  // available ephemeral port when the real HTTP server binds. Probing it first
  // allocates a different ephemeral port and can fail in constrained test
  // environments, which was then misreported as "Port 0 is occupied".
  if (isEphemeralPort(port)) {
    logger.info("✅ Port 0 requested — the OS will assign an available port");
    return;
  }
