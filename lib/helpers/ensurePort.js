...
function killPids(pids) {
  for (const pid of pids) {
    try {
      ...
      logger.warning(`⚡ Killed PID ${pid} occupying port`);
    } catch (e) {
      // Process may have already exited — not an error worth surfacing.
      logger.warning(`⚠️  Could not kill PID ${pid}: ${e.message}`);
    }
  }
}
...
export async function ensurePort(port) {
  ...
  logger.warning(`⚠️  Port ${port} is in use — looking for the occupying process…`);

  const pids = pidsOnPort(port);
  if (pids.length) {
    logger.warning(`🔪 Killing PID(s): ${pids.join(", ")}`);
    killPids(pids);
  } else {
    logger.warning("⚠️  Could not identify occupying PID — waiting for port to free on its own…");
  }
