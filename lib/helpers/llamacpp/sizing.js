...
export function serveCtxFor(modelKey, env, hardware, profile, extraOpts = {}) {
  ...
  // than a 4K window. An 8 GiB-class machine running E2B has room for an 8K KV
  // cache; smaller machines retain conservative RAM-fit sizing. Auxiliary/VLM
  // calls carry an explicit ceiling and must not inherit the main-agent floor.
  const isE2BDefault = /\/gemma-4-E2B-it-qat-GGUF(?::|$)/i.test(String(modelKey));
  const agentFloor = isE2BDefault && !extraOpts.ceiling && totalRamGB >= 7.5 && configuredAgentFloor > 0
