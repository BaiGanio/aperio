...
export function serveCtxFor(modelKey, env, hardware, profile, extraOpts = {}) {
  ...
  // than a 4K window. An 8 GiB-class machine running either curated small
  // Gemma 4 agent (E2B or E4B) has room for an 8K KV cache; smaller machines
  // retain conservative RAM-fit sizing. Auxiliary/VLM calls carry an explicit
  // ceiling and must not inherit the main-agent floor.
  const isCuratedSmallAgentModel = /\/gemma-4-E[24]B-it-qat-GGUF(?::|$)/i.test(String(modelKey));
  const agentFloor = isCuratedSmallAgentModel && !extraOpts.ceiling && totalRamGB >= 7.5 && configuredAgentFloor > 0
