...
import { factsForHf, resolvePerfProfile, resolveKvCachePolicy, defaultLocalModel } from "../../providers/index.js";
...
// The default model selection itself is configuration; its sizing metadata is
// loaded from the model_facts DB table before this preset is built.
export const DEFAULT_VLM_MODEL = CONFIG.find(entry => entry.key === "LLAMACPP_VLM_MODEL")?.default || "";
...
export function buildModelsPreset(env = process.env, hardware = {}) {
  ...
  // fact from model_facts, if one is declared.
  if (!omitVlm) {
