...
export function createEmbeddingQueue({ store, generateEmbedding, storeName = 'memories', setEmbedding }) {
  // id → { text, attempts, nextRetryAt }
  const queue = new Map();
  // Defaults to the `memories` table's setter — a queue built for another
  // table (e.g. self_memories) must pass its own, or a successful retry here
  // would write into the wrong table's rows.
  const applyEmbedding = setEmbedding ?? ((id, embedding) => store.setEmbedding(id, embedding));
  ...
  async function flush() {
    ...
    for (const [id, entry] of queue) {
      ...
      try {
        ...
        await applyEmbedding(id, embedding);
        queue.delete(id);
