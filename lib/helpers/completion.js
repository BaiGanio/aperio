...
export async function complete(messages, { maxTokens = 600 } = {}) {
  ...
  // Send the router alias (aperio-main), not the raw HF id: requesting the raw
  // repo:quant makes llama.cpp's router load a SECOND, full-context copy of the
  // model from its auto-discovered cache preset — doubling resident RAM (and, on
  // tight machines, an OOM). Mirrors the interactive path (agent/providers/
  // llamacpp.js). requestModel is unset for non-llama providers, so this is a
  // no-op there.
  const res = await fetch(`${provider.baseURL}/chat/completions`, {
    method:  'POST',
    headers,
    body:    JSON.stringify({ model: provider.requestModel || provider.model, messages, max_tokens: maxTokens }),
  });
