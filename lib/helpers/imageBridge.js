 ...
 * Used to decide whether the main model needs the VLM text-bridge at all.
 *
 * @param {string} name - model name, e.g. "qwen2.5vl:7b"
 * @returns {boolean}
 */
export function isVisionModel(name = "") {
  return /(?:llava|bakllava|moondream|minicpm-?v|llama3\.2-vision|vision|vl(?::|-|$)|gemma[34]n?)/i.test(name);
}
