...
// engine directly — so the only thing that needs to track AI_PROVIDER here is
// which model name to show the user in progress/log text.
const VLM_MODEL = (process.env.AI_PROVIDER || "").toLowerCase() === "llamacpp" ? LLAMACPP_VLM_MODEL : OLLAMA_VLM_MODEL;

/**
 * Heuristically detect whether an Ollama model name can see images natively.
 ...
 */
export function isStandaloneVisionRequest(text = "") {
  const t = String(text)
  ...
  for (const msg of messages) {
    ...
    for (let i = 0; i < imageBlocks.length; i++) {
      ...
      try {
        ...
        const desc = await callTool("describe_image", { data: img.source.data, prompt });
