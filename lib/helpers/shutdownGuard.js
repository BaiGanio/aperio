...
export function createWatchdog({
  ...
  if (!enabled) {
    // Idle guard off, but an explicit Quit must still stop the server.
    return { heartbeat: () => {}, stop: () => {}, quit: () => process.kill(process.pid, "SIGTERM") };
  }
  ...
  // (stops Ollama if safe, then exits) instead of waiting out the timer.
  function quit()      { clearTimeout(timer); return onIdle(); }
