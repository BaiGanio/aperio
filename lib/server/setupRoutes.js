...
import { runBootstrap, bootstrapEvents, stepState, STEPS, getBootstrapDownload } from "../../bootstrap.js";
...
export function registerSetupRoutes({
  ...
  // Offline D3 for the codegraph Map view (#283): served from the pinned npm
  // package through an explicit route so the UI never reaches for a CDN.
  app.get("/vendor/d3.min.js", (_req, res) => {
    res.sendFile(resolve(root, "node_modules/d3/dist/d3.min.js"));
  });
  // Prism syntax highlighting (#466): served from the pinned npm package for the
  // same reason as d3 above, but the trigger here was Subresource Integrity.
  // SRI cannot protect the CDN build: prism-autoloader injects a fresh <script>
  // for every language grammar it needs at runtime, and those injected tags
  // carry no integrity attribute — a compromised CDN could ship arbitrary code
  // through them while the hashed autoloader itself still verified. Self-hosting
  // takes the CDN out of the script path entirely instead.
  app.get("/vendor/prismjs/prism.js", (_req, res) => {
    res.sendFile(resolve(root, "node_modules/prismjs/prism.js"));
  });
  app.get("/vendor/prismjs/plugins/autoloader/prism-autoloader.min.js", (_req, res) => {
    res.sendFile(resolve(root, "node_modules/prismjs/plugins/autoloader/prism-autoloader.min.js"));
  });
  // Grammar files, requested by the autoloader. The allowlist regex is the path
  // guard: only `prism-<lang>.min.js` resolves, so no `..` segment or absolute
  // path can escape the components directory.
  app.get("/vendor/prismjs/components/:file", (req, res, next) => {
    if (!/^prism-[a-z0-9+#-]+\.min\.js$/.test(req.params.file)) return next();
    res.sendFile(resolve(root, "node_modules/prismjs/components", req.params.file));
  });
  // Bootstrap Icons (#466): the last CDN asset that SRI could not actually
  // protect. The stylesheet itself was hash-pinned, but it then requests
  // ./fonts/bootstrap-icons.woff2 — and SRI does not cascade to a sub-resource
  // a stylesheet fetches, so that font arrived unverified. A compromised CDN
  // could ship arbitrary font bytes while the pinned stylesheet still passed.
  // Serving both halves from the pinned npm package removes the CDN entirely.
  app.get("/vendor/bootstrap-icons/bootstrap-icons.min.css", (_req, res) => {
    res.sendFile(resolve(root, "node_modules/bootstrap-icons/font/bootstrap-icons.min.css"));
  });
  // The font files the stylesheet above requests. Same guard shape as the Prism
  // grammar route: the allowlist regex IS the path guard, so no `..` segment or
  // absolute path can escape the fonts directory. Express has already stripped
  // the `?<cache-buster>` suffix the stylesheet appends before `:file` is bound,
  // so the query-string form resolves like any other request.
  app.get("/vendor/bootstrap-icons/fonts/:file", (req, res, next) => {
    if (!/^bootstrap-icons\.woff2?$/.test(req.params.file)) return next();
    res.sendFile(resolve(root, "node_modules/bootstrap-icons/font/fonts", req.params.file));
  });
  // Codegraph Atlas (#283 step 6): its own page, not a modal in index.html —
  // opened in a new tab from the Code Graph panel's Map button, so it gets a
  // real window instead of a fixed-size card.
  app.get("/codegraph-atlas", (_req, res) => {
    res.sendFile(resolve(root, "public/codegraph-atlas.html"));
  });
  ...
  app.get("/api/setup/specs", setupLimiter, async (_req, res) => {
    try {
      // model sizing facts are database-backed; opening the selected backend
      // applies migrations and hydrates the synchronous runtime snapshot.
