...
export async function search(store, { query, kind, repo, limit = 20 }, { generateEmbedding, vectorEnabled }) {
  ...
  if (!queryVec) {
    ...
    params.push(limit);
    const { rows } = await pool.query(
      `SELECT s.qualified, s.kind, s.name, s.signature, s.start_line, s.end_line, f.path,
    ...
    return { matches: rows, mode: 'fulltext' };
  }
  ...
  const { rows } = await pool.query(`
    ...
    SELECT s.qualified, s.kind, s.name, s.signature, s.start_line, s.end_line, f.path, fu.rrf AS score
      FROM fused fu JOIN cg_symbols s ON s.id = fu.id JOIN cg_files f ON f.id = s.file_id
     ORDER BY fu.rrf DESC LIMIT $${idx}
  `, params);
  return { matches: rows, mode: 'hybrid' };
}

export async function outline(store, { path: filePath }) {
  const { rows } = await store.pool.query(
    `SELECT s.kind, s.name, s.qualified, s.start_line, s.end_line, s.signature
       FROM cg_symbols s JOIN cg_files f ON f.id = s.file_id
      WHERE f.path = $1 ORDER BY s.start_line`, [filePath]
  );
  return { path: filePath, symbols: rows };
}

export async function context(store, { qualified }) {
  const { rows } = await store.pool.query(
    `SELECT s.start_line, s.end_line, s.signature, s.doc, s.kind, s.name,
            f.path, r.root_path
       FROM cg_symbols s
       JOIN cg_files   f ON f.id = s.file_id
       JOIN cg_repos   r ON r.id = f.repo_id
      WHERE s.qualified = $1 LIMIT 1`, [qualified]
  );
  return rows[0] ?? null;
}
...
async function walkEdges(pool, qualified, depth, direction) {
  const { rows: target } = await pool.query(`SELECT id FROM cg_symbols WHERE qualified = $1 LIMIT 1`, [qualified]);
  ...
  for (let hop = 1; hop <= depth; hop++) {
    ...
    const { rows } = await pool.query(
      direction === 'callers'
        ? `SELECT e.src_symbol_id AS id, s.qualified, s.kind, s.name, f.path, e.src_line
             FROM cg_edges e JOIN cg_symbols s ON s.id = e.src_symbol_id
             JOIN cg_files f ON f.id = s.file_id
            WHERE e.kind = 'calls' AND e.dst_symbol_id = ANY($1)`
        : `SELECT e.dst_symbol_id AS id, s.qualified, s.kind, s.name, f.path, e.src_line
    ...
    for (const r of rows) {
      ...
      seen.add(r.id);
      next.push(r.id);
      out.push({ hop, qualified: r.qualified, kind: r.kind, name: r.name, path: r.path, line: r.src_line });
    }
    frontier = next;
  }
  return out;
...
export async function callers(store, { qualified, depth = 1 }) {
  return walkEdges(store.pool, qualified, Math.min(Math.max(depth, 1), 5), 'callers');
}
export async function callees(store, { qualified, depth = 1 }) {
  return walkEdges(store.pool, qualified, Math.min(Math.max(depth, 1), 5), 'callees');
}
