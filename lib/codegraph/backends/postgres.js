...
// cg_symbols.embedding entirely) would otherwise leave its symbols
...
// re-enqueues every other root's pending symbols on its own startup (up to
// one embedding call per symbol per root before the 5s queue debounce
// catches up).
...
export async function listRepoRoots(store) {
  const { rows } = await store.pool.query(`SELECT root_path FROM cg_repos ORDER BY root_path`);
...
export async function listSymbolsWithoutEmbeddings(store, rootPath, { limit = null, offset = 0 } = {}) {
  const { rows } = await store.pool.query(
    `SELECT s.id, s.name, s.signature, s.doc
      ...
      WHERE s.embedding IS NULL AND r.root_path = $1
