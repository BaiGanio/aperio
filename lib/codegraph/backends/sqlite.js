...
// Symbols missing a vector, in the same {id, text} shape indexRepoFiles()
// hands to the embed queue. The watcher's own bulkPending only covers
// symbols touched by *this* indexing pass — a repo that hasn't changed since
// the last index never gets requeued, so a provider change (which wipes
// vec_cg_symbols entirely) would otherwise leave its symbols permanently
// unembedded. Called once at watcher startup alongside bulkPending, scoped to
// rootPath's own repo — otherwise every watched root re-enqueues every other
// root's pending symbols on its own startup (up to one embedding call per
// symbol per root before the 5s queue debounce catches up).
// Every indexed repo root. Used by the reindex driver (issue #287, WS1) to
// fan the per-root pending scans above across all roots without introducing a
// second, graph-wide query shape.
export async function listRepoRoots(store) {
  return store.db.prepare(`SELECT root_path FROM cg_repos ORDER BY root_path`)
    .all().map(r => r.root_path);
}

// The ORDER BY is what makes `offset` mean the same thing across the reindex
// driver's successive pages.
export async function listSymbolsWithoutEmbeddings(store, rootPath, { limit = null, offset = 0 } = {}) {
  const db = store.db;
  const rows = db.prepare(`
    SELECT s.id, s.name, s.signature, s.doc
    FROM cg_symbols s
    JOIN cg_files f ON f.id = s.file_id
    JOIN cg_repos r ON r.id = f.repo_id
    LEFT JOIN vec_cg_symbols v ON v.rowid = s.id
    WHERE v.rowid IS NULL AND r.root_path = ?
    ORDER BY s.id
    LIMIT ? OFFSET ?
  `).all(rootPath, limit ?? -1, offset);
  return rows.map(s => ({ id: s.id, text: [s.name, s.signature, s.doc].filter(Boolean).join('. ') }));
}
