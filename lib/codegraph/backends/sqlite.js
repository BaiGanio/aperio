...
export async function search(store, { query, kind, repo, limit = 20 }, { generateEmbedding, vectorEnabled }) {
  ...
  // ── FTS-only path ──────────────────────────────────────────────────────────
  if (!queryVec) {
    const params = { q: query, cap: limit };
  ...
  const detail = db.prepare(`
    SELECT s.id, s.qualified, s.kind, s.name, s.signature, s.start_line, s.end_line, f.path
      FROM cg_symbols s JOIN cg_files f ON f.id = s.file_id
     WHERE s.id IN (${placeholders})
  `).all(...idList);
  const byId = new Map(detail.map(r => [r.id, r]));
  const matches = ids.map(([id, score]) => ({ ...byId.get(id), score }));
...
async function walkEdges(db, qualified, depth, direction) {
  const target = db.prepare(`SELECT id FROM cg_symbols WHERE qualified = ? LIMIT 1`).get(qualified);
...
export async function callers(store, { qualified, depth = 1 }) {
  return walkEdges(store.db, qualified, Math.min(Math.max(depth, 1), 5), 'callers');
}
export async function callees(store, { qualified, depth = 1 }) {
  return walkEdges(store.db, qualified, Math.min(Math.max(depth, 1), 5), 'callees');
}
