...
import { loadCachedGraph } from './graphCache.js';
...
export async function ensureAnalysis(mod, store, repoId) {
  ...
  const graph = await loadCachedGraph(mod, store, repoId);
  _computeCount++;
...
export async function buildGraphPayload(mod, store, repoId, { limit = 300, focus = [], community = null } = {}) {
  ...
  const { nodes: allNodes, edges } = await loadCachedGraph(mod, store, repoId);
