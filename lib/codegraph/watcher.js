...
export async function startWatcher(store, rootPath, events) {
  ...
  const rootAlreadyKnown = (await listRepoRoots(store)).includes(rootPath);
  ...
  try {
    ...
    const deferredToReindex = rootAlreadyKnown && (await pendingStoreNames(store)).has("codegraph");
