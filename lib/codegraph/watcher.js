...
import { indexRepo, indexFile, removeFile, sweepMissing, isCodegraphAvailable, listPendingEmbeddings, SKIP_DIRS, JS_EXT } from './indexer.js';
...
export async function startWatcher(store, rootPath, events) {
  ...
  try {
    ...
    // change (which wipes vec_cg_symbols outright) would otherwise leave them
    // permanently unembedded. Same query shape as initEmbeddings() for
    // memories: fast no-op in the steady state, catches everything a plain
    // reindex would miss when it isn't. enqueueMany dedupes by id, so overlap
    // with bulkPending above is harmless.
