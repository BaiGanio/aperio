 ...
 * @param {pg.Pool} pool
 * @param {string}  rootPath  absolute path; must be inside the allowed-folders list
...
export async function startWatcher(store, rootPath, events, {
  ...
  if (!checkReadPath(rootPath)) {
    throw new Error(`codegraph watcher refused: ${rootPath} not in the allowed-folders list`);
  }
...
/**
 * Start watchers for every path in the allowed-folders list.
