...
// The Bash grammar bundled by tree-sitter-wasms can throw from its external
// scanner for otherwise valid scripts (notably larger scripts using nested
// case/compound commands). Code graph indexing is best-effort, so retain the
// useful declaration symbols instead of dropping the whole file and logging a
// noisy startup error. This intentionally extracts only function declarations;
// shell commands do not produce useful call/import edges for the graph.
function extractBashFallback(source) {
  const symbols = [];
  const re = /^(?:function\s+)?([A-Za-z_][A-Za-z0-9_]*)\s*(?:\(\s*\))?\s*\{/gm;
  let match;
  while ((match = re.exec(source)) !== null) {
    const line = source.slice(0, match.index).split("\n").length;
    symbols.push({
      localId: `s${symbols.length}`,
      kind: "function",
      name: match[1],
      qualified: match[1],
      start_line: line,
...
export async function extract(source, filePath) {
  ...
  try {
    ...
    if (ext === "sh" || ext === "bash" || ext === "zsh") {
      return extractBashFallback(source);
    }
