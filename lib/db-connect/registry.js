...
import { openManagedSqlite, managedEncryptionKey } from "./drivers/managed-sqlite.js";
...
import { isManagedExtractionFile, adoptManagedExtractionRow } from "./extraction.js";
 ...
 *  `provisioned` rows (Aperio-owned files, e.g. the self-provisioned
 *  `extraction` connection) open through the encryption-aware managed path
 *  instead of the plain one — never for a user's own external sqlite file,
 *  which openSqlite continues to open exactly as before. The `provisioned`
 *  flag alone is never trusted: it's stored, user-reachable data, so it must
 *  also match a known managed file's real, recomputed path (requires `store`;
 *  callers with no store handle — e.g. testConnectionConfig — never get the
 *  managed path). */
export async function openDriver(cfg, store) {
  ...
  if (cfg.engine === "sqlite") {
    return cfg.provisioned && store && isManagedExtractionFile(cfg, store)
      ? await openManagedSqlite({ file: cfg.file, readOnly, keyBuf: managedEncryptionKey() })
      : openSqlite({ file: cfg.file, readOnly });
  }
...
export async function getDriver(store, name) {
  ...
  // One-time legacy adoption on the resolution path: a v1-era extraction row
  // (saved file hashed from the raw `options`) is migrated to the stable
  // current path the first time ANY operation — read, schema, or write —
  // resolves this connection, so a later behavior-only option edit cannot
  // orphan the adopted row (P1 review finding). Cheap no-op for every other
  // connection and for already-current extraction rows.
  const effective = await adoptManagedExtractionRow(store, decrypted);
  const driver = await openDriver(effective, store);
