...
//
// Dialect-aware bind-parameter counting/description for the database tool
// (issue #170). Split out of ./classify.js (2026-08-16) — the statement
// classifier/router (splitStatements, classify, the isAllowedFor* gates) and
// this placeholder analyzer are two distinct concerns that happen to share a
// file's worth of quoting/comment-masking logic, but neither calls into the
// other's internals. See classify.js for the routing half.
...
/**
 * Same quoting/comment rules as splitStatements() in ./classify.js, but
 * literal and comment spans are collapsed to a single space rather than kept
 * verbatim, so a placeholder-shaped token inside a string ('?', '$1',
 * ':name', …) can never be counted as a real bind parameter. Also masks
 * dialect-specific forms splitStatements doesn't need to know about but
 * placeholder counting does, each gated to the dialect that actually has the
 * construct:
 ...
 *
 * `backslashEscapesOverride` (true | false | null/undefined) lets a caller
 * override that per-engine default with a per-CONNECTION fact — a server
 * actually running MySQL's `NO_BACKSLASH_ESCAPES` or Postgres's legacy
 * `standard_conforming_strings = off` sees the opposite of the engine's
 * usual default, and there's no way to detect that without a live
 * round-trip query per connection (rejected as disproportionate — see
 * `id/reference/tech-debt.md`). null/undefined keeps today's engine-guess
 * behavior unchanged. It never overrides a Postgres `E'...'`/`e'...'`
 * literal, which interprets backslash escapes unconditionally regardless of
 * `standard_conforming_strings`.
 */
function maskLiteralsAndComments(sql, engine, backslashEscapesOverride) {
  let out = "";
  let i = 0;
  const n = sql.length;
  ...
  while (i < n) {
    ...
    if (c === "'" || c === "`" || (c === '"' && engine !== "mysql")) {
      ...
      const backslashEscapes = isPostgresEString ? true :
        backslashEscapesOverride ?? (engine === "mysql");
      i++;
      while (i < n) {
        const ch = sql[i];
 ...
 *
 * `backslashEscapesOverride` — see maskLiteralsAndComments; passed straight
 * through, unused for engines other than mysql/postgres.
 */
export function countPlaceholders(sql, engine, backslashEscapesOverride) {
  const masked = maskLiteralsAndComments(String(sql ?? ""), engine, backslashEscapesOverride);
