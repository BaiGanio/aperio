 ...
 * port and database name only — never the embedded user or password. Hashing
 * the raw connection string would make the extraction path change on an
 * ordinary, expected password rotation (or any other credential-only edit to
 * the connection URL); the stored `file` would then stop matching
 * isManagedExtractionFile's recomputed path and every extraction operation
 * would report a reserved-name collision, making the old database
 * permanently inaccessible through Aperio for no operational reason.
 */
function postgresIdentity(connectionString) {
  ...
  // libpq keyword/value form: "host=... port=... dbname=... user=... password=...".
  const kv = {};
  ...
  if (kv.host || kv.dbname) {
    return `${kv.host || "localhost"}:${kv.port || "5432"}/${kv.dbname || ""}`;
  }
