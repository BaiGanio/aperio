...
/**
 * The error to show when the reserved "extraction" name can't be resolved.
 * Two very different situations share the same symptom, and the person
 * reading the message needs to know which one they're in: a genuinely
 * unrelated connection that happens to use the name (nothing to recover,
 * just rename it), versus a row carrying this module's own `provisioned`
 * marker whose file no longer matches this profile's current identity — most
 * often because the database connection settings changed around an app
 * upgrade (see "Db-connect — extraction identity / managed lock" in
 * id/reference/tech-debt.md). The second case can't be auto-recovered — the
 * old identity a stale row's hash was derived from can't be un-hashed — but
 * the file itself is still on disk, so its path is surfaced for manual
 * recovery instead of just saying "rename it."
 */
export function reservedExtractionNameError(cfg, store) {
  if (isManaged(cfg)) {
    return Object.assign(
      new Error(
        `"${EXTRACTION_CONNECTION}" is reserved for Aperio's self-provisioned document-extraction database. ` +
        `The connection named "${cfg.name}" looks like an EARLIER one Aperio provisioned for this same purpose, ` +
        `but its saved file no longer matches this profile's current identity — this usually happens when the ` +
        `database connection settings changed around an app upgrade. Aperio can't safely reuse it automatically. ` +
        `If you had extracted documents before, they may still be on disk at "${cfg.file}" — you can recover that ` +
        `file by hand. To continue, rename this connection in Settings → Database connections to free the ` +
        `reserved name; Aperio will provision a fresh one next time it needs to write.`
      ),
      { userFacing: true }
    );
  }
  return collisionError(cfg?.name);
}
...
export async function provisionExtractionConnection(store) {
  ...
  if (existing) {
    ...
    throw reservedExtractionNameError(existing, store);
  }
