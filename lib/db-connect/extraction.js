...
export function isManagedExtractionFile(cfg, store) {
  ...
  // Adoption is in place: the legacy path is recognized as this module's own
  // file for THIS profile and used exactly as before — never renamed, because
  // a v0 path is derived only from host/port/database and may legitimately be
  // shared by several upgraded profiles whose rows were created under the old
  // identity, and a rename would silently hand that shared file to whichever
  // profile migrated first. The acceptance set stays namespace-constrained
  // (var/extraction/<hash>.db for hashes of THIS profile's own connection
  // string or its ancestors), so the forged-`provisioned` TOCTOU protection
  // below is unchanged: an attacker-chosen path still never matches.
