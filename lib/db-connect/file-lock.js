...
// marker left by a reclaimer that crashed mid-reclaim is stolen once it is
// older than RECLAIM_MARKER_STALE_MS, so a crash cannot wedge reclamation (or
// the 15s acquire deadline) forever.
//
...
function reclaimIfStale(lockPath) {
  ...
  try {
    ...
    // on the next poll. Only a marker old enough to be abandoned is stolen.
    try {
      if (Date.now() - statSync(markerPath).mtimeMs > RECLAIM_MARKER_STALE_MS) unlinkSync(markerPath);
  ...
  try {
    ...
    try { unlinkSync(markerPath); } catch { /* already stolen or gone */ }
  }
}
