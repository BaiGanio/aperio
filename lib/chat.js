...
function initHeader(mode, model, dockerOn, db, showReasoning) {
  _headerMode      = mode;
  _headerModel     = model;
...
async function runProxy(port, initialReasoning) {
  ...
  initHeader(`proxy :${port}`, `${process.env.AI_PROVIDER} (${process.env.OLLAMA_MODEL})`, isDocker(), isDocker() ? 'postgres' : 'lancedb', showReasoning);
...
async function runStandalone(initialReasoning) {
  ...
  initHeader("standalone", `${provider.name} (${provider.model})`, isDocker(), isDocker() ? 'postgres' : 'lancedb', showReasoning);
