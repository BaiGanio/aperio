...
const DB_PROVIDER = IS_DOCKER ? 'postgres' : 'lancedb';
// ─── ANSI ─────────────────────────────────────────────────────────────────────
const R      = "\x1b[0m";
const BOLD   = "\x1b[1m";
const DIM    = "\x1b[2m";
...
function redrawHeader() {
  ...
  const line1 = `  ${BOLD}${CYAN}Aperio mode:${R}  ${GRAY}${_headerMode}${R}  •  ${_headerModel}${R}  •  Docker: ${CYAN}${IS_DOCKER ? 'on': 'off'}${R}  •  DB: ${CYAN}${_headerDB}${R}`;
...
function initHeader(mode, model, db, showReasoning) {
  _headerMode      = mode;
  _headerModel     = model;
  _headerDB        = db;
...
function isDocker() {
  try {
    // Check for the .dockerenv file or "docker" in cgroups
...
function pidsOnPort(port) {
  try {
    ...
    return [...out.matchAll(/pid=(\d+)/g)].map(m => parseInt(m[1]));
  } catch {}
  try {
...
async function runProxy(port, initialReasoning) {
  ...
  initHeader(`proxy :${port}`, "server", DB_PROVIDER, showReasoning);
...
async function runStandalone(initialReasoning) {
  ...
  initHeader("standalone", `${provider.name} (${provider.model})`, DB_PROVIDER, showReasoning);
