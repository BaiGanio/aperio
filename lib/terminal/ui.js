...
import {
  R, BOLD, DIM, CYAN, GRAY, GREEN, YELLOW, RED, MAGENTA,
  getHeaderInfo,
...
const DEFAULT_LLAMACPP_MODEL =
  CONFIG.find(entry => entry.key === "LLAMACPP_MODEL")?.default || "(unset)";
...
// ── The mascot, in four lines ─────────────────────────────────────────────────
// The same robot that greets you on the site and in the web UI — antenna,
// headphones, two round eyes. Drawn beside the help header only when the
// terminal is wide enough to keep it out of the text's way.
const MASCOT = [
  "     ╷      ",
  "  ┌───────┐ ",
  " ═┫ ◉ ▬ ◉ ┣═",
...
const MASCOT_MIN_COLS = 80;

/**
 * Header lines with the robot down the left margin. Falls back to the plain
 * text lines on a narrow terminal, so nothing ever wraps mid-drawing.
 */
function helpHeaderLines(lines) {
  if ((process.stdout.columns || 80) < MASCOT_MIN_COLS) return lines.map(l => `  ${l}`);
  const gap = "  ";
  return lines.map((line, i) => {
    const art = MASCOT[i] ?? " ".repeat(MASCOT[0].length);
    return `${MAGENTA}${art}${R}${gap}${line}`;
  });
}

// ── printHelp — plain-language command guide (path-aware) ─────────────────────
export function printHelp({ proxy = false, showExamples = true, lang = "en", version = "" } = {}) {
  const S = resolveStrings(lang);
  ...
  // The robot stands beside the whole header block — title, rule and both
  // lines of intro prose — so it never ends up next to half a sentence.
  const introLines = S.help_intro.split("\n").map(l => l.trim());
  const header = helpHeaderLines([
    `${BOLD}${S.help_title}${R}${version ? `${DIM}   v${version}${R}` : ""}`,
    `${GRAY}${"─".repeat(58)}${R}`,
    ...introLines.map(l => `${GRAY}${l}${R}`),
  ]);
  process.stdout.write(`\n${header.join("\n")}\n`);
...
export async function printConfig({ port } = {}) {
  ...
  if (provider === "llamacpp") {
    out += row("LLAMACPP_MODEL", "LLAMACPP_MODEL", DEFAULT_LLAMACPP_MODEL);
