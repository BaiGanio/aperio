...
/**
 * Checks if provider is llamacpp (case-insensitive)
 */
export function isLlamaCppProvider(providerName) {
  return (providerName || "").toLowerCase() === "llamacpp";
}
...
/**
 * Checks if command is a model-switch command (e.g. "model anthropic claude-haiku-4-5-20251001")
 */
...
/**
 * Builds the content array for a chat message carrying queued attachment
 * blocks. The user's typed text must be the FIRST block: every downstream
 * consumer that classifies a turn's intent (tool-profile selection, skill
 * matching, standalone-vision detection) extracts "the first text block" as
 * the user's request. An image attachment contributes its own `[Image: name]`
 * label block ahead of the pixel block — putting those before the user's text
 * would make that label the extracted intent instead, silently discarding the
 * actual request (see the ws handler, which uses this same ordering).
 */
export function buildAttachedUserContent(pendingAttachmentBlocks, raw) {
  return pendingAttachmentBlocks.length > 0
    ? [{ type: "text", text: raw }, ...pendingAttachmentBlocks]
    : raw;
}
