...
/**
 * Checks if command asks for help — bare ("help" / "?") or focused on a single
 * command ("help attach"). Multi-word lines like "help me plan my week" are not
 * help (they're chat); only one trailing token is allowed.
 */
export function isHelpCommand(cmd) {
  return /^(help|\?)(\s+\S+)?$/i.test(cmd.trim());
}

/**
 * Extracts the target of a focused help request ("help attach" → "attach"),
 * or null for bare help. Used to pick per-command docs vs. the full guide (#178).
 */
