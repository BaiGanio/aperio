    ...
    while (true) {
      ...
      outer: while (true) {
        ...
        for (const line of lines) {
          ...
          // ── Delegate all reasoning extraction to the adapter ─────────────
          const { contentToken } = reasoningAdapter.processDelta(delta, adapterState, adapterEmit);

          // ── Handle content token returned by the adapter ──────────────────
          if (contentToken) {
            fullText += contentToken;

            if (OLLAMA_THINKS) {
              // Thinking models: stream content tokens immediately
              emit(emitter, { type: "token", text: contentToken });
            } else {
              // Non-thinking models: buffer in case it turns out to be a tool call
      ...
      if (OLLAMA_THINKS) {
        ...
        if (intercepted) {
          ...
          emit(emitter, { type: "stream_start" });
          emit(emitter, { type: "stream_end", text: fixUnclosedFence(cleanText) });
...
export async function handleUserRequest(userMessage) {
  ...
  if (matchedSkill) {
    ...
    console.log("No specific skill matched. Falling back to general response.");
  }
}