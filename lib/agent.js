...
function trimByTokens(msgs, hwm, contextWindow) {
...
export async function createAgent({ root, version, clientName = "Aperio-agent" }) {
  ...
  // The agent identity lives at id/whoami.md (singular source of truth).
  ...
  // Map of user-interface locales the UI can request → English language name
  // we tell the model. Mirrors public/scripts/i18n.js LOCALE_META.
  const LANG_NAMES = {
  ...
  function buildLanguageDirective(lang) {
    ...
    return `The user's interface language is "${name}" (locale code: ${lang}). ` +
      `Respond to the user in ${name} using natural, native phrasing — ` +
      `unless the user explicitly writes in another language, in which case mirror that language. ` +
      `Translate any non-${name} content the user attaches into ${name} when summarising. ` +
      `Keep code, identifiers, file paths, command-line snippets, and proper names in their original form.`;
  }

  const getSystemPrompt = (userMessage = "", lang = "en") => {
  ...
  async function runAnthropicLoop(messages, emitter, opts = {}) {
    ...
    while (true) {
      ...
      const stream = provider.client.messages.stream({ model: provider.model, max_tokens: 8192, system: getSystemPrompt(lastUserText, opts.lang), tools: anthropicTools, messages: trimmed });
  ...
  async function runOllamaLoop(messages, emitter, opts = {}, getAbort = () => null, setAbort = () => {}) {
    let tokenHWM = 0; // true HWM — never decreases, survives tool-call rounds
    while (true) {
      tokenHWM = Math.max(tokenHWM, streamUsage.input_tokens);
      streamUsage = { input_tokens: 0, output_tokens: 0, thinking_tokens: 0 };
      if (getAbort()?.signal?.aborted) { emitter.send({ type: "stream_end", text: "", usage: streamUsage }); return ""; }
      // First call: estimate directly so trim logic runs even before the first API report
      const hwm = tokenHWM > 0 ? tokenHWM : messages.reduce((s, m) => s + estimateMsgTokens(m), 0);
      ...
      const lastUserMsg = [...trimmed].reverse().find(m => m.role === "user");
      const lastUserText = typeof lastUserMsg?.content === "string" ? lastUserMsg.content : lastUserMsg?.content?.find?.(b => b.type === "text")?.text ?? "";
      const systemPrompt = getSystemPrompt(lastUserText, opts.lang);
  ...
  const alwaysOnSkillNames = getAlwaysOnSkills(skillIndex).map(s => s.name);

  return {
    provider, mcpTools, alwaysOnSkillNames,
