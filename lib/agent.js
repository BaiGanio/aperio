...
export function resolveProvider() {
  ...
  if (PROVIDER === "ollama") {
    return {
      ...
      contextWindow: parseInt(process.env.OLLAMA_NUM_CTX || "32768", 10),
    };
  }
...
function inferZodType(zodField) {
  ...
  // Zod v4: def.type is a plain string ("string", "number", "boolean", "array", "enum", "optional", …)
  if (def.type) {
    if (def.type === "optional") {
      // unwrap and recurse into the inner type
      return inferZodType(def.innerType);
    }
    if (def.type === "enum") return "string";
    if (def.type === "array") return "array";
    return def.type; // "string" | "number" | "boolean" | …
  }
  // Zod v3 fallback: def.typeName is "ZodString", "ZodNumber", etc.
...
export function zodToJsonSchema(zodSchema) {
  ...
  // Zod v4 exposes shape as a plain object; v3 exposes it as a function
...
function trimByTokens(messages, inputTokens, contextWindow) {
  if (inputTokens < contextWindow * CTX_TRIM_TARGET) return { messages, dropped: 0 };

  const pressure = (inputTokens - contextWindow * CTX_TRIM_TARGET) /
                   (contextWindow * (1 - CTX_TRIM_TARGET));
  const dropFraction = Math.min(0.5, pressure * 0.5);
  const toDrop = Math.floor((messages.length - 1) * dropFraction);
  if (toDrop <= 0) return { messages, dropped: 0 };

  const kept = Math.max(CTX_MIN_MESSAGES, messages.length - toDrop);
  return { messages: [messages[0], ...messages.slice(-(kept - 1))], dropped: toDrop };
}
...
function dropOrphanedToolResults(messages) {
  ...
  return i === 1 ? messages : [messages[0], ...messages.slice(i)];
}
...
class OllamaStreamHandler {
  ...
  processDelta(delta) {
    ...
    // Accumulate reasoning_content for history (DeepSeek thinking mode requirement)
...
class ToolExecutor {
  ...
  async executeThinkingResponse(cleanText, toolCalls, streamHandler, isThinkingModel, reasoningContent = null) {
    ...
    if (toolCalls.length > 0) {
      this.emitter.send({ type: "reasoning_done" });
      await this.executeToolCalls(toolCalls, cleanText, reasoningContent);
      return null; // Continue loop
    }

    this.emitter.send({ type: "reasoning_done" });
    this.emitter.send({ type: "stream_end", text: "", usage: streamUsage });
    this.messages.push({ role: "assistant", content: [{ type: "text", text: cleanText }], ...(reasoningContent ? { reasoning_content: reasoningContent } : {}) });
    return cleanText;
  }
...
export async function createAgent({ root, version, clientName = "Aperio-agent" }) {
  ...
  const alwaysOnContent = getAlwaysOnSkills(skillIndex)
    .map(s => s.content)
    .join("\n\n---\n\n");
  ...
  const getSystemPrompt = (userMessage = "") => {
    ...
    if (onDemand) {
      logger.info(`🎯 Skill matched: ${onDemand.name}`);
      parts.push(onDemand.content);
    }
  ...
  function toOpenAIMessages(messages, systemPrompt) {
    return [
      ...
      ...messages.flatMap(m => {
        // Tool results: expand into one tool message per result
  ...
  async function runOllamaLoop(messages, emitter, opts = {}, getAbort = () => null, setAbort = () => {}) {
    while (true) {
      // Save token count from the previous call BEFORE resetting — used for trimming.
      ...
      const pct = prevInputTokens > 0
        ? Math.round((prevInputTokens / provider.contextWindow) * 100)
        : 0;

      if (pct >= Math.round(CTX_WARN_TARGET * 100) && pct < Math.round(CTX_TRIM_TARGET * 100)) {
        emitter.send({ type: "context_warning", pct });
      }

      const { messages: rawMessages, dropped } = prevInputTokens > 0
        ? trimByTokens(messages, prevInputTokens, provider.contextWindow)
        : {
            messages: messages.length > MAX_HISTORY
              ? [messages[0], ...messages.slice(-(MAX_HISTORY - 1))]
              : messages,
            dropped: 0,
          };
      ...
      if (dropped > 0) {
        logger.info(`[agent] context trimmed: dropped ${dropped} messages at ${pct}% pressure`);
      ...
      if (OLLAMA_THINKS) {
        const result = await toolExecutor.executeThinkingResponse(cleanText, toolCalls, streamHandler, true, reasoningContent);
        if (result !== null) return result;
      } else {
        const result = await toolExecutor.executeNonThinkingResponse(cleanText, toolCalls, streamHandler, reasoningContent);
        if (result !== null) return result;
      }
