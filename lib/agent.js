...
class OllamaStreamHandler {
  ...
  processChunk(chunk) {
    for (const line of chunk.split("\n")) {
      ...
      if (parsed.usage) streamUsage = { input_tokens: parsed.usage.prompt_tokens ?? 0, output_tokens: parsed.usage.completion_tokens ?? 0 };
...
class ToolExecutor {
  ...
  async executeThinkingResponse(cleanText, toolCalls, streamHandler, isThinkingModel, reasoningContent = null) {
    ...
    if (toolCalls.length > 0) { this.emitter.send({ type: "reasoning_done" }); await this.executeToolCalls(toolCalls, cleanText, reasoningContent); return null; }
  ...
  async executeNonThinkingResponse(cleanText, toolCalls, streamHandler, reasoningContent = null) {
    if (toolCalls.length > 0) { await this.executeToolCalls(toolCalls, cleanText, reasoningContent); return null; }
  ...
  async function runAnthropicLoop(messages, emitter) {
    while (true) {
      ...
      for await (const event of stream) {
        ...
        if (event.type === "message_start") streamUsage = { input_tokens: event.message.usage.input_tokens ?? 0, output_tokens: event.message.usage.output_tokens ?? 0 };
