...
const TRANSLATIONS = {
  en: {
    ...
    startup_bd_more:          "more…",

    idm_title:                "Startup prompt details",
    idm_identity_heading:     "Identity — id/whoami.md",
    idm_identity_hint:        "This is the identity Aperio reads at startup. Edits are saved to id/whoami.md and take effect on the next turn.",
    idm_identity_saved:       "Saved",
    idm_identity_save_error:  "Could not save whoami.md.",
    idm_tools_heading:        "Tool schemas",
    idm_tools_empty:          "No tools available for the current model.",
    idm_memory_heading:       "Memory index",
    idm_memory_open_browser:  "Open memory browser",
    idm_load_error:           "Could not load this information.",
    ...
    ctx_cost_unavailable:     "No pricing data available. Aperio fetches prices from OpenRouter at boot.",
    ...
    setup_step_of:            "{n} of {total} complete",
    ...
    wiz_ram:                  "total RAM available to Aperio",
    wiz_disk:                 "free for model cache at",
    ...
    /* Settings overlay (#252) */
    stov_nav_label: "Config",
    stov_nav_title: "Configuration — every setting, saved to the database",
    stov_title: "Settings",
    stov_sub: "saved to the database · .env can override",
    stov_search_ph: "Search settings…",
    stov_mode_simple: "Simple",
    stov_mode_advanced: "Advanced",
    stov_close: "Close",
    stov_loading: "Loading…",
    stov_load_failed: "Failed to load configuration",
    stov_cat_provider: "Provider & Models",
    stov_cat_provider_blurb: "Which AI does the thinking. The local llama.cpp engine is free and private; cloud providers need an API key.",
    stov_cat_memory: "Memory & Privacy",
    stov_cat_memory_blurb: "What Aperio remembers, for how long, and how it is protected at rest.",
    stov_cat_features: "Features",
    stov_cat_features_blurb: "Optional abilities: code & document graphs, shell tool, background agents, integrations.",
    stov_cat_network: "Network & Security",
    stov_cat_network_blurb: "Bootstrap and hardening. These load before the database exists, so they are edited in .env and shown here read-only.",
    stov_cat_advanced: "Advanced",
    stov_cat_advanced_blurb: "Timeouts, logging, tool-safety knobs, and everything imported from an unmanaged .env.",
    stov_nav_note_html: "Every value here can also live in <code>.env</code> — see <code>docs/config-reference.md</code>. Set <code>APERIO_CONFIG_PRECEDENCE=env</code> to make the file win.",
    stov_readonly_env: "edit in .env",
    stov_src_env: "from .env",
    stov_src_db: "from UI",
    stov_src_default: "default",
    stov_secret_set: "configured ✓",
    stov_secret_unset: "not set",
    stov_secret_keep_ph: "•••• set (blank = keep)",
    stov_secret_unset_ph: "paste value — stored write-only",
    stov_none: "none",
    stov_encrypt_hint: "Database encryption is off — secrets are stored in plaintext. Recommended: enable APERIO_DB_ENCRYPT, or keep keys in .env with APERIO_CONFIG_PRECEDENCE=env.",
    stov_empty_search: "No settings match \"{q}\" — try the Advanced view.",
    stov_empty_simple: "Nothing here in Simple view — switch to Advanced.",
    stov_unsaved: "{n} unsaved change(s)",
    stov_save: "Save",
    stov_discard: "Discard",
    stov_save_failed: "Save failed: {err}",
    stov_restart_needed: "Saved. Restart Aperio for the changes to take effect.",
    stov_restart_now: "Restart now",
    stov_restart_later: "Later",
    stov_reindex_warning: "You changed the embedding provider or dimensions. Existing embeddings no longer match — rebuild the vector index (a fresh database) after restart, or old memories/graph results won't be found.",
    stov_precedence_env_notice: "APERIO_CONFIG_PRECEDENCE=env — values written in .env override the UI. Fields marked \"from .env\" are controlled by the file; saving them here has no effect until the line is removed from .env.",
    stov_restarting_title: "Restarting Aperio…",
    stov_restarting_msg: "Applying your changes. This usually takes a few seconds.",
    stov_reconnecting_title: "Reconnecting…",
    stov_reconnecting_msg: "Aperio is starting back up.",
    stov_still_starting_title: "Still starting…",
    stov_still_starting_msg: "This is taking longer than usual — the page will reload automatically as soon as Aperio is back.",
  },
};
...
function pickInitialLang() {
  ...
  // Server-detected locale, stamped as <html data-aperio-lang="…"> (CSP-safe;
  // an inline window.__APERIO_LANG__ script would be refused by script-src).
  const fromServer = document.documentElement?.dataset?.aperioLang;
  if (fromServer && SUPPORTED_LANGS.includes(fromServer)) return fromServer;
