...
runScript('add_slide', () => {
  const unpackedDir = requireArg(process.argv[2], 'Usage: node scripts/add_slide.js <unpacked_dir> <source>');
  const source = requireArg(process.argv[3], 'Usage: node scripts/add_slide.js <unpacked_dir> <source>');
  ...
  function getNextSlideId() {
    ...
    if (!existsSync(presPath)) {
      throw Object.assign(new Error(`presentation.xml missing in ${dir}`), { code: 'BAD_INPUT_DIR' });
    }
  ...
  function addToContentTypes(dest) {
    ...
    if (!existsSync(ctPath)) {
      throw Object.assign(new Error(`[Content_Types].xml missing in ${dir}`), { code: 'BAD_INPUT_DIR' });
    }
  ...
  function addToPresentationRels(dest) {
    ...
    if (!existsSync(relsPath)) {
      throw Object.assign(new Error(`presentation.xml.rels missing in ${dir}`), { code: 'BAD_INPUT_DIR' });
    }
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideLayout" Target="../slideLayouts/${source}"/>
    ...
    if (!existsSync(srcSlide)) {
      throw Object.assign(new Error(`source slide not found: ${srcSlide}`), { code: 'INPUT_MISSING' });
    }
  ...
  // Verify the new slide file landed on disk.
  const newSlideAbs = resolve(join(slidesDir, dest));
  if (!existsSync(newSlideAbs)) {
    throw Object.assign(new Error(`add_slide claims success but ${newSlideAbs} is missing`), { code: 'OUTPUT_MISSING' });
  }

  console.log(`✅ add_slide: created ${newSlideAbs} from ${source}`);
