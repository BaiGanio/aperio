...
- Localized terminal welcome/help — English by default; `lang <code>` switches and persists, or set `APERIO_UI_LANG` in `.env` (saved pref wins over env); translations overlay `cli_`-prefixed keys in the shared `public/locales/<lang>.json`, falling back to English per-string
