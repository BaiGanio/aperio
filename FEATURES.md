...
- Multi-repo support, honors the allowed-folders list (`APERIO_ALLOWED_PATHS`)
...
- Native image and skill support for Codex and Claude Code — both providers previously silently dropped any attached image and never received Aperio's skill matching. Codex now decodes image content blocks to temp files under the session scratch dir and passes them via the CLI's real `-i/--image <FILE>...` flag (cleaned up after the turn); Claude Code now sends its prompt through the Agent SDK's `AsyncIterable<SDKUserMessage>` form carrying the same Anthropic-shaped image blocks Aperio already builds. Both providers also now run Aperio's own skill matcher each turn (a new `ctx.getSkillsBlock`, reusing the same match/dedup guard every other provider uses) instead of pointing native `.codex/skills/`/`.claude/skills/` discovery at Aperio's `skills/` directory — which would have bypassed the skill panel's enable/disable state and the forced `/skill` command — so the `skills_matched` chip now fires for these providers too. Fixed alongside: Claude Code's `runClaudeCodeLoop` was silently discarding its entire `opts` parameter, so no `extraSystem` content (memory pointers, post-compression RAG context, workspace directives) ever reached a Claude Code turn; it now reaches the model via the SDK's `systemPrompt.append` channel, concatenated with the skill content. `providerDropsImages()`/the image-drop notice machinery stays in place (now empty) for a future text-only provider.
...
- Reasoning / thinking mode with reasoning-chain replay, at parity across all providers — llama.cpp/DeepSeek's native reasoning fields, Anthropic extended thinking (`ANTHROPIC_THINKING_BUDGET`, off by default — thinking tokens are billed output), Gemini `includeThoughts`, and Claude Code's SDK-default adaptive thinking all stream the same collapsed `reasoning_start`/`reasoning_token`/`reasoning_done` bubble with a real thinking-token count from the provider's own usage breakdown (not an estimate, where the API reports one). Codex surfaces its `--json` `reasoning` item the same way (`CODEX_REASONING_SUMMARY`, default `auto` — free, just a summary of tokens already billed as `reasoning_output_tokens`). Anthropic's `redacted_thinking` blocks (present when thinking content is encrypted rather than shown) are preserved verbatim across turns, as required for a subsequent tool-use turn to validate.
- Error and empty-turn bubble parity across providers — Anthropic no longer throws past its provider loop on a failed stream open or a mid-stream error; both now stream the shared `⚠️` token bubble + `stream_end` every other provider loop uses, instead of surfacing through a separate error path. A single shared "(model produced no response)" fallback (`emitEmptyResponseFallback`, `lib/tools/executor.js`) now covers a genuinely empty or whitespace-only completion on every provider, not just llama.cpp/DeepSeek.
- Honest capability signals for Codex and Claude Code — both build their prompt from the last user message's text only, so an attached image is invisible to the model on these two providers; attaching one now surfaces a visible notice (`providerDropsImages`, `lib/providers/index.js`) at send time instead of the attachment silently vanishing, and the turn continues text-only. Skills matching also does not run for these two providers (neither calls `getSystemPrompt`) — this is a known, documented gap rather than a silent one: no `skills_matched` chip is shown for Codex or Claude Code turns.
...
- Background-agents UI panel — right-side sidebar with live master switch, per-job trigger/mode/last-verdict, "Run now", and per-job run history (`lib/routes/api-agents.js`, `public/scripts/agents-panel.js`). The job form favors plain English over raw config for non-coders (#169): 7 one-click templates with plain-English labels (nightly/hourly dedup, daily backup, doc/memory/priority digests, code-change changelog), debounce/timeout shown in seconds and preset minutes instead of raw milliseconds, freeform provider/model/timeout collapsed behind an "Advanced" section, steps-JSON parse errors reported with line/column, and job-id/watcher-source/tool hints rewritten in plain English.
- Visual (no-JSON) steps builder (#326) — `steps` mode renders each step as a card with a tool dropdown sourced from the live MCP registry (`GET /api/agents/tools`, `lib/workers/background-job-tools.js`) and dynamic input fields driven by that tool's schema, plus add/reorder (drag handle or up/down)/delete per step; a synchronized raw-JSON `<textarea>` stays available for power users and for registered tools outside the curated visual subset. Steps are validated against the live tool registry at create/update time, not just at run time.
- "What should this job do?" wizard (#327) — a plain-English textarea on the new-job form ("every night, clean up duplicate memories and regenerate their embeddings") posts to `POST /api/agents/wizard`, which asks the active model for a suggested job (trigger + either a steps pipeline restricted to the visual-builder's tool catalog, or a freeform prompt) and prefills the form for review. Suggestions default to `enabled: false`; a malformed or unusable model response fails clearly (502) rather than producing a broken draft, and step-validation warnings surface inline instead of silently being dropped.
...
- Confirm-on-write gate — `write_file` / `edit_file` / `append_file` execute directly for any target already inside the allowed-folders list (`APERIO_ALLOWED_PATHS`, which grants read and write alike); two-phase token-confirmed only when the turn is tainted by untrusted content (edit shows a capped unified diff); `delete_file` always uses the same durable approval path regardless of taint
...
- Model-tier dashboard export — automatically writes a metrics-only
  ...
  `docs/benchmarks/pilot/qualification-data.js`, which
  `docs/benchmarks/pilot/qualification.html` loads automatically; prompts, answers,
...
- Test suite via `npm test` — unit, integration, e2e, the agent-loop harness,
  the doc-intelligence suite, and the continuous-audit gates; e2e and audit
  gates can also run independently with `npm run test:e2e` and
  `npm run test:audit` (the latter also has its own CI workflow)
- Real-app E2E route coverage expansion (`npm run test:e2e:real`):
