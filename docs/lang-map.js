/**
 * lang-map.js — world-map language switcher for the landing page.
 *
 * Ported from public/scripts/lang-map.js (the Aperio Web UI). Opens from the
 * navbar globe button. A world map (framed on Europe) where countries whose
 * language the page speaks glow in purple, planned languages render dashed,
 * and everything else stays muted. Clicking a country (or a row in the list
 * pane) switches the page language via setLang() from translations.js.
 *
 * Country shapes (world-paths.js, ~880 KB) are lazy-loaded on first open —
 * they are a straight copy of public/scripts/world-paths.js.
...
(function () {
  ...
  // Native + English names per available language (mirrors the Web UI's
  // LOCALE_META in public/scripts/i18n.js).
  const LOCALE_META = {
  ...
  function init() {
    ...
    svg.addEventListener("pointermove", (e) => {
      ...
      tip.innerHTML = `<div class="lm-tip-country"></div>`;
    ...
    async function selectLang(lang) {
      // setLang comes from translations.js; it fetches the locale on demand.
      await setLang(lang);
      renderCounts();
      renderList();
