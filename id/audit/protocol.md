...
Use this as the starting prompt for future audit sessions. Load alongside a
role-specific lens from this directory (e.g. `security.md`, `software-architect.md`)
for a focused deep-dive, or run alone for a broad baseline review.
...
3. Check worktree status first and avoid touching unrelated user changes.
4. Read `package.json`, `README.md`, `SECURITY.md`, `server.js`.
5. Inspect route modules under `lib/routes/`.
6. Inspect WebSocket handling in `lib/emitters/handlers/wsHandler.js`.
7. Inspect path/tool boundaries in `lib/routes/paths.js`, `mcp/tools/files.js`, and `mcp/tools/shell.js`.
8. Run `npm test`. If local listener tests fail with `EPERM`, rerun with local network permissions.
9. Report findings first, ordered by severity, with file/line references.
10. For a deeper dive on one axis, load the matching lens from this directory:
    - `security.md` — threat model, vulns, trust boundaries
    - `software-architect.md` — coupling, modularity, error propagation
    - `code-reviewer.md` — bugs, edge cases, unsafe operations
    - `socratic-questioner.md` — assumptions, blind spots, untested claims
    - `product-thinker.md` — UX coherence, feature fit, user-facing impact
