...
| Semantic search returns nothing and the log says `no prebuilt extension for win32-arm64` | Expected on Windows-on-ARM: `sqlite-vec` publishes no build for it, so the vector sidecars are ordinary tables and recall is full-text only. Everything else works. `npm run embeddings:reindex` refuses there on purpose — the vectors would be unsearchable and discarded on the next machine that can load the extension |
| Every store went stale after moving `var/aperio.db` to another machine | The sidecars are physically rebuilt when the database crosses between a machine that can load `sqlite-vec` and one that cannot, which destroys the vectors either way. They are marked stale and refilled by the background reindex on the next server start |
