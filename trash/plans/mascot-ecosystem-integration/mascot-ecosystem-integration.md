...
**Body, not head** (developer call, 2026-07-31): the whole mascot appears wherever it has
room, one size notch larger than first drafted. The head mark survives only in the one
slot where a body is illegible — the 18px inline terminal icon — and in the favicons.

| Asset | Purpose | Spec |
|-------|---------|------|
| `avatar-56.png` (+`avatar-112.png` @2x) | chat AI avatar (44px circle) | full body, trimmed, centred in a square, transparent |
| `body-256.webp` | page headers / 404 / section edges | full body, 256², WebP for page weight |
| `mono.png` | empty states, offline banner | full body desaturated at opacity 0.35, so empties feel quiet, not loud |
| `head-64.webp` | terminal demo inline icon | 64px head — the body reads as a smudge at 18px |

Generated by `npm run gen:mascot` (`scripts/gen-mascot-derivatives.js`); `npm run
gen:mascot:check` fails if the committed files drift from the masters. The script also
mirrors the PNG favicon set into `public/assets/mascot/` for the app's own pages.
...
WS0 status: assets generated 2026-07-31, contact sheet rendered for approval.
...
1. **`.mascot-bubble` component** (docs/styles.css): full-body robot (96px) + speech-bubble
   with tail, aurora edge. Used in exactly **three** places (restraint — the bubble is
...
2. **Terminal demo**: swap the `🤖` emoji (`terminal_provider` row) for `head-64.webp`
   at 18px inline — the robot literally *is* the provider line.
3. **`docs/404.html`**: GitHub Pages serves it automatically. Full-body robot +
   "This page isn't in my memory." + link home. Self-contained (inline CSS, absolute
   `/aperio/` asset paths) because the page is served at arbitrary depths. En-only.
4. **Wallpaper strip**: footer row linking the four JPEGs through generated
   `*-thumb.webp` previews, labelled by resolution only — numbers need no translation,
   and the full 120–320 KB JPEGs never load with the page.
...
WS3 status: done 2026-07-31 — 3 bubbles (dark/light/bg verified), terminal icon,
404 page, wallpaper strip; 41 KB of new bytes on the landing page (budget 60 KB);
reduced-motion verified; zero new i18n keys.

### WS4 — Repo & beyond (~1 h, optional tail)

1. **CLI `help` banner** — **adopted 2026-07-31**. Four-line ASCII robot (antenna,
   headphones, two eyes) down the left margin of the help header, in aurora magenta,
   with the version beside the title. Falls back to the plain header under 80 columns.
2. **Demo videos**: future recordings open on the mascot card (note for the demo-recording
   harness; no work now) — logged in A2D.
3. **Mascot name** — **decided 2026-07-31: the mascot is Aperio.** "Meno" was declined;
   the robot is the project, not a separate character, so no signature line and no
   first-person rewording of the 404.

**Works when**: developer has explicitly approved/declined each item. → Tests: **T4**.
WS4 status: done 2026-07-31 — banner adopted, name decided, demo-video note logged.
