...
**Status:** CLOSED 2026-08-02, dual-backend-proven — Steps 1–3 (T-G3 migrations, T-G4 handlers,
T-G5 MCP surface, including T-G5.2's full e2e round-trip) all done and verified against
**both SQLite and live Postgres**, plus 5 rounds of code review closing 7 findings (dedup never
wired into the exposed flow, keyword-matching substring false positives, frozen template
confidence, unverified log-record trust, propose-token confirmation bypass, tools unreachable
from the web agent, `database` not loaded alongside extraction intent) and one claimed finding
investigated and found not reproducible. 5123/5123 (SQLite) across the full project test suite;
5222/5222 with `APERIO_E2E_POSTGRES_URL` set including the new Postgres coverage (§ below).
Docs synced (AGENTS.md, id/reference/mcp-tools.md, FEATURES.md, CHANGELOG.md) and folded back
into the epic doc's own evidence log. WS0-R/WS1 are green on the local hero model; WS2's skill
passes T-G2.1/2.2/2.4 live but T-G2.3 (SQL provenance) is a confirmed false pass on gemma4
...
| 2026-08-02 | Postgres verification — T-G3.1/T-G4/T-G5/T-G5.2, the previously-open gap, now closed | **All green against a real, isolated scratch Postgres (`pgvector/pgvector:pg16` in a disposable, non-persistent container on a non-default port — deliberately NOT the shared `docker/docker-compose.yml` `aperio_data` volume, which turned out to predate this session by over a month and was left untouched).** T-G3.1: all 14 migrations apply cleanly via the real `db/migrate.js` path, a second run is a true no-op, `ON DELETE SET NULL` and all three CHECK constraints (bad JSON, out-of-range confidence, invalid verification_state) verified directly via `psql` — identical behavior to the SQLite result. T-G4/T-G5/T-G5.2: two new opt-in test files, gated on `APERIO_E2E_POSTGRES_URL` (same convention as `postgres-vec-meta.test.js`/`tests/integration/db/contract/backends.js`, each provisioning and dropping its own throwaway probe database) — `tests/integration/handlers/extraction/extractionHandlers.postgres.test.js` (14 tests: CRUD via `RETURNING *`, Postgres 23505 unique-violation mapping on both create and rename, multi-template ranking/substring-safety, cold-start reject-persists-nothing, digest-tamper rejection against `agent_interrupts`' native `jsonb` column, extraction_log dedup/coexistence/FK-null-out edge cases) and `tests/integration/handlers/extraction/wsG5-2-e2e.postgres.test.js` (10 tests: the same full sample-bill chain as the SQLite `wsG5-2-e2e.test.js`, run against the real household-gen corpus with Aperio's own store backed by Postgres). **All 24 new tests passed on the first run with zero code changes required** — the store.pool branches written throughout WS3 were correct from the start; the gap was verification, not a latent bug. `tests/integration/mcp/tool-profile-coverage.test.js` reconfirmed green. Full regression sweep with `APERIO_E2E_POSTGRES_URL` set: 5222/5222 clean on a TAP-reporter run; two earlier `npm test` runs each showed exactly one unrelated failure (`tests/integration/db/postgres-vec-meta.test.js`, issue #287 territory, predates WS3) that did not reproduce standalone, in combination with the two new files, or on a third full run — concluded to be a pre-existing, load-induced flake in that file under full ~1047-suite parallel contention against a single-CPU-limited local scratch instance, not a WS3 regression (verified by re-running the full suite once more with the two new files entirely absent from the glob — same class of full-suite load, no crash reproduced either way, consistent with intermittent timing sensitivity rather than a deterministic interaction). Not chased further as it sits outside this task's boundary (AGENTS.md: work only the assigned task). Scratch container and both probe databases confirmed torn down with zero stray state. WS3 is now dual-backend-proven, closing the epic doc's own evidence log gap. |
